...
let ConfigurationMigrationWorkbenchContribution = class ConfigurationMigrationWorkbenchContribution extends Disposable {
  ...
  async migrateConfigurationsForFolderAndOverride(migration, resource) {
    ...
    const targetPairs = this.workspaceService.getWorkbenchState() === 3 ? [['user', 2], ['userLocal', 3], ['userRemote', 4], ['workspace', 5], ['workspaceFolder', 6]] : [['user', 2], ['userLocal', 3], ['userRemote', 4], ['workspace', 5]];
    for (const [dataKey, target] of targetPairs) {
      ...
      if (migrationValues.length) {
        await Promise.allSettled(migrationValues.map(async ([[key, value], overrideIdentifiers]) => this.configurationService.updateValue(key, value.value, {
          resource,
          overrideIdentifiers
        }, target)));
      }
    ...
    registry.registerConfiguration({
      ...
      'properties': {
        'security.allowedUNCHosts': {
          ...
          'default': [],
          'markdownDescription': localizeWithPath(_moduleId, 5, 'A set of UNC host names (without leading or trailing backslash, for example `192.168.0.1` or `my-server`) to allow without user confirmation. If a UNC host is being accessed that is not allowed via this setting or has not been acknowledged via user confirmation, an error will occur and the operation stopped. A restart is required when changing this setting. Find out more about this setting at https://aka.ms/vscode-windows-unc.'),
          'scope': 2
        },
        'security.restrictUNCAccess': {
          'type': 'boolean',
          'default': true,
          'markdownDescription': localizeWithPath(_moduleId, 6, 'If enabled, only allows access to UNC host names that are allowed by the `#security.allowedUNCHosts#` setting or after user confirmation. Find out more about this setting at https://aka.ms/vscode-windows-unc.'),
          'scope': 2
        }
      }
    });
...
exports.applicationConfigurationNodeBase = applicationConfigurationNodeBase;
exports.problemsConfigurationNodeBase = problemsConfigurationNodeBase;
exports.securityConfigurationNodeBase = securityConfigurationNodeBase;
exports.workbenchConfigurationNodeBase = workbenchConfigurationNodeBase;