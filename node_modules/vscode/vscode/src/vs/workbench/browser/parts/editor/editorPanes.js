...
const _moduleId = "vs/workbench/browser/parts/editor/editorPanes";
let EditorPanes = class EditorPanes extends Disposable {
  ...
  async doShowErrorDialog(error, editor) {
    ...
    if (!message) {
      message = localizeWithPath(_moduleId, 0, "Unable to open '{0}'", editor.getName());
    }
    const buttons = [];
    if (errorActions && errorActions.length > 0) {
      ...
      buttons.push({
        label: localizeWithPath(_moduleId, 1, "&&OK"),
        run: () => undefined
      });
    }
