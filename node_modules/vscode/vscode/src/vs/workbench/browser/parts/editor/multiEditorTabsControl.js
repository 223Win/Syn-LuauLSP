...
const {} = require('./media/multieditortabscontrol.css.js');
const {
  isWindows,
  isMacintosh
...
} = require('../../../../editor/common/services/treeViewsDndService.js');
const {
  DraggedTreeItemsIdentifier
} = require('../../../../editor/common/services/treeViewsDnd.js');
const {
...
} = require('../../../common/editor/filteredEditorGroupModel.js');
const {
  IHostService
} = require('../../../services/host/browser/host.service.js');
var MultiEditorTabsControl_1;
const _moduleId = "vs/workbench/browser/parts/editor/multiEditorTabsControl";
let MultiEditorTabsControl = class MultiEditorTabsControl extends EditorTabsControl {
  ...
  didActiveEditorChange() {
    if (!this.activeTabLabel?.editor && this.tabsModel.activeEditor || this.activeTabLabel?.editor && !this.tabsModel.activeEditor || !this.activeTabLabel?.editor || !this.tabsModel.isActive(this.activeTabLabel.editor)) {
      return true;
    }
    return false;
  }
  ...
  moveEditor(editor, fromTabIndex, targeTabIndex) {
    ...
    }, Math.min(fromTabIndex, targeTabIndex), Math.max(fromTabIndex, targeTabIndex));
    this.layout(this.dimensions, {
      forceRevealActiveTab: true
    });
  }
  ...
  createTab(tabIndex, tabsContainer, tabsScrollbar) {
    ...
    const tabActionBar = new ActionBar(tabActionsContainer, {
      ariaLabel: localizeWithPath(_moduleId, 0, "Tab actions"),
  ...
  registerTabListeners(tab, tabIndex, tabsContainer, tabsScrollbar) {
    ...
    disposables.add(new DragAndDropObserver(tab, {
      onDragStart: e => {
        ...
        this.editorTransfer.setData([new DraggedEditorIdentifier({
          editor,
          groupId: this.groupView.id
      ...
      onDragEnd: async e => {
        ...
        const editor = this.tabsModel.getEditorByIndex(tabIndex);
        if (!isNewWindowOperation || isWindowDraggedOver() || !editor) {
          return;
        }
  ...
  doLayoutTabsWrapping(dimensions) {
    ...
    if (this.groupsView.partOptions.wrapTabs) {
      ...
      if (tabsWrapMultiLine) {
        if (tabsContainer.offsetHeight > dimensions.available.height || allTabsWidth === visibleTabsWidth && tabsContainer.offsetHeight === this.tabHeight || !lastTabFitsWrapped()) {
  ...
  doLayoutTabsNonWrapping(options) {
    ...
    if (this.blockRevealActiveTab || typeof activeTabPosX !== 'number' || typeof activeTabWidth !== 'number' || activeTabPositionStatic || !dimensionsChanged && !options?.forceRevealActiveTab) {
...
MultiEditorTabsControl = MultiEditorTabsControl_1 = __decorate([__param(5, IContextMenuService), __param(6, IInstantiationService), __param(7, IContextKeyService), __param(8, IKeybindingService), __param(9, INotificationService), __param(10, IQuickInputService), __param(11, IThemeService), __param(12, IEditorService), __param(13, IPathService), __param(14, ITreeViewsDnDService), __param(15, IEditorResolverService), __param(16, IHostService)], MultiEditorTabsControl);
