...
const {
  TextDiffEditorModel
} = require('../../../common/editor/textDiffEditorModel.js');
const {
  ITelemetryService
  ...
  async handleSetInputError(error, input, options) {
    ...
    if (error.fileOperationResult === 7) {
      ...
      if (error instanceof TooLargeFileOperationError) {
        message = localizeWithPath(_moduleId, 1, "At least one file is not displayed in the text compare editor because it is very large ({0}).", ByteSize.formatSize(error.size));
      } else {
        message = localizeWithPath(_moduleId, 2, "At least one file is not displayed in the text compare editor because it is very large.");
      }
