...
const {} = require('./media/editorstatus.css.js');
const {
  localizeWithPath,
  localize2WithPath
} = require('../../../../nls.js');
...
} = require('../../../../editor/contrib/linesOperations/browser/linesOperations.js');
const {
  IndentUsingSpaces,
  IndentUsingTabs,
...
} = require('../../../../editor/contrib/indentation/browser/indentation.js');
const {
  BaseBinaryResourceEditor
} = require('./binaryEditor.js');
const {
  BinaryResourceDiffEditor
} = require('./binaryDiffEditor.js');
const {
  IEditorService
...
const {
  IStatusbarService
} = require('../../../services/statusbar/browser/statusbar.service.js');
const {
  IMarkerData,
  MarkerSeverity
...
} = require('../../../services/languageDetection/common/languageDetectionWorkerService.js');
const {
  ILanguageDetectionService
} = require('../../../services/languageDetection/common/languageDetectionWorkerService.service.js');
const {
  ContextKeyExpr
...
const _moduleId = "vs/workbench/browser/parts/editor/editorStatus";
...
const nlsSingleSelectionRange = localizeWithPath(_moduleId, 0, "Ln {0}, Col {1} ({2} selected)");
const nlsSingleSelection = localizeWithPath(_moduleId, 1, "Ln {0}, Col {1}");
const nlsMultiSelectionRange = localizeWithPath(_moduleId, 2, "{0} selections ({1} characters selected)");
const nlsMultiSelection = localizeWithPath(_moduleId, 3, "{0} selections");
const nlsEOLLF = localizeWithPath(_moduleId, 4, "LF");
const nlsEOLCRLF = localizeWithPath(_moduleId, 5, "CRLF");
let EditorStatus = class EditorStatus extends Disposable {
  ...
  async showIndentationPicker() {
    ...
    if (!activeTextEditorControl) {
      return this.quickInputService.pick([{
        label: localizeWithPath(_moduleId, 6, "No text editor active at this time")
      }]);
    }
    if (this.editorService.activeEditor?.isReadonly()) {
      return this.quickInputService.pick([{
        label: localizeWithPath(_moduleId, 7, "The active code editor is read-only.")
      }]);
    }
    const picks = [assertIsDefined(activeTextEditorControl.getAction(IndentUsingSpaces.ID)), assertIsDefined(activeTextEditorControl.getAction(IndentUsingTabs.ID)), assertIsDefined(activeTextEditorControl.getAction(ChangeTabDisplaySize.ID)), assertIsDefined(activeTextEditorControl.getAction(DetectIndentation.ID)), assertIsDefined(activeTextEditorControl.getAction(IndentationToSpacesAction.ID)), assertIsDefined(activeTextEditorControl.getAction(IndentationToTabsAction.ID)), assertIsDefined(activeTextEditorControl.getAction(TrimTrailingWhitespaceAction.ID))].map(a => {
      return {
        id: a.id,
        label: a.label,
    ...
    picks.splice(3, 0, {
      type: 'separator',
      label: localizeWithPath(_moduleId, 8, "convert file")
    });
    picks.unshift({
      type: 'separator',
      label: localizeWithPath(_moduleId, 9, "change view")
    });
    const action = await this.quickInputService.pick(picks, {
      placeHolder: localizeWithPath(_moduleId, 10, "Select Action"),
      matchOnDetail: true
    });
    return action?.run();
  }
  updateTabFocusModeElement(visible) {
    if (visible) {
      if (!this.tabFocusModeElement.value) {
        const text = localizeWithPath(_moduleId, 11, "Tab Moves Focus");
        this.tabFocusModeElement.value = this.statusbarService.addEntry({
          name: localizeWithPath(_moduleId, 12, "Accessibility Mode"),
          text,
          ariaLabel: text,
          tooltip: localizeWithPath(_moduleId, 13, "Disable Accessibility Mode"),
  ...
  updateColumnSelectionModeElement(visible) {
    if (visible) {
      if (!this.columnSelectionModeElement.value) {
        const text = localizeWithPath(_moduleId, 14, "Column Selection");
        this.columnSelectionModeElement.value = this.statusbarService.addEntry({
          name: localizeWithPath(_moduleId, 15, "Column Selection Mode"),
          text,
          ariaLabel: text,
          tooltip: localizeWithPath(_moduleId, 16, "Disable Column Selection Mode"),
  ...
  updateSelectionElement(text) {
    ...
    const props = {
      name: localizeWithPath(_moduleId, 17, "Editor Selection"),
      text,
      ariaLabel: text,
      tooltip: localizeWithPath(_moduleId, 18, "Go to Line/Column"),
  ...
  updateIndentationElement(text) {
    ...
    const props = {
      name: localizeWithPath(_moduleId, 19, "Editor Indentation"),
      text,
      ariaLabel: text,
      tooltip: localizeWithPath(_moduleId, 20, "Select Indentation"),
  ...
  updateEncodingElement(text) {
    ...
    const props = {
      name: localizeWithPath(_moduleId, 21, "Editor Encoding"),
      text,
      ariaLabel: text,
      tooltip: localizeWithPath(_moduleId, 22, "Select Encoding"),
  ...
  updateEOLElement(text) {
    ...
    const props = {
      name: localizeWithPath(_moduleId, 23, "Editor End of Line"),
      text,
      ariaLabel: text,
      tooltip: localizeWithPath(_moduleId, 24, "Select End of Line Sequence"),
  ...
  updateLanguageIdElement(text) {
    ...
    const props = {
      name: localizeWithPath(_moduleId, 25, "Editor Language"),
      text,
      ariaLabel: text,
      tooltip: localizeWithPath(_moduleId, 26, "Select Language Mode"),
  ...
  updateMetadataElement(text) {
    ...
    const props = {
      name: localizeWithPath(_moduleId, 27, "File Information"),
      text,
      ariaLabel: text,
      tooltip: localizeWithPath(_moduleId, 28, "File Information")
    };
  ...
  onIndentationChange(editorWidget) {
    ...
    if (editorWidget) {
      ...
      if (model) {
        ...
        update.indentation = modelOpts.insertSpaces ? modelOpts.tabSize === modelOpts.indentSize ? localizeWithPath(_moduleId, 29, "Spaces: {0}", modelOpts.indentSize) : localizeWithPath(_moduleId, 30, "Spaces: {0} (Tab Size: {1})", modelOpts.indentSize, modelOpts.tabSize) : localizeWithPath(_moduleId, 31, "Tab Size: {0}", modelOpts.tabSize);
      }
    }
...
let EditorStatusContribution = class EditorStatusContribution extends Disposable {
  ...
  constructor(instantiationService, editorGroupService, editorService) {
    ...
    this._register(editorGroupService.onDidCreateAuxiliaryEditorPart(({
      part,
...
EditorStatusContribution = __decorate([__param(0, IInstantiationService), __param(1, IEditorGroupsService), __param(2, IEditorService)], EditorStatusContribution);
      ...
      if (this.currentMarker) {
        ...
        if (!this.statusBarEntryAccessor.value) {
          this.statusBarEntryAccessor.value = this.statusbarService.addEntry({
            name: localizeWithPath(_moduleId, 32, "Current Problem"),
            text: '',
            ariaLabel: ''
          }, 'statusbar.currentProblem', 0);
        }
...
let ShowLanguageExtensionsAction = class ShowLanguageExtensionsAction extends Action {
  static {
    ShowLanguageExtensionsAction_1 = this;
  }
  static {
    this.ID = 'workbench.action.showLanguageExtensions';
  }
  constructor(fileExtension, commandService, galleryService) {
    super(ShowLanguageExtensionsAction_1.ID, localizeWithPath(_moduleId, 33, "Search Marketplace Extensions for '{0}'...", fileExtension));
...
ShowLanguageExtensionsAction = ShowLanguageExtensionsAction_1 = __decorate([__param(1, ICommandService), __param(2, IExtensionGalleryService)], ShowLanguageExtensionsAction);
class ChangeLanguageAction extends Action2 {
  ...
  constructor() {
    super({
      ...
      title: localize2WithPath(_moduleId, 34, 'Change Language Mode'),
      f1: true,
      keybinding: {
        weight: 200,
  ...
  async run(accessor) {
    ...
    const picks = languages.map(({
      ...
      if (currentLanguageName === languageName) {
        description = localizeWithPath(_moduleId, 35, "({0}) - Configured Language", languageId);
      } else {
        description = localizeWithPath(_moduleId, 36, "({0})", languageId);
      }
      return {
        label: languageName,
        meta: extensions,
    ...
    picks.unshift({
      type: 'separator',
      label: localizeWithPath(_moduleId, 37, "languages (identifier)")
    });
    ...
    if (hasLanguageSupport && resource) {
      ...
      configureLanguageSettings = {
        label: localizeWithPath(_moduleId, 38, "Configure '{0}' language based settings...", currentLanguageName)
      };
      picks.unshift(configureLanguageSettings);
      configureLanguageAssociations = {
        label: localizeWithPath(_moduleId, 39, "Configure File Association for '{0}'...", ext)
      };
      picks.unshift(configureLanguageAssociations);
    }
    const autoDetectLanguage = {
      label: localizeWithPath(_moduleId, 40, "Auto Detect")
    };
    picks.unshift(autoDetectLanguage);
    const pick = await quickInputService.pick(picks, {
      placeHolder: localizeWithPath(_moduleId, 41, "Select Language Mode"),
  ...
  configureFileAssociation(resource, languageService, quickInputService, configurationService) {
    ...
    const picks = languages.map(({
      ...
      return {
        ...
        label: languageName,
        iconClasses: getIconClassesForLanguageId(languageId),
        description: languageId === currentAssociation ? localizeWithPath(_moduleId, 42, "Current Association") : undefined
      };
    });
    setTimeout(async () => {
      const language = await quickInputService.pick(picks, {
        placeHolder: localizeWithPath(_moduleId, 43, "Select Language Mode to Associate with '{0}'", extension || base)
      });
      if (language) {
...
class ChangeEOLAction extends Action2 {
  constructor() {
    super({
      ...
      title: localize2WithPath(_moduleId, 44, 'Change End of Line Sequence'),
      f1: true
    });
  }
  async run(accessor) {
    ...
    const eol = await quickInputService.pick(EOLOptions, {
      placeHolder: localizeWithPath(_moduleId, 45, "Select End of Line Sequence"),
...
class ChangeEncodingAction extends Action2 {
  constructor() {
    super({
      ...
      title: localize2WithPath(_moduleId, 46, 'Change File Encoding'),
      f1: true
    });
  }
  async run(accessor) {
    ...
    if (!encodingSupport) {
      await quickInputService.pick([{
        label: localizeWithPath(_moduleId, 47, "No file active at this time")
      }]);
      return;
    }
    const saveWithEncodingPick = {
      label: localizeWithPath(_moduleId, 48, "Save with Encoding")
    };
    const reopenWithEncodingPick = {
      label: localizeWithPath(_moduleId, 49, "Reopen with Encoding")
    };
    ...
    if (guessedEncoding && configuredEncoding !== guessedEncoding && SUPPORTED_ENCODINGS[guessedEncoding]) {
      ...
      picks.unshift({
        ...
        description: localizeWithPath(_moduleId, 50, "Guessed from content")
      });
    }
    const encoding = await quickInputService.pick(picks, {
      placeHolder: isReopenWithEncoding ? localizeWithPath(_moduleId, 51, "Select File Encoding to Reopen File") : localizeWithPath(_moduleId, 52, "Select File Encoding to Save with"),
