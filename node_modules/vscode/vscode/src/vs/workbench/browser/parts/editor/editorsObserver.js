...
let EditorsObserver = class EditorsObserver extends Disposable {
  ...
  async ensureOpenedEditorsLimit(exclude, groupId) {
    ...
    if (this.editorGroupService.partOptions.limit?.perEditorGroup) {
      if (typeof groupId === 'number') {
        ...
        if (group) {
          await this.doEnsureOpenedEditorsLimit(limit, group.getEditors(0).map(editor => ({
            editor,
            groupId
          })), exclude);
        }
  ...
  async doEnsureOpenedEditorsLimit(limit, mostRecentEditors, exclude) {
    ...
    const leastRecentlyClosableEditors = mostRecentEditorsCountingForLimit.reverse().filter(({
      editor,
      groupId
    }) => {
      if (editor.isDirty() && !editor.isSaving() || editor.hasCapability(512)) {
    ...
    for (const {
      groupId,
      editor
    } of leastRecentlyClosableEditors) {
...
EditorsObserver = EditorsObserver_1 = __decorate([__param(1, IEditorGroupsService), __param(2, IStorageService)], EditorsObserver);
