...
const {} = require('./media/editorgroupview.css.js');
const {
  isSerializedEditorGroupModel,
  EditorGroupModel,
  isGroupEditorCloseEvent,
  isGroupEditorOpenEvent
} = require('../../../common/editor/editorGroupModel.js');
const {
...
const {
  getMimeTypes
} = require('../../../../editor/common/services/languagesAssociations.js');
const {
  extname,
  isEqual
...
} = require('./editorGroupWatermark.js');
const {
  EditorTitleControl
} = require('./editorTitleControl.js');
const {
  EditorPane
...
const _moduleId = "vs/workbench/browser/parts/editor/editorGroupView";
let EditorGroupView = EditorGroupView_1 = class EditorGroupView extends Themable {
  ...
  createContainerToolbar() {
    ...
    const containerToolbar = this._register(new ActionBar(toolbarContainer, {
      ariaLabel: localizeWithPath(_moduleId, 0, "Empty editor group actions"),
    ...
    const updateContainerToolbar = () => {
      ...
      createAndFillInActionBarActions(containerToolbarMenu, {
        arg: {
          groupId: this.id
        },
  ...
  updateContainer() {
    if (this.isEmpty) {
      ...
      this.element.setAttribute('aria-label', localizeWithPath(_moduleId, 1, "{0} (empty)", this.ariaLabel));
    } else {
  ...
  restoreEditors(from) {
    ...
    return this.doShowEditor(activeEditor, {
      active: true,
      isNew: false
  ...
  get label() {
    if (this.groupsLabel) {
      return localizeWithPath(_moduleId, 2, "{0}: Group {1}", this.groupsLabel, this._index + 1);
    }
    return localizeWithPath(_moduleId, 3, "Group {0}", this._index + 1);
  }
  get ariaLabel() {
    if (this.groupsLabel) {
      return localizeWithPath(_moduleId, 4, "{0}: Editor Group {1}", this.groupsLabel, this._index + 1);
    }
    return localizeWithPath(_moduleId, 5, "Editor Group {0}", this._index + 1);
  }
  get disposed() {
  ...
  async doOpenEditor(editor, options, internalOptions) {
    ...
    const pinned = options?.sticky || !this.groupsView.partOptions.enablePreview && !options?.transient || editor.isDirty() || (options?.pinned ?? typeof options?.index === 'number') || typeof options?.index === 'number' && this.model.isSticky(options.index) || editor.hasCapability(512);
    ...
    if (isNew && this.count === 1 && this.editorPartsView.groups.length > 1) {
  ...
  doMoveOrCopyEditorAcrossGroups(editor, target, openOptions, internalOptions) {
    ...
    if (!keepCopy || editor.hasCapability(8)) {
      ...
      if (typeof canMoveVeto === 'string') {
        this.dialogService.error(canMoveVeto, localizeWithPath(_moduleId, 6, "Try saving or reverting the editor first and then try again."));
        return false;
      }
    }
  ...
  createEditorActions(disposables) {
    ...
    if (activeEditorPane instanceof EditorPane) {
      ...
      createAndFillInActionBarActions(editorTitleMenu, {
...
EditorGroupView = EditorGroupView_1 = __decorate([__param(5, IInstantiationService), __param(6, IContextKeyService), __param(7, IThemeService), __param(8, ITelemetryService), __param(9, IKeybindingService), __param(10, IMenuService), __param(11, IContextMenuService), __param(12, IFileDialogService), __param(13, IEditorService), __param(14, IFilesConfigurationService), __param(15, IUriIdentityService), __param(16, ILogService), __param(17, IEditorResolverService), __param(18, IHostService), __param(19, IDialogService)], EditorGroupView);
