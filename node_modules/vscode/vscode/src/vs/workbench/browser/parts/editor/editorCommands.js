...
const {
  SideBySideEditor
} = require('./sideBySideEditor.js');
const {
  TextDiffEditor
} = require('./textDiffEditor.js');
const {
...
const {
  columnToEditorGroup
} = require('../../../services/editor/common/editorGroupColumn.js');
const {
...
} = require('../../../services/untitled/common/untitledTextEditorService.service.js');
const {
...
const EDITOR_CORE_NAVIGATION_COMMANDS = [SPLIT_EDITOR, CLOSE_EDITOR_COMMAND_ID, UNPIN_EDITOR_COMMAND_ID, UNLOCK_GROUP_COMMAND_ID, TOGGLE_MAXIMIZE_EDITOR_GROUP];
...
function registerActiveEditorMoveCopyCommand() {
  ...
  KeybindingsRegistry.registerCommandAndKeybindingRule({
    ...
    metadata: {
      ...
      args: [{
        ...
        description: localizeWithPath(_moduleId, 2, "Argument Properties:\n\t* 'to': String value providing where to move.\n\t* 'by': String value providing the unit for move (by tab or by group).\n\t* 'value': Number value providing how many positions or an absolute position to move."),
  ...
  function moveActiveTab(args, control) {
    ...
    group.moveEditor(control.input, group, {
      index
    });
  }
...
function registerOpenEditorAtIndexCommands() {
  ...
  for (let i = 0; i < 9; i++) {
    ...
    KeybindingsRegistry.registerCommandAndKeybindingRule({
      ...
      mac: {
        primary: 256 | toKeyCode(visibleIndex)
      },
  ...
  for (let groupIndex = 1; groupIndex < 8; groupIndex++) {
    KeybindingsRegistry.registerCommandAndKeybindingRule({
      ...
      handler: accessor => {
        ...
        const lastGroup = editorGroupService.findGroup({
          location: 1
        });
        if (!lastGroup) {
          return;
...
function registerCloseEditorCommands() {
  function closeEditorHandler(accessor, forceCloseStickyEditors, resourceOrContext, context) {
    ...
    if (keepStickyEditors) {
      ...
      if (activeEditor && activeGroup.isSticky(activeEditor)) {
        const nextNonStickyEditorInGroup = activeGroup.getEditors(0, {
          excludeSticky: true
        })[0];
        if (nextNonStickyEditorInGroup) {
          return activeGroup.openEditor(nextNonStickyEditorInGroup);
        }
        const nextNonStickyEditorInAllGroups = editorService.getEditors(0, {
          excludeSticky: true
        })[0];
    ...
    const {
      editors,
      groups
    } = getEditorsContext(accessor, resourceOrContext, context);
    return Promise.all(groups.map(async group => {
      if (group) {
        const editorsToClose = coalesce(editors.filter(editor => editor.groupId === group.id).map(editor => typeof editor.editorIndex === 'number' ? group.getEditorByIndex(editor.editorIndex) : group.activeEditor)).filter(editor => !keepStickyEditors || !group.isSticky(editor));
  ...
  KeybindingsRegistry.registerCommandAndKeybindingRule({
    ...
    handler: (accessor, resourceOrContext, context) => {
      ...
      return Promise.all(groups.map(async group => {
        if (group) {
          const editorsToKeep = editors.filter(editor => editor.groupId === group.id).map(editor => typeof editor.editorIndex === 'number' ? group.getEditorByIndex(editor.editorIndex) : group.activeEditor);
          const editorsToClose = group.getEditors(1, {
            excludeSticky: true
          }).filter(editor => !editorsToKeep.includes(editor));
...
function registerFocusEditorGroupWihoutWrapCommands() {
  ...
  for (const command of commands) {
    CommandsRegistry.registerCommand(command.id, async accessor => {
      ...
      }, editorGroupService.activeGroup, false);
      group?.focus();
    });
  }
}
...
exports.API_OPEN_DIFF_EDITOR_COMMAND_ID = API_OPEN_DIFF_EDITOR_COMMAND_ID;
exports.API_OPEN_EDITOR_COMMAND_ID = API_OPEN_EDITOR_COMMAND_ID;
exports.API_OPEN_WITH_EDITOR_COMMAND_ID = API_OPEN_WITH_EDITOR_COMMAND_ID;
exports.CLOSE_EDITORS_AND_GROUP_COMMAND_ID = CLOSE_EDITORS_AND_GROUP_COMMAND_ID;
exports.CLOSE_EDITORS_IN_GROUP_COMMAND_ID = CLOSE_EDITORS_IN_GROUP_COMMAND_ID;
exports.CLOSE_EDITORS_TO_THE_RIGHT_COMMAND_ID = CLOSE_EDITORS_TO_THE_RIGHT_COMMAND_ID;
exports.CLOSE_EDITOR_COMMAND_ID = CLOSE_EDITOR_COMMAND_ID;
exports.CLOSE_EDITOR_GROUP_COMMAND_ID = CLOSE_EDITOR_GROUP_COMMAND_ID;
exports.CLOSE_OTHER_EDITORS_IN_GROUP_COMMAND_ID = CLOSE_OTHER_EDITORS_IN_GROUP_COMMAND_ID;
exports.CLOSE_PINNED_EDITOR_COMMAND_ID = CLOSE_PINNED_EDITOR_COMMAND_ID;
exports.CLOSE_SAVED_EDITORS_COMMAND_ID = CLOSE_SAVED_EDITORS_COMMAND_ID;
exports.COPY_ACTIVE_EDITOR_COMMAND_ID = COPY_ACTIVE_EDITOR_COMMAND_ID;
exports.COPY_EDITOR_GROUP_INTO_NEW_WINDOW_COMMAND_ID = COPY_EDITOR_GROUP_INTO_NEW_WINDOW_COMMAND_ID;
exports.COPY_EDITOR_INTO_NEW_WINDOW_COMMAND_ID = COPY_EDITOR_INTO_NEW_WINDOW_COMMAND_ID;
exports.EDITOR_CORE_NAVIGATION_COMMANDS = EDITOR_CORE_NAVIGATION_COMMANDS;
exports.FOCUS_ABOVE_GROUP_WITHOUT_WRAP_COMMAND_ID = FOCUS_ABOVE_GROUP_WITHOUT_WRAP_COMMAND_ID;
exports.FOCUS_BELOW_GROUP_WITHOUT_WRAP_COMMAND_ID = FOCUS_BELOW_GROUP_WITHOUT_WRAP_COMMAND_ID;
exports.FOCUS_FIRST_SIDE_EDITOR = FOCUS_FIRST_SIDE_EDITOR;
exports.FOCUS_LEFT_GROUP_WITHOUT_WRAP_COMMAND_ID = FOCUS_LEFT_GROUP_WITHOUT_WRAP_COMMAND_ID;
exports.FOCUS_OTHER_SIDE_EDITOR = FOCUS_OTHER_SIDE_EDITOR;
exports.FOCUS_RIGHT_GROUP_WITHOUT_WRAP_COMMAND_ID = FOCUS_RIGHT_GROUP_WITHOUT_WRAP_COMMAND_ID;
exports.FOCUS_SECOND_SIDE_EDITOR = FOCUS_SECOND_SIDE_EDITOR;
exports.JOIN_EDITOR_IN_GROUP = JOIN_EDITOR_IN_GROUP;
exports.KEEP_EDITOR_COMMAND_ID = KEEP_EDITOR_COMMAND_ID;
exports.LAYOUT_EDITOR_GROUPS_COMMAND_ID = LAYOUT_EDITOR_GROUPS_COMMAND_ID;
exports.LOCK_GROUP_COMMAND_ID = LOCK_GROUP_COMMAND_ID;
exports.MOVE_ACTIVE_EDITOR_COMMAND_ID = MOVE_ACTIVE_EDITOR_COMMAND_ID;
exports.MOVE_EDITOR_GROUP_INTO_NEW_WINDOW_COMMAND_ID = MOVE_EDITOR_GROUP_INTO_NEW_WINDOW_COMMAND_ID;
exports.MOVE_EDITOR_INTO_NEW_WINDOW_COMMAND_ID = MOVE_EDITOR_INTO_NEW_WINDOW_COMMAND_ID;
exports.NEW_EMPTY_EDITOR_WINDOW_COMMAND_ID = NEW_EMPTY_EDITOR_WINDOW_COMMAND_ID;
exports.OPEN_EDITOR_AT_INDEX_COMMAND_ID = OPEN_EDITOR_AT_INDEX_COMMAND_ID;
exports.PIN_EDITOR_COMMAND_ID = PIN_EDITOR_COMMAND_ID;
exports.REOPEN_WITH_COMMAND_ID = REOPEN_WITH_COMMAND_ID;
exports.SHOW_EDITORS_IN_GROUP = SHOW_EDITORS_IN_GROUP;
exports.SPLIT_EDITOR = SPLIT_EDITOR;
exports.SPLIT_EDITOR_DOWN = SPLIT_EDITOR_DOWN;
exports.SPLIT_EDITOR_IN_GROUP = SPLIT_EDITOR_IN_GROUP;
exports.SPLIT_EDITOR_LEFT = SPLIT_EDITOR_LEFT;
exports.SPLIT_EDITOR_RIGHT = SPLIT_EDITOR_RIGHT;
exports.SPLIT_EDITOR_UP = SPLIT_EDITOR_UP;
exports.TOGGLE_KEEP_EDITORS_COMMAND_ID = TOGGLE_KEEP_EDITORS_COMMAND_ID;
exports.TOGGLE_LOCK_GROUP_COMMAND_ID = TOGGLE_LOCK_GROUP_COMMAND_ID;
exports.TOGGLE_MAXIMIZE_EDITOR_GROUP = TOGGLE_MAXIMIZE_EDITOR_GROUP;
exports.TOGGLE_SPLIT_EDITOR_IN_GROUP = TOGGLE_SPLIT_EDITOR_IN_GROUP;
exports.TOGGLE_SPLIT_EDITOR_IN_GROUP_LAYOUT = TOGGLE_SPLIT_EDITOR_IN_GROUP_LAYOUT;
exports.UNLOCK_GROUP_COMMAND_ID = UNLOCK_GROUP_COMMAND_ID;
exports.UNPIN_EDITOR_COMMAND_ID = UNPIN_EDITOR_COMMAND_ID;
