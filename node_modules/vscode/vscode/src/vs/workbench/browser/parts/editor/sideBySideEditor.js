...
const {} = require('./media/sidebysideeditor.css.js');
const {
  localizeWithPath
} = require('../../../../nls.js');
const {
...
const {
  SplitView,
  Sizing
} = require('../../../../base/browser/ui/splitview/splitview.js');
const {
  Emitter,
  Relay,
  Event
...
} = require('./editorWithViewState.js');
const {
...
const _moduleId = "vs/workbench/browser/parts/editor/sideBySideEditor";
...
let SideBySideEditor = class SideBySideEditor extends AbstractEditorWithViewState {
  ...
  getTitle() {
    ...
    return localizeWithPath(_moduleId, 0, "Side by Side Editor");
  }
  async setInput(input, options, context, token) {
    ...
    await Promise.all([this.secondaryEditorPane?.setInput(input.secondary, secondary, context, token), this.primaryEditorPane?.setInput(input.primary, primary, context, token)]);
  ...
  getSelection() {
    ...
    if (isEditorPaneWithSelection(lastFocusedEditorPane)) {
      ...
      if (selection) {
        return new SideBySideAwareEditorPaneSelection(selection, lastFocusedEditorPane === this.primaryEditorPane ? SideBySideEditor$1.PRIMARY : SideBySideEditor$1.SECONDARY);
      }
    }
    return undefined;
  }
