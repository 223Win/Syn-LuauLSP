...
const {
  FOLDER_SCOPES,
  WORKSPACE_SCOPES
} = require('../../configuration/common/configuration.js');
const {
  createValidator
} = require('./preferencesValidation.js');
const _moduleId = "vs/workbench/services/preferences/common/preferencesModels";
const nullRange = {
  startLineNumber: -1,
  startColumn: -1,
  endLineNumber: -1,
...
class DefaultSettings extends Disposable {
  ...
  getMostCommonlyUsedSettings(allSettingsGroups) {
    ...
    return {
      id: 'mostCommonlyUsed',
      range: nullRange,
      title: localizeWithPath(_moduleId, 0, "Commonly Used"),
      titleRange: nullRange,
      sections: [{
        settings
      }]
...
class DefaultSettingsEditorModel extends AbstractSettingsModel {
  ...
  writeSettingsGroupToBuilder(builder, settingsGroup, filterMatches) {
    ...
    const fixedMatches = flatten(filterMatches.map(m => m.matches || []).map((settingMatches, i) => {
...
function defaultKeybindingsContents(keybindingService) {
  const defaultsHeader = '// ' + localizeWithPath(_moduleId, 1, "Override key bindings by placing them into your key bindings file.");
