  ...
  return value => {
    ...
    if (prop.type === 'string') {
      if (prop.enum && !isStringArray(prop.enum)) {
        errors.push(localizeWithPath(_moduleId, 2, 'The enum options should be strings, but there is a non-string option. Please file an issue with the extension author.'));
...
function getInvalidTypeError(value, type) {
  ...
  if (!typeArr.some(_type => valueValidatesAsType(value, _type))) {
    return localizeWithPath(_moduleId, 4, "Setting has an invalid type, expected {0}. Fix in JSON.", JSON.stringify(type));
  }
  return;
}
...
function toRegExp(pattern) {
  try {
    ...
    try {
      ...
      console.error(localizeWithPath(_moduleId, 5, "Error parsing the following regex both with and without the u flag:"), pattern);
      return /.*/;
    }
  }
}
function getStringValidators(prop) {
  ...
  return [{
    ...
    message: localizeWithPath(_moduleId, 6, "Value must be {0} or fewer characters long.", prop.maxLength)
  }, {
    ...
    message: localizeWithPath(_moduleId, 9, "Invalid color format. Use #RGB, #RGBA, #RRGGBB or #RRGGBBAA.")
  }, {
    ...
    message: localizeWithPath(_moduleId, 13, "Value is not accepted. Valid values: {0}.", prop.enum ? prop.enum.map(key => `"${key}"`).join(', ') : '[]')
          ...
          if (typeof propItems.pattern === 'string') {
            ...
            arrayValue.forEach(v => {
              if (!patternRegex.test(v)) {
                message += propItems.patternErrorMessage || localizeWithPath(_moduleId, 25, 'Value {0} must match regex {1}.', withQuotes(v), withQuotes(propItems.pattern));
              }
            });
          }
          ...
          if (propItemsEnum) {
            arrayValue.forEach(v => {
              if (propItemsEnum.indexOf(v) === -1) {
                message += localizeWithPath(_moduleId, 26, 'Value {0} is not one of {1}', withQuotes(v), '[' + propItemsEnum.map(withQuotes).join(', ') + ']');
                message += '\n';
              }
            });
          }
