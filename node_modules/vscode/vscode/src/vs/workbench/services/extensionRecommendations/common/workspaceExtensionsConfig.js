...
const {
  IJSONEditingService
} = require('../../configuration/common/jsonEditing.service.js');
const {
  ResourceMap
} = require('../../../../base/common/map.js');
const {} = require('../../../../platform/instantiation/common/instantiation.js');
const _moduleId = "vs/workbench/services/extensionRecommendations/common/workspaceExtensionsConfig";
...
let WorkspaceExtensionsConfigService = class WorkspaceExtensionsConfigService extends Disposable {
  ...
  async toggleRecommendation(extensionId) {
    ...
    const workspaceOrFolders = isRecommended ? await this.pickWorkspaceOrFolders(recommendedWorksapceFolders, isWorkspaceRecommended ? workspace : undefined, localizeWithPath(_moduleId, 0, "Remove extension recommendation from")) : await this.pickWorkspaceOrFolders(workspace.folders, workspace.configuration ? workspace : undefined, localizeWithPath(_moduleId, 1, "Add extension recommendation to"));
  ...
  async toggleUnwantedRecommendation(extensionId) {
    ...
    const workspaceOrFolders = isUnwanted ? await this.pickWorkspaceOrFolders(unWantedWorksapceFolders, isWorkspaceUnwanted ? workspace : undefined, localizeWithPath(_moduleId, 0, "Remove extension recommendation from")) : await this.pickWorkspaceOrFolders(workspace.folders, workspace.configuration ? workspace : undefined, localizeWithPath(_moduleId, 1, "Add extension recommendation to"));
    for (const workspaceOrWorkspaceFolder of workspaceOrFolders) {
      if (isWorkspace(workspaceOrWorkspaceFolder)) {
  ...
  async pickWorkspaceOrFolders(workspaceFolders, workspace, placeHolder) {
    ...
    const folderPicks = workspaceFolders.map(workspaceFolder => {
      return {
        ...
        description: localizeWithPath(_moduleId, 2, "Workspace Folder"),
    ...
    if (workspace) {
      ...
      folderPicks.push({
        label: localizeWithPath(_moduleId, 3, "Workspace"),
...
exports.EXTENSIONS_CONFIG = EXTENSIONS_CONFIG;
