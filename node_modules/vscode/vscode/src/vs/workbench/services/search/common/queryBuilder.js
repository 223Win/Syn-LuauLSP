...
} = require('../../../../editor/common/model/textModelSearch.js');
const {
  localizeWithPath
} = require('../../../../nls.js');
const {
...
let QueryBuilder = class QueryBuilder {
  ...
  commonQueryFromFileList(files) {
    ...
    files.forEach(file => {
      ...
      if (providerExists) {
        const searchRoot = this.workspaceContextService.getWorkspaceFolder(file)?.uri ?? file.with({
  ...
  expandOneSearchPath(searchPath) {
    ...
    if (this.workspaceContextService.getWorkbenchState() === 2) {
      ...
      if (folderMatches.length) {
        ...
        const searchPathNotFoundError = localizeWithPath(_moduleId, 0, "Workspace folder does not exist: {0}", probableWorkspaceFolderName);
...
QueryBuilder = __decorate([__param(0, IConfigurationService), __param(1, IWorkspaceContextService), __param(2, IEditorGroupsService), __param(3, ILogService), __param(4, IPathService)], QueryBuilder);
