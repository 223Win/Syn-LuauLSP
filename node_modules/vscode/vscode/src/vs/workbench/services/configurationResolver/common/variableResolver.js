...
const {
  cwd
} = require('../../../../base/common/process.js');
const {
  isString,
  isObject,
  isUndefinedOrNull
...
const {
  VariableKind,
  VariableError
} = require('./configurationResolver.js');
const {
  replaceAsync
...
class AbstractVariableResolverService {
  ...
  async evaluateSingleVariable(environment, match, variable, folderUri, commandValueMapping) {
    ...
    const getFilePath = variableKind => {
      ...
      throw new VariableError(variableKind, localizeWithPath(_moduleId, 0, "Variable {0} can not be resolved. Please open an editor.", match));
    };
    const getFolderPathForFile = variableKind => {
      ...
      throw new VariableError(variableKind, localizeWithPath(_moduleId, 1, "Variable {0}: can not find workspace folder of '{1}'.", match, basename(filePath)));
    };
    const getFolderUri = variableKind => {
      if (argument) {
        ...
        throw new VariableError(variableKind, localizeWithPath(_moduleId, 2, "Variable {0} can not be resolved. No such folder '{1}'.", match, argument));
      }
      if (folderUri) {
        return folderUri;
      }
      if (this._context.getWorkspaceFolderCount() > 1) {
        throw new VariableError(variableKind, localizeWithPath(_moduleId, 3, "Variable {0} can not be resolved in a multi folder workspace. Scope this variable using ':' and a workspace folder name.", match));
      }
      throw new VariableError(variableKind, localizeWithPath(_moduleId, 4, "Variable {0} can not be resolved. Please open a folder.", match));
    };
    switch (variable) {
      case 'env':
        if (argument) {
        ...
        throw new VariableError(VariableKind.Env, localizeWithPath(_moduleId, 5, "Variable {0} can not be resolved because no environment variable name is given.", match));
      case 'config':
        if (argument) {
          const config = this._context.getConfigurationValue(folderUri, argument);
          if (isUndefinedOrNull(config)) {
            throw new VariableError(VariableKind.Config, localizeWithPath(_moduleId, 6, "Variable {0} can not be resolved because setting '{1}' not found.", match, argument));
          }
          if (isObject(config)) {
            throw new VariableError(VariableKind.Config, localizeWithPath(_moduleId, 7, "Variable {0} can not be resolved because '{1}' is a structured value.", match, argument));
          }
          return config;
        }
        throw new VariableError(VariableKind.Config, localizeWithPath(_moduleId, 8, "Variable {0} can not be resolved because no settings name is given.", match));
      case 'command':
      ...
      case 'extensionInstallFolder':
        if (argument) {
          ...
          if (!ext) {
            throw new VariableError(VariableKind.ExtensionInstallFolder, localizeWithPath(_moduleId, 9, "Variable {0} can not be resolved because the extension {1} is not installed.", match, argument));
          }
          return this.fsPath(ext.extensionLocation);
        }
        throw new VariableError(VariableKind.ExtensionInstallFolder, localizeWithPath(_moduleId, 10, "Variable {0} can not be resolved because no extension name is given.", match));
      default:
        {
          switch (variable) {
            case 'workspaceRoot':
            ...
            case 'userHome':
              {
                ...
                throw new VariableError(VariableKind.UserHome, localizeWithPath(_moduleId, 11, "Variable {0} can not be resolved. UserHome path is not defined", match));
              }
            case 'lineNumber':
              {
                ...
                throw new VariableError(VariableKind.LineNumber, localizeWithPath(_moduleId, 12, "Variable {0} can not be resolved. Make sure to have a line selected in the active editor.", match));
              }
            case 'selectedText':
              {
                ...
                throw new VariableError(VariableKind.SelectedText, localizeWithPath(_moduleId, 13, "Variable {0} can not be resolved. Make sure to have some text selected in the active editor.", match));
              }
            case 'file':
  ...
  resolveFromMap(variableKind, match, argument, commandValueMapping, prefix) {
    if (argument && commandValueMapping) {
      ...
      throw new VariableError(variableKind, localizeWithPath(_moduleId, 14, "Variable {0} can not be resolved because the command has no value.", match));
    }
    return match;
  }
}
