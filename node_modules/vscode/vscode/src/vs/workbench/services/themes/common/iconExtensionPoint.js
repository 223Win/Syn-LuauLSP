...
const iconConfigurationExtPoint = ExtensionsRegistry.registerExtensionPoint({
  ...
  jsonSchema: {
    ...
    propertyNames: {
      ...
      patternErrorMessage: localizeWithPath(_moduleId, 2, 'Identifiers can only contain letters, digits and minuses and need to consist of at least two segments in the form `component-iconname`.')
    },
    additionalProperties: {
      ...
      properties: {
        ...
        default: {
          ...
          description: localizeWithPath(_moduleId, 6, 'The default of the icon. Either a reference to an extisting ThemeIcon or an icon in an icon font.')
        }
      },
    ...
    iconConfigurationExtPoint.setHandler((extensions, delta) => {
      for (const extension of delta.added) {
        ...
        if (!extensionValue || typeof extensionValue !== 'object') {
          collector.error(localizeWithPath(_moduleId, 7, "'configuration.icons' must be an object with the icon names as properties."));
          return;
        }
        for (const id in extensionValue) {
          if (!id.match(iconIdPattern)) {
            collector.error(localizeWithPath(_moduleId, 8, "'configuration.icons' keys represent the icon id and can only contain letter, digits and minuses. They need to consist of at least two segments in the form `component-iconname`."));
            return;
          }
          ...
          if (typeof iconContribution.description !== 'string' || iconContribution.description.length === 0) {
            collector.error(localizeWithPath(_moduleId, 9, "'configuration.icons.description' must be defined and can not be empty"));
            return;
          }
          ...
          if (typeof defaultIcon === 'string') {
            ...
            if (!format) {
              collector.warn(localizeWithPath(_moduleId, 10, "Expected `contributes.icons.default.fontPath` to have file extension 'woff', woff2' or 'ttf', is '{0}'.", fileExt));
              return;
            }
            ...
            if (!isEqualOrParent(iconFontLocation, extensionLocation)) {
              collector.warn(localizeWithPath(_moduleId, 11, "Expected `contributes.icons.default.fontPath` ({0}) to be included inside extension's folder ({0}).", iconFontLocation.path, extensionLocation.path));
              return;
            }
            ...
            collector.error(localizeWithPath(_moduleId, 12, "'configuration.icons.default' must be either a reference to the id of an other theme icon (string) or a icon definition (object) with properties `fontPath` and `fontCharacter`."));
          }
        }
      }
