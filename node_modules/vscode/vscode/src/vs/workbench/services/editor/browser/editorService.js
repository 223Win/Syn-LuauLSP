...
const {
  isEditorReplacement
} = require('../common/editorGroupsService.js');
const {
  IEditorGroupsService
} = require('../common/editorGroupsService.service.js');
const {
  isPreferredGroup
} = require('../common/editorService.js');
const {
...
const {
  EditorsObserver
} = require('../../../browser/parts/editor/editorsObserver.js');
const {
  timeout,
  Promises
...
} = require('../common/editorResolverService.service.js');
const {
  IWorkspaceTrustRequestService
} = require('../../../../platform/workspace/common/workspaceTrust.service.js');
const {
  IHostService
} = require('../../host/browser/host.service.js');
const {
  findGroup
} = require('../common/editorGroupFinder.js');
const {
  ITextEditorService
} = require('../../textfile/common/textEditorService.service.js');
var EditorService_1;
      ...
      case 1:
        {
          ...
          for (const group of this.editorGroupsContainer.getGroups(2)) {
            editors.push(...group.getEditors(1, options).map(editor => ({
              editor,
              groupId: group.id
            })));
          }
  ...
  getAllModifiedEditors(options) {
    ...
    for (const group of this.editorGroupsContainer.getGroups(1)) {
      for (const editor of group.getEditors(0)) {
        ...
        if ((typeof options?.includeUntitled === 'boolean' || !options?.includeUntitled?.includeScratchpad) && editor.hasCapability(512)) {
          continue;
        }
