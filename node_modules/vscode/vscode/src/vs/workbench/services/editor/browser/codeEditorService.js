...
} = require('../../../../editor/browser/services/abstractCodeEditorService.js');
const {
  IThemeService
...
const {
  IEditorService
} = require('../common/editorService.service.js');
...
let CodeEditorService = class CodeEditorService extends AbstractCodeEditorService {
  ...
  async doOpenCodeEditorFromDiff(input, source, sideBySide) {
    const activeTextEditorControl = this.editorService.activeTextEditorControl;
    if (!sideBySide && isDiffEditor(activeTextEditorControl) && input.options && input.resource && source === activeTextEditorControl.getModifiedEditor() && activeTextEditorControl.getModel() && isEqual(input.resource, activeTextEditorControl.getModel()?.modified.uri)) {
  ...
  async doOpenCodeEditor(input, source, sideBySide) {
    ...
    if (!enablePreviewFromCodeNavigation && source && !input.options?.pinned && !sideBySide && !isEqual(source.getModel()?.uri, input.resource)) {
...
CodeEditorService = __decorate([__param(0, IEditorService), __param(1, IThemeService), __param(2, IConfigurationService)], CodeEditorService);
