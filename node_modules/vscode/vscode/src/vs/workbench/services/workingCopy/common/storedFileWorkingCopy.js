...
const {
  IWorkingCopyService
} = require('./workingCopyService.service.js');
const {
  TaskSequentializer,
  raceCancellation,
  timeout
...
} = require('./workingCopyFileService.service.js');
const {
  IFilesConfigurationService
} = require('../../filesConfiguration/common/filesConfigurationService.service.js');
const {
  IWorkingCopyBackupService
} = require('./workingCopyBackup.service.js');
...
} = require('./workingCopyEditorService.service.js');
const {
  IEditorService
} = require('../../editor/common/editorService.service.js');
const {
  IElevatedFileService
} = require('../../files/common/elevatedFileService.service.js');
const {
  ResourceWorkingCopy
} = require('./resourceWorkingCopy.js');
const Severity$1 = require('../../../../base/common/severity.js');
var StoredFileWorkingCopy_1;
const _moduleId = "vs/workbench/services/workingCopy/common/storedFileWorkingCopy";
...
let StoredFileWorkingCopy = class StoredFileWorkingCopy extends ResourceWorkingCopy {
  ...
  doHandleSaveError(error, options) {
    ...
    if (fileOperationError.fileOperationResult === 3) {
      message = localizeWithPath(_moduleId, 0, "Failed to save '{0}': The content of the file is newer. Do you want to overwrite the file with your changes?", this.name);
      primaryActions.push(toAction({
        id: 'fileWorkingCopy.overwrite',
        label: localizeWithPath(_moduleId, 1, "Overwrite"),
        run: () => this.save({
          ...options,
      ...
      primaryActions.push(toAction({
        ...
        label: localizeWithPath(_moduleId, 2, "Discard"),
        run: () => this.revert()
      }));
    } else {
      ...
      if (canSaveElevated && (isPermissionDenied || triedToUnlock)) {
        primaryActions.push(toAction({
          ...
          label: triedToUnlock ? isWindows ? localizeWithPath(_moduleId, 3, "Overwrite as Admin...") : localizeWithPath(_moduleId, 4, "Overwrite as Sudo...") : isWindows ? localizeWithPath(_moduleId, 5, "Retry as Admin...") : localizeWithPath(_moduleId, 6, "Retry as Sudo..."),
          run: () => {
            this.save({
              ...options,
              writeElevated: true,
        ...
        primaryActions.push(toAction({
          ...
          label: localizeWithPath(_moduleId, 7, "Retry"),
          run: () => this.save({
            ...options,
            reason: 1
          })
      ...
      primaryActions.push(toAction({
        ...
        label: localizeWithPath(_moduleId, 8, "Save As..."),
        run: async () => {
      ...
      if (isWriteLocked) {
        if (triedToUnlock && canSaveElevated) {
          message = isWindows ? localizeWithPath(_moduleId, 9, "Failed to save '{0}': File is read-only. Select 'Overwrite as Admin' to retry as administrator.", this.name) : localizeWithPath(_moduleId, 10, "Failed to save '{0}': File is read-only. Select 'Overwrite as Sudo' to retry as superuser.", this.name);
        } else {
          message = localizeWithPath(_moduleId, 11, "Failed to save '{0}': File is read-only. Select 'Overwrite' to attempt to make it writeable.", this.name);
        }
      } else if (canSaveElevated && isPermissionDenied) {
        message = isWindows ? localizeWithPath(_moduleId, 12, "Failed to save '{0}': Insufficient permissions. Select 'Retry as Admin' to retry as administrator.", this.name) : localizeWithPath(_moduleId, 13, "Failed to save '{0}': Insufficient permissions. Select 'Retry as Sudo' to retry as superuser.", this.name);
      } else {
        message = localizeWithPath(_moduleId, 14, "Failed to save '{0}': {1}", this.name, toErrorMessage(error, false));
      }
    }
...
StoredFileWorkingCopy = StoredFileWorkingCopy_1 = __decorate([__param(5, IFileService), __param(6, ILogService), __param(7, IWorkingCopyFileService), __param(8, IFilesConfigurationService), __param(9, IWorkingCopyBackupService), __param(10, IWorkingCopyService), __param(11, INotificationService), __param(12, IWorkingCopyEditorService), __param(13, IEditorService), __param(14, IElevatedFileService)], StoredFileWorkingCopy);
