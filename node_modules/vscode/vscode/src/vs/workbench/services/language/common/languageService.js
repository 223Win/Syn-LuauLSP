...
const {
  LanguageService
} = require('../../../../editor/common/services/languageService.js');
const {
  ...
  jsonSchema: {
    ...
    items: {
      ...
      properties: {
        ...
        firstLine: {
          description: localizeWithPath(_moduleId, 7, 'A regular expression matching the first line of a file of the language.'),
          type: 'string'
        },
        configuration: {
          description: localizeWithPath(_moduleId, 8, 'A relative path to a file containing configuration options for the language.'),
          type: 'string',
          default: './language-configuration.json'
        },
        icon: {
          type: 'object',
          description: localizeWithPath(_moduleId, 9, 'A icon to use as file icon, if no icon theme provides one for the language.'),
          properties: {
            light: {
...
class LanguageTableRenderer extends Disposable {
  ...
  render(manifest) {
    ...
    const headers = [localizeWithPath(_moduleId, 12, "ID"), localizeWithPath(_moduleId, 13, "Name"), localizeWithPath(_moduleId, 14, "File Extensions"), localizeWithPath(_moduleId, 15, "Grammar"), localizeWithPath(_moduleId, 16, "Snippets")];
    ...
    languagesExtPoint.setHandler(extensions => {
      ...
      for (let i = 0, len = extensions.length; i < len; i++) {
        ...
        if (!Array.isArray(extension.value)) {
          extension.collector.error(localizeWithPath(_moduleId, 18, "Invalid `contributes.{0}`. Expected an array.", languagesExtPoint.name));
          continue;
        }
...
function isValidLanguageExtensionPoint(value, collector) {
  ...
  if (typeof value.id !== 'string') {
    collector?.error(localizeWithPath(_moduleId, 20, "property `{0}` is mandatory and must be of type `string`", 'id'));
    return false;
  }
  if (!isUndefinedOrStringArray(value.extensions)) {
    collector?.error(localizeWithPath(_moduleId, 21, "property `{0}` can be omitted and must be of type `string[]`", 'extensions'));
    return false;
  }
  if (!isUndefinedOrStringArray(value.filenames)) {
    collector?.error(localizeWithPath(_moduleId, 22, "property `{0}` can be omitted and must be of type `string[]`", 'filenames'));
    return false;
  }
  if (typeof value.firstLine !== 'undefined' && typeof value.firstLine !== 'string') {
    collector?.error(localizeWithPath(_moduleId, 23, "property `{0}` can be omitted and must be of type `string`", 'firstLine'));
    return false;
  }
  if (typeof value.configuration !== 'undefined' && typeof value.configuration !== 'string') {
    collector?.error(localizeWithPath(_moduleId, 24, "property `{0}` can be omitted and must be of type `string`", 'configuration'));
    return false;
  }
  if (!isUndefinedOrStringArray(value.aliases)) {
    collector?.error(localizeWithPath(_moduleId, 25, "property `{0}` can be omitted and must be of type `string[]`", 'aliases'));
    return false;
  }
  if (!isUndefinedOrStringArray(value.mimetypes)) {
    collector?.error(localizeWithPath(_moduleId, 26, "property `{0}` can be omitted and must be of type `string[]`", 'mimetypes'));
    return false;
  }
  if (typeof value.icon !== 'undefined') {
    if (typeof value.icon !== 'object' || typeof value.icon.light !== 'string' || typeof value.icon.dark !== 'string') {
      collector?.error(localizeWithPath(_moduleId, 27, "property `{0}` can be omitted and must be of type `object` with properties `{1}` and `{2}` of type `string`", 'icon', 'light', 'dark'));
      return false;
    }
  }
  return true;
...
exports.languagesExtPoint = languagesExtPoint;