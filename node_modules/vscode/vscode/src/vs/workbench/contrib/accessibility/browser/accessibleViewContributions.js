...
} = require('../../../browser/parts/notifications/notificationsCommands.js');
const {
  WorkbenchList
...
const {} = require('../../../../editor/contrib/inlineCompletions/browser/inlineCompletionContextKeys.js');
const {} = require('../../../../platform/contextkey/common/contextkey.js');
const {
  IContextKeyService
...
const {
  COMMENTS_VIEW_ID,
  CommentsMenus
} = require('../../comments/browser/commentsTreeViewer.js');
const {
  IViewsService
} = require('../../../services/views/common/viewsService.service.js');
const {
  CONTEXT_KEY_HAS_COMMENTS
} = require('../../comments/browser/commentsView.js');
const {
  IMenuService
...
function getActionsFromNotification(notification, accessibilitySignalService) {
  ...
  if (actions) {
    actions.push({
      ...
      label: localizeWithPath(_moduleId, 2, "Clear Notification"),
      tooltip: localizeWithPath(_moduleId, 2, "Clear Notification"),
      run: () => {
        notification.close();
...
ExtensionAccessibilityHelpDialogContribution = __decorate([__param(0, IKeybindingService)], ExtensionAccessibilityHelpDialogContribution);
function registerAccessibilityHelpAction(keybindingService, viewDescriptor) {
  ...
  disposableStore.add(AccessibilityHelpAction.addImplementation(95, viewDescriptor.id, accessor => {
    const accessibleViewService = accessor.get(IAccessibleViewService);
    const viewsService = accessor.get(IViewsService);
    accessibleViewService.show(new ExtensionContentProvider(viewDescriptor.id, {
      type: "help"
    }, () => helpContent.value, () => viewsService.openView(viewDescriptor.id, true)));
    return true;
