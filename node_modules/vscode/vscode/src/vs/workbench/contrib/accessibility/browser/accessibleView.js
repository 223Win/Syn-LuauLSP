...
} = require('./accessibilityConfiguration.js');
const {
  IChatCodeBlockContextProviderService
} = require('../../chat/browser/chat.service.js');
const {
...
let AccessibleView = class AccessibleView extends Disposable {
  ...
  show(provider, symbol, showAccessibleViewHelp, position) {
    ...
    const delegate = {
      getAnchor: () => {
        return {
          x: getActiveWindow().innerWidth / 2 - Math.min(this._layoutService.activeContainerDimension.width * 0.62, 600) / 2,
    ...
    for (const token of tokens) {
      ...
      if (label) {
        symbols.push({
          ...
          label: localizeWithPath(_moduleId, 0, "({0}) {1}", token.type, label),
          ariaLabel: localizeWithPath(_moduleId, 1, "({0}) {1}", token.type, label),
          firstListItem
        });
  ...
  disableHint() {
    ...
    alert(localizeWithPath(_moduleId, 2, '{0} accessibility verbosity is now disabled', this._currentProvider.verbositySettingKey));
  }
  ...
  _render(provider, container, showAccessibleViewHelp) {
    ...
    const readMoreLink = provider.options.readMoreUrl ? localizeWithPath(_moduleId, 3, "\n\nOpen a browser window with more information related to accessibility (H).") : '';
    let disableHelpHint = '';
    ...
    this._getTextModel(URI.from({
      ...
      if (verbose && !showAccessibleViewHelp && hasActions) {
        actionsHint = provider.options.position ? localizeWithPath(_moduleId, 5, 'Explore actions such as disabling this hint (Shift+Tab), use Escape to exit this dialog.') : localizeWithPath(_moduleId, 6, 'Explore actions such as disabling this hint (Shift+Tab).');
      }
      ...
      this._editorWidget.updateOptions({
        ariaLabel
      });
  ...
  _getChatHints() {
    ...
    if (insertAtCursorKb) {
      hint += localizeWithPath(_moduleId, 15, " - Insert the code block at the cursor ({0}).\n", insertAtCursorKb);
    } else {
      hint += localizeWithPath(_moduleId, 16, " - Insert the code block at the cursor by configuring a keybinding for the Chat: Insert Code Block command.\n");
    }
    if (insertIntoNewFileKb) {
      hint += localizeWithPath(_moduleId, 17, " - Insert the code block into a new file ({0}).\n", insertIntoNewFileKb);
    } else {
      hint += localizeWithPath(_moduleId, 18, " - Insert the code block into a new file by configuring a keybinding for the Chat: Insert into New File command.\n");
    }
    if (runInTerminalKb) {
      hint += localizeWithPath(_moduleId, 19, " - Run the code block in the terminal ({0}).\n", runInTerminalKb);
    } else {
      hint += localizeWithPath(_moduleId, 20, " - Run the coe block in the terminal by configuring a keybinding for the Chat: Insert into Terminal command.\n");
    }
    return hint;
  }
  _getNavigationHint() {
    ...
    if (nextKeybinding && previousKeybinding) {
      hint = localizeWithPath(_moduleId, 21, "Show the next ({0}) or previous ({1}) item.", nextKeybinding, previousKeybinding);
    } else {
      hint = localizeWithPath(_moduleId, 22, "Show the next or previous item by configuring keybindings for the Show Next & Previous in Accessible View commands.");
    }
    return hint;
  }
  ...
  _getGoToSymbolHint(providerHasSymbols) {
    ...
    if (providerHasSymbols) {
      if (goToSymbolKb) {
        ...
        goToSymbolHint = localizeWithPath(_moduleId, 26, 'To go to a symbol, configure a keybinding for the command Go To Symbol in Accessible View');
      }
    }
    return goToSymbolHint;
  }
};
AccessibleView = __decorate([__param(0, IOpenerService), __param(1, IInstantiationService), __param(2, IConfigurationService), __param(3, IModelService), __param(4, IContextViewService), __param(5, IContextKeyService), __param(6, IAccessibilityService), __param(7, IKeybindingService), __param(8, ILayoutService), __param(9, IMenuService), __param(10, ICommandService), __param(11, IChatCodeBlockContextProviderService), __param(12, IStorageService)], AccessibleView);
let AccessibleViewService = class AccessibleViewService extends Disposable {
  ...
  getOpenAriaHint(verbositySettingKey) {
    ...
    if (keybinding) {
      ...
      hint = localizeWithPath(_moduleId, 28, "Inspect this in the accessible view via the command Open Accessible View which is currently not triggerable via keybinding.");
    }
    return hint;
  }
  disableHint() {
...
AccessibleViewService = __decorate([__param(0, IInstantiationService), __param(1, IConfigurationService), __param(2, IKeybindingService)], AccessibleViewService);
...
AccessibleViewSymbolQuickPick = __decorate([__param(1, IQuickInputService)], AccessibleViewSymbolQuickPick);
