...
} = require('./viewParts/notebookKernelView.js');
const {
  NOTEBOOK_EDITOR_ID,
...
const {
  NotebookPerfMarks
} = require('../common/notebookPerformance.js');
const {
...
} = require('../../../services/workingCopy/common/workingCopyBackup.service.js');
const {
  streamToBuffer
...
} = require('../common/services/notebookWorkerService.service.js');
const {
  IPreferencesService
...
const _moduleId = "vs/workbench/contrib/notebook/browser/notebookEditor";
  ...
  async setInput(input, options, context, token, noRetry) {
    try {
      ...
      if (model === null) {
        ...
        if (!knownProvider) {
          throw new Error(localizeWithPath(_moduleId, 0, "Cannot open resource with notebook editor type '{0}', please check if you have the right extension installed and enabled.", input.viewType));
        }
        ...
        throw createEditorOpenError(new Error(localizeWithPath(_moduleId, 1, "Cannot open resource with notebook editor type '{0}', please check if you have the right extension installed and enabled.", input.viewType)), [toAction({
          id: 'workbench.notebook.action.installOrEnableMissing',
          label: extensionInfo ? localizeWithPath(_moduleId, 2, "Enable extension for '{0}'", input.viewType) : localizeWithPath(_moduleId, 3, "Install extension for '{0}'", input.viewType),
          run: async () => {
            ...
            try {
              if (extensionInfo) {
                await this._extensionsWorkbenchService.setEnablement(extensionInfo, extensionInfo.enablementState === 7 ? 9 : 8);
              } else {
          ...
          label: localizeWithPath(_moduleId, 4, "Open As Text"),
          run: async () => {
      ...
      if (e.fileOperationResult === 7) {
        ...
        if (e instanceof TooLargeFileOperationError) {
          message = localizeWithPath(_moduleId, 5, "The notebook is not displayed in the notebook editor because it is very large ({0}).", ByteSize.formatSize(e.size));
        } else {
          message = localizeWithPath(_moduleId, 6, "The notebook is not displayed in the notebook editor because it is very large.");
        }
      ...
      const error = createEditorOpenError(e instanceof Error ? e : new Error(e ? e.message : ''), [toAction({
        ...
        label: localizeWithPath(_moduleId, 7, "Open in Text Editor"),
        run: async () => {
  ...
  _handlePerfMark(perf, input, notebook) {
    ...
    if (startTime !== undefined && extensionActivated !== undefined) {
      ...
      if (inputLoaded !== undefined) {
        ...
        webviewCommLoadingTimespan = inputLoaded - extensionActivated;
      }
