...
} = require('./notebookVisibleCellObserver.js');
const {
...
const {
  ...
  cellStatusIconError
} = require('../../notebookEditorWidget.js');
const {
  successStateIcon,
  errorStateIcon,
  pendingStateIcon,
...
} = require('../cellCommands/cellCommands.js');
const {
  IKeybindingService
} = require('../../../../../../platform/keybinding/common/keybinding.service.js');
var ExecutionStateCellStatusBarItem_1, TimerCellStatusBarItem_1;
const _moduleId = "vs/workbench/contrib/notebook/browser/contrib/cellStatusBar/executionStatusBarItemController";
  ...
  _getItemForState(runState, internalMetadata) {
    ...
    if (!state && lastRunSuccess) {
      ...
      return [{
        ...
        tooltip: localizeWithPath(_moduleId, 1, "Failed"),
        alignment: 1,
        priority: Number.MAX_SAFE_INTEGER
      }];
...
let TimerCellStatusBarItem = class TimerCellStatusBarItem extends Disposable {
  ...
  _getTimeItem(startTime, endTime, adjustment = 0, runtimeInformation) {
    ...
    if (runtimeInformation) {
      ...
      renderTimes += `\n*${localizeWithPath(_moduleId, 4, "Use the links above to file an issue using the issue reporter.")}*\n`;
      tooltip = {
        value: localizeWithPath(_moduleId, 5, "**Last Execution** {0}\n\n**Execution Time** {1}\n\n**Overhead Time** {2}\n\n**Render Times**\n\n{3}", lastExecution, formatCellDuration(executionDuration), formatCellDuration(timerDuration - executionDuration), renderTimes),
        isTrusted: true
      };
    }
    return {
...
TimerCellStatusBarItem = TimerCellStatusBarItem_1 = __decorate([__param(2, INotebookExecutionStateService), __param(3, INotebookService)], TimerCellStatusBarItem);
...
let DiagnosticCellStatusBarItem = class DiagnosticCellStatusBarItem extends Disposable {
  ...
  async _update() {
    ...
    if (!!this.cell.cellDiagnostics.ErrorDetails) {
      ...
      const tooltip = localizeWithPath(_moduleId, 6, "Quick Actions {0}", `(${keybinding})`);
      item = {
        text: `$(sparkle)`,
        tooltip,
        alignment: 1,
...
DiagnosticCellStatusBarItem = __decorate([__param(2, IKeybindingService)], DiagnosticCellStatusBarItem);
