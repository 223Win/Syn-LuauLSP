...
} = require('./notebookIndentationActions.js');
const {
...
registerAction2(class ChangeCellLanguageAction extends NotebookCellAction {
  ...
  async showLanguagePicker(accessor, context) {
    ...
    const picks = [autoDetectMode, {
      type: 'separator',
      label: localizeWithPath(_moduleId, 12, "languages (identifier)")
    }, ...topItems, {
      type: 'separator'
    }, ...mainItems];
    const selection = await quickInputService.pick(picks, {
      placeHolder: localizeWithPath(_moduleId, 13, "Select Language Mode")
    });
  ...
  async showNotebookIndentationPicker(accessor, context) {
    ...
    if (!activeNotebook || activeNotebook.isDisposed) {
      return quickInputService.pick([{
        label: localizeWithPath(_moduleId, 17, "No notebook editor active at this time")
      }]);
    }
    if (activeNotebook.isReadOnly) {
      return quickInputService.pick([{
        label: localizeWithPath(_moduleId, 18, "The active notebook editor is read-only.")
      }]);
    }
    const picks = [new NotebookIndentUsingTabs(), new NotebookIndentUsingSpaces(), new NotebookChangeTabDisplaySize(), new NotebookIndentationToTabsAction(), new NotebookIndentationToSpacesAction()].map(item => {
      return {
        id: item.desc.id,
    ...
    picks.splice(3, 0, {
      type: 'separator',
      label: localizeWithPath(_moduleId, 19, "convert file")
    });
    picks.unshift({
      type: 'separator',
      label: localizeWithPath(_moduleId, 20, "change view")
    });
    const action = await quickInputService.pick(picks, {
      placeHolder: localizeWithPath(_moduleId, 21, "Select Action"),
      matchOnDetail: true
    });
    if (!action) {
      return;
...
exports.CLEAR_CELL_OUTPUTS_COMMAND_ID = CLEAR_CELL_OUTPUTS_COMMAND_ID;
exports.SELECT_NOTEBOOK_INDENTATION_ID = SELECT_NOTEBOOK_INDENTATION_ID;