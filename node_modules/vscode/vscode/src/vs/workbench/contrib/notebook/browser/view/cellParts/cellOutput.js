...
} = require('../../controller/cellOutputActions.js');
const {
  CLEAR_CELL_OUTPUTS_COMMAND_ID
} = require('../../controller/editActions.js');
const {
...
let CellOutputElement = class CellOutputElement extends Disposable {
  ...
  _renderMissingRenderer(viewModel, preferredMimeType) {
    ...
    if (!preferredMimeType) {
      ...
      return this._renderMessage(viewModel, localizeWithPath(_moduleId, 1, "No renderer could be found for output. It has the following mimetypes: {0}", mimeTypesMessage));
    }
  ...
  async _pickActiveMimeTypeRenderer(outputItemDiv, notebookTextModel, kernel, viewModel) {
    ...
    picker.placeholder = items.length !== mimeTypes.length ? localizeWithPath(_moduleId, 5, "Select mimetype to render for current output") : localizeWithPath(_moduleId, 6, "Select mimetype to render for current output");
...
CellOutputElement = __decorate([__param(5, INotebookService), __param(6, IQuickInputService), __param(7, IContextKeyService), __param(8, IMenuService), __param(9, IPaneCompositePartService), __param(10, IInstantiationService)], CellOutputElement);
