...
} = require('../../../../inlineChat/browser/inlineChatSavingService.service.js');
const {
  SessionPrompt,
  ReplyResponse,
  SessionExchange,
  EmptyResponse,
  ErrorResponse
} = require('../../../../inlineChat/browser/inlineChatSession.js');
const {
  IInlineChatSessionService
} = require('../../../../inlineChat/browser/inlineChatSessionService.service.js');
const {
  InlineChatWidget
} = require('../../../../inlineChat/browser/inlineChatWidget.js');
const {
  performAsyncTextEdit,
  asProgressiveEdit
} = require('../../../../inlineChat/browser/utils.js');
const {
  CTX_INLINE_CHAT_LAST_RESPONSE_TYPE
} = require('../../../../inlineChat/common/inlineChat.js');
const {
  insertCell,
  runDeleteAction
} = require('../cellOperations.js');
const {
...
} = require('./notebookChatContext.js');
const {
...
let NotebookChatController = class NotebookChatController extends Disposable {
  ...
  _createWidget(index, input, autoSend, initEditingCell) {
    ...
    const fakeParentEditor = this._widgetDisposableStore.add(this._instantiationService.createInstance(CodeEditorWidget, fakeParentEditorElement, {}, {
      isSimpleWidget: true
    }));
    ...
    const progress = new AsyncProgress(async data => {
      ...
      if (data.edits?.length) {
        ...
        progressiveEditsQueue.queue(async () => {
          await this._makeChanges(data.edits, data.editsShouldBeInstant ? undefined : {
  ...
  async _startSession(editor, token) {
    ...
    const session = await this._inlineChatSessionService.createSession(editor, {
      editMode: "live"
    }, token);
    if (!session) {
      return;
  ...
  async feedbackLast(kind) {
    if (this._activeSession?.lastExchange && this._activeSession.lastExchange.response instanceof ReplyResponse) {
      ...
      this._widget?.inlineChatWidget.updateStatus('Thank you for your feedback!', {
        resetAfter: 1250
      });
    }
  }
...
NotebookChatController = NotebookChatController_1 = __decorate([__param(1, IInstantiationService), __param(2, IInlineChatSessionService), __param(3, IContextKeyService), __param(4, ICommandService), __param(5, IEditorWorkerService), __param(6, IInlineChatSavingService), __param(7, IModelService), __param(8, ILanguageService), __param(9, INotebookExecutionStateService), __param(10, IStorageService)], NotebookChatController);
class EditStrategy {
  ...
  async cancel() {
    const {
      textModelN: modelN,
      textModelNAltVersion,
      textModelNSnapshotAltVersion
    } = this._session;
