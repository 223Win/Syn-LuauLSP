...
const _moduleId = "vs/workbench/contrib/notebook/browser/viewParts/notebookKernelQuickPickStrategy";
...
function toKernelQuickPick(kernel, selected) {
  ...
  if (kernel.id === selected?.id) {
    if (!res.description) {
      res.description = localizeWithPath(_moduleId, 0, "Currently Selected");
    } else {
      res.description = localizeWithPath(_moduleId, 1, "{0} - Currently Selected", res.description);
    }
  }
  return res;
}
class KernelPickerStrategyBase {
  ...
  async showQuickPick(editor, wantedId, skipAutoRun) {
    ...
    quickPick.placeholder = selected ? localizeWithPath(_moduleId, 2, "Change kernel for '{0}'", this._labelService.getUriLabel(notebook.uri, {
      relative: true
    })) : localizeWithPath(_moduleId, 3, "Select kernel for '{0}'", this._labelService.getUriLabel(notebook.uri, {
      relative: true
    }));
    ...
    const extensionRecommendataionPromise = quickPickItems.length === 0 ? createCancelablePromise(token => this._showInstallKernelExtensionRecommendation(notebook, quickPick, this._extensionWorkbenchService, token)) : undefined;
  ...
  async _getKernelRecommendationsQuickPickItems(notebookTextModel, extensionWorkbenchService) {
    ...
    if (suggestedExtension) {
      ...
      const extensions = extensionWorkbenchService.installed.filter(e => (e.enablementState === 3 || e.enablementState === 8 || e.enablementState === 9) && suggestedExtension.extensionIds.includes(e.identifier.id));
      ...
      quickPickItems.push({
        ...
        label: `$(${Codicon.lightbulb.id}) ` + localizeWithPath(_moduleId, 4, 'Install/Enable suggested extensions'),
        extensionIds: suggestedExtension.extensionIds
      });
    }
    quickPickItems.push({
      id: 'install',
      label: localizeWithPath(_moduleId, 5, "Browse marketplace for kernel extensions")
    });
...
let KernelPickerMRUStrategy = class KernelPickerMRUStrategy extends KernelPickerStrategyBase {
  ...
  _getKernelPickerQuickPickItems(notebookTextModel, matchResult, notebookKernelService, scopedContextKeyService) {
    ...
    quickPickItems.push({
      id: 'selectAnother',
      label: localizeWithPath(_moduleId, 6, "Select Another Kernel..."),
  ...
  async displaySelectAnotherQuickPick(editor, kernelListEmpty) {
    ...
    const quickPickItem = await new Promise(resolve => {
      quickPick.title = kernelListEmpty ? localizeWithPath(_moduleId, 7, "Select Kernel") : localizeWithPath(_moduleId, 8, "Select Another Kernel");
      quickPick.placeholder = localizeWithPath(_moduleId, 9, "Type to choose a kernel source");
      quickPick.busy = true;
  ...
  async _calculdateKernelSources(editor) {
    ...
    quickPickItems.push(...validActions.map(action => {
      const buttons = action.documentation ? [{
        ...
        tooltip: localizeWithPath(_moduleId, 10, 'Learn More')
      }] : [];
      return {
  ...
  async _selectOneKernel(notebook, source, kernels) {
    ...
    quickPick.items = quickPickItems;
    quickPick.canSelectMany = false;
    quickPick.title = localizeWithPath(_moduleId, 11, "Select Kernel from {0}", source);
  ...
  static updateKernelStatusAction(notebook, action, notebookKernelService, notebookKernelHistoryService) {
    ...
    if (detectionTasks.length) {
      ...
      if (info.selected) {
        ...
        action.tooltip = kernelInfo ? localizeWithPath(_moduleId, 12, "Selected Kernel: {0} (Kernel Detection Tasks Running)", kernelInfo) : localizeWithPath(_moduleId, 13, "Detecting Kernels");
      } else {
        action.label = localizeWithPath(_moduleId, 13, "Detecting Kernels");
      }
      return;
    }
    ...
    if (selected) {
      ...
      action.label = localizeWithPath(_moduleId, 7, "Select Kernel");
      action.class = ThemeIcon.asClassName(selectKernelIcon);
      action.tooltip = '';
    }
  }
...
KernelPickerMRUStrategy = __decorate([__param(0, INotebookKernelService), __param(1, IProductService), __param(2, IQuickInputService), __param(3, ILabelService), __param(4, ILogService), __param(5, IPaneCompositePartService), __param(6, IExtensionsWorkbenchService), __param(7, IExtensionService), __param(8, ICommandService), __param(9, INotebookKernelHistoryService), __param(10, IOpenerService)], KernelPickerMRUStrategy);
