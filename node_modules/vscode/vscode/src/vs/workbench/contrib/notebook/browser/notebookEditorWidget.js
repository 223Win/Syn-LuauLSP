...
const {} = require('./media/notebook.css.js');
const {} = require('./media/notebookCellChat.css.js');
const {} = require('./media/notebookCellEditorHint.css.js');
const {} = require('./media/notebookCellInsertToolbar.css.js');
const {} = require('./media/notebookCellStatusBar.css.js');
const {} = require('./media/notebookCellTitleToolbar.css.js');
const {} = require('./media/notebookFocusIndicator.css.js');
const {} = require('./media/notebookToolbar.css.js');
const {} = require('./media/notebookDnd.css.js');
const {} = require('./media/notebookFolding.css.js');
const {} = require('./media/notebookCellOutput.css.js');
const {} = require('./media/notebookEditorStickyScroll.css.js');
const {} = require('./media/notebookKernelActionViewItem.css.js');
const {} = require('./media/notebookOutline.css.js');
const {
  getWindow,
  append,
  createStyleSheet,
...
const {
  FontMeasurements
} = require('../../../../editor/browser/config/fontMeasurements.js');
const {
  BareFontInfo
...
const {
  registerZIndex,
  ZIndex
} = require('../../../../platform/layout/browser/zIndexRegistry.js');
const {
...
} = require('../../debug/browser/debugColors.js');
const {
  CellEditState,
  CellFocusMode,
  CellRevealRangeType,
...
} = require('./notebookEditorExtensions.js');
const {
  INotebookEditorService
} = require('./services/notebookEditorService.service.js');
const {
  notebookDebug
} = require('./notebookLogger.js');
const {
  NotebookLayoutChangedEvent
} = require('./notebookViewEvents.js');
const {
  CellContextKeyManager
} = require('./view/cellParts/cellContextKeys.js');
const {
  CellDragAndDropController
} = require('./view/cellParts/cellDnd.js');
const {
  NotebookCellList,
  ListViewInfoAccessor,
  NOTEBOOK_WEBVIEW_BOUNDARY
} = require('./view/notebookCellList.js');
const {
  BackLayerWebView
} = require('./view/renderers/backLayerWebView.js');
const {
  CodeCellRenderer,
  MarkupCellRenderer,
  NotebookCellListDelegate
} = require('./view/renderers/cellRenderer.js');
const {
  outputDisplayLimit,
  CodeCellViewModel
} = require('./viewModel/codeCellViewModel.js');
const {
  NotebookEventDispatcher
} = require('./viewModel/eventDispatcher.js');
const {
  MarkupCellViewModel
} = require('./viewModel/markupCellViewModel.js');
const {
  NotebookViewModel
} = require('./viewModel/notebookViewModelImpl.js');
const {
  ViewContext
} = require('./viewModel/viewContext.js');
const {
  NotebookEditorWorkbenchToolbar
} = require('./viewParts/notebookEditorToolbar.js');
const {
  NotebookEditorContextKeys
} = require('./viewParts/notebookEditorWidgetContextKeys.js');
const {
  NotebookOverviewRuler
} = require('./viewParts/notebookOverviewRuler.js');
const {
  ListTopCellToolbar
} = require('./viewParts/notebookTopCellToolbar.js');
const {
  SelectionStateType,
...
} = require('../common/notebookExecutionService.service.js');
const {
...
} = require('./notebookOptions.js');
const {
  INotebookRendererMessagingService
} = require('../common/notebookRendererMessagingService.service.js');
const {
  INotebookService
} = require('../common/notebookService.service.js');
const {
...
} = require('./viewModel/cellEditorOptions.js');
const {
  FloatingEditorClickMenu
} = require('../../../browser/codeeditor.js');
const {
  CellFindMatchModel
} = require('./contrib/find/findModel.js');
const {
  INotebookLoggingService
} = require('../common/notebookLoggingService.service.js');
const {
  Schemas
...
} = require('./viewParts/notebookEditorStickyScroll.js');
const {
  IKeybindingService
...
} = require('../../webview/browser/webview.contribution.js');
const {
  NotebookAccessibilityProvider
} = require('./notebookAccessibilityProvider.js');
const _moduleId = "vs/workbench/contrib/notebook/browser/notebookEditorWidget";
const $ = $$1;
function getDefaultNotebookCreationOptions() {
  const skipContributions = ['editor.contrib.review', FloatingEditorClickMenu.ID, 'editor.contrib.dirtydiff', 'editor.contrib.testingOutputPeek', 'editor.contrib.testingDecorations', 'store.contrib.stickyScrollController', 'editor.contrib.findController', 'editor.contrib.emptyTextEditorHint'];
...
let NotebookEditorWidget = class NotebookEditorWidget extends Disposable {
  ...
  constructor(creationOptions, dimension, instantiationService, editorGroupsService, notebookRendererMessaging, notebookEditorService, notebookKernelService, _notebookService, configurationService, contextKeyService, layoutService, contextMenuService, telemetryService, notebookExecutionService, notebookExecutionStateService, editorProgressService, logService, keybindingService, codeEditorService) {
    ...
    this._register(this._notebookOptions.onDidChangeOptions(e => {
      ...
      if (e.compactView || e.focusIndicator || e.insertToolbarPosition || e.cellToolbarLocation || e.dragAndDropEnabled || e.fontSize || e.markupFontSize || e.fontFamily || e.insertToolbarAlignment || e.outputFontSize || e.outputLineHeight || e.outputFontFamily || e.outputWordWrap || e.outputScrolling || e.outputLinkifyFilePaths || e.minimalError) {
  ...
  _createCellList() {
    ...
    const renderers = [this.instantiationService.createInstance(CodeCellRenderer, this, this._renderedEditors, this._dndController, getScopedContextKeyService), this.instantiationService.createInstance(MarkupCellRenderer, this, this._dndController, this._renderedEditors, getScopedContextKeyService)];
  ...
  async _warmupViewportMarkdownCells(viewModel, viewState) {
    if (viewState && viewState.cellTotalHeights) {
      ...
      const initRequests = viewModel.viewCells.filter(cell => cell.cellKind === CellKind.Markup).slice(0, 5).map(cell => this.createMarkupCellInitialization(cell, -10000));
  ...
  async revealFirstLineIfOutsideViewport(cell) {
    await this._list.revealCell(cell, 6);
  }
  ...
  async layoutNotebookCell(cell, height, context) {
    ...
    const doLayout = () => {
      ...
      if (!this.hasEditorFocus()) {
        ...
        if (cellIndex !== undefined && visibleRanges && visibleRanges.length && visibleRanges[0].start === cellIndex && this._list.scrollTop > this.getAbsoluteTopOfElement(cell)) {
...
}, localizeWithPath(_moduleId, 0, "The border color for notebook cells."));
registerColor('notebook.focusedEditorBorder', {
  light: focusBorder,
  dark: focusBorder,
  hcDark: focusBorder,
  hcLight: focusBorder
}, localizeWithPath(_moduleId, 1, "The color of the notebook cell editor border."));
...
}, localizeWithPath(_moduleId, 2, "The error icon color of notebook cells in the cell status bar."));
const runningCellRulerDecorationColor = registerColor('notebookEditorOverviewRuler.runningCellForeground', {
  light: debugIconStartForeground,
  dark: debugIconStartForeground,
  hcDark: debugIconStartForeground,
  hcLight: debugIconStartForeground
}, localizeWithPath(_moduleId, 3, "The color of the running cell decoration in the notebook editor overview ruler."));
...
}, localizeWithPath(_moduleId, 4, "The error icon color of notebook cells in the cell status bar."));
registerColor('notebookStatusRunningIcon.foreground', {
  light: foreground,
  dark: foreground,
  hcDark: foreground,
  hcLight: foreground
}, localizeWithPath(_moduleId, 5, "The running icon color of notebook cells in the cell status bar."));
registerColor('notebook.outputContainerBorderColor', {
  dark: null,
  light: null,
  hcDark: null,
  hcLight: null
}, localizeWithPath(_moduleId, 6, "The border color of the notebook output container."));
registerColor('notebook.outputContainerBackgroundColor', {
  dark: null,
  light: null,
  hcDark: null,
  hcLight: null
}, localizeWithPath(_moduleId, 7, "The color of the notebook output container background."));
registerColor('notebook.cellToolbarSeparator', {
  dark: Color.fromHex('#808080').transparent(0.35),
  light: Color.fromHex('#808080').transparent(0.35),
  hcDark: contrastBorder,
  hcLight: contrastBorder
}, localizeWithPath(_moduleId, 8, "The color of the separator in the cell bottom toolbar"));
const focusedCellBackground = registerColor('notebook.focusedCellBackground', {
  dark: null,
  light: null,
  hcDark: null,
  hcLight: null
}, localizeWithPath(_moduleId, 9, "The background color of a cell when the cell is focused."));
registerColor('notebook.selectedCellBackground', {
  dark: listInactiveSelectionBackground,
  light: listInactiveSelectionBackground,
  hcDark: null,
  hcLight: null
}, localizeWithPath(_moduleId, 10, "The background color of a cell when the cell is selected."));
registerColor('notebook.cellHoverBackground', {
  ...
  hcDark: null,
  hcLight: null
}, localizeWithPath(_moduleId, 11, "The background color of a cell when the cell is hovered."));
registerColor('notebook.selectedCellBorder', {
  dark: notebookCellBorder,
  light: notebookCellBorder,
  hcDark: contrastBorder,
  hcLight: contrastBorder
}, localizeWithPath(_moduleId, 12, "The color of the cell's top and bottom border when the cell is selected but not focused."));
registerColor('notebook.inactiveSelectedCellBorder', {
  dark: null,
  light: null,
  hcDark: focusBorder,
  hcLight: focusBorder
}, localizeWithPath(_moduleId, 13, "The color of the cell's borders when multiple cells are selected."));
registerColor('notebook.focusedCellBorder', {
  dark: focusBorder,
  light: focusBorder,
  hcDark: focusBorder,
  hcLight: focusBorder
}, localizeWithPath(_moduleId, 14, "The color of the cell's focus indicator borders when the cell is focused."));
registerColor('notebook.inactiveFocusedCellBorder', {
  dark: notebookCellBorder,
  light: notebookCellBorder,
  hcDark: notebookCellBorder,
  hcLight: notebookCellBorder
}, localizeWithPath(_moduleId, 15, "The color of the cell's top and bottom border when a cell is focused while the primary focus is outside of the editor."));
...
}, localizeWithPath(_moduleId, 16, "The background color of notebook cell status bar items."));
registerColor('notebook.cellInsertionIndicator', {
  light: focusBorder,
  dark: focusBorder,
  hcDark: focusBorder,
  hcLight: focusBorder
}, localizeWithPath(_moduleId, 17, "The color of the notebook cell insertion indicator."));
...
}, localizeWithPath(_moduleId, 18, "Notebook scrollbar slider background color."));
...
}, localizeWithPath(_moduleId, 19, "Notebook scrollbar slider background color when hovering."));
...
}, localizeWithPath(_moduleId, 20, "Notebook scrollbar slider background color when clicked on."));
registerColor('notebook.symbolHighlightBackground', {
  dark: Color.fromHex('#ffffff0b'),
  light: Color.fromHex('#fdff0033'),
  hcDark: null,
  hcLight: null
}, localizeWithPath(_moduleId, 21, "Background color of highlighted cell"));
registerColor('notebook.cellEditorBackground', {
  light: SIDE_BAR_BACKGROUND,
  dark: SIDE_BAR_BACKGROUND,
  hcDark: null,
  hcLight: null
}, localizeWithPath(_moduleId, 22, "Cell editor background color."));
const notebookEditorBackground = registerColor('notebook.editorBackground', {
  ...
  hcDark: null,
  hcLight: null
}, localizeWithPath(_moduleId, 23, "Notebook background color."));
exports.NotebookEditorWidget = NotebookEditorWidget;
exports.cellStatusIconError = cellStatusIconError;
exports.cellStatusIconSuccess = cellStatusIconSuccess;
exports.focusedCellBackground = focusedCellBackground;
exports.getDefaultNotebookCreationOptions = getDefaultNotebookCreationOptions;
exports.notebookCellBorder = notebookCellBorder;
exports.runningCellRulerDecorationColor = runningCellRulerDecorationColor;