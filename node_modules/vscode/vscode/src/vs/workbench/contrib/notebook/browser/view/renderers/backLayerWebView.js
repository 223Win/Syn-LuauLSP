...
} = require('../../../../../../editor/common/languages/supports/tokenization.js');
const {
  tokenizeToString
} = require('../../../../../../editor/common/languages/textToHtmlTokenizer.js');
const {
  localizeWithPath
} = require('../../../../../../nls.js');
const {
...
} = require('../notebookCellList.js');
const {
  preloadsScriptStr
} = require('./webviewPreloads.js');
const {
  transformWebviewThemeVars
} = require('./webviewThemeMapping.js');
const {
  MarkupCellViewModel
...
const {
  WebviewOriginStore
} = require('../../../../webview/browser/webview.js');
const {
  IWebviewService
} = require('../../../../webview/browser/webview.service.js');
const {
...
const {
  ...
  asWebviewUri
} = require('../../../../webview/common/webview.js');
const {
...
const _moduleId = "vs/workbench/contrib/notebook/browser/view/renderers/backLayerWebView";
...
let BackLayerWebView = class BackLayerWebView extends Themable {
  ...
  _generateStyles() {
    return {
      ...
      'notebook-cell-markup-empty-content': localizeWithPath(_moduleId, 0, "Empty markdown cell, double-click or press enter to edit."),
      'notebook-cell-renderer-not-found-error': localizeWithPath(_moduleId, 1, "No renderer found for '$0'"),
      'notebook-cell-renderer-fallbacks-exhausted': localizeWithPath(_moduleId, 2, "Could not render content for '$0'")
    };
  }
  ...
  getBuiltinLocalResourceRoots() {
    ...
    return [dirname(FileAccess.asFileUri('vs/loader.js'))];
  }
        ...
        case 'clicked-link':
          {
            if (matchesScheme(data.href, Schemas.command)) {
              ...
              this.openerService.open(data.href, {
                ...
                fromWorkspace: true,
                allowCommands: ['github-issues.authNow', 'workbench.extensions.search', 'workbench.action.openSettings', '_notebook.selectKernel', 'jupyter.viewOutput']
              });
              return;
            }
  ...
  _openUri(uri) {
    ...
    if (match) {
      match.group.openEditor(match.editor, lineNumber !== undefined && column !== undefined ? {
        selection: {
          startLineNumber: lineNumber,
          startColumn: column
        }
  ...
  _createInset(webviewService, content) {
    ...
    const webview = webviewService.createWebviewElement({
      ...
      title: localizeWithPath(_moduleId, 3, "Notebook webview content"),
      options: {
  ...
  _getResourceRootsCache() {
    ...
    return [this.notebookService.getNotebookProviderResourceRoots(), this.notebookService.getRenderers().map(x => dirname(x.entrypoint.path)), ...Array.from(this.notebookService.getStaticPreloads(this.notebookViewType), x => [dirname(x.entrypoint), ...x.localResourceRoots]), workspaceFolders, notebookDir, this.getBuiltinLocalResourceRoots()].flat();
  }
  initializeWebViewState() {
    ...
    if (this.initializeMarkupPromise?.isFirstInit) ;else {
  ...
  updateOutputRenderers() {
    ...
    const renderersData = this.getRendererData();
    this.localResourceRootsCache = this._getResourceRootsCache();
    const mixedResourceRoots = [...(this.localResourceRootsCache || []), ...(this._currentKernel ? [this._currentKernel.localResourceRoot] : [])];
