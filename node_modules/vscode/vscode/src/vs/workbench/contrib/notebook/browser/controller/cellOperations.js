...
} = require('../../common/model/notebookCellTextModel.js');
const {
  SelectionStateType,
  CellKind
...
const _moduleId = "vs/workbench/contrib/notebook/browser/controller/cellOperations";
...
async function joinSelectedCells(bulkEditService, notificationService, context) {
  ...
  if (!isSameKind) {
    const message = localizeWithPath(_moduleId, 0, "Cannot join cells of different kinds");
  ...
  if (edits.length) {
    await bulkEditService.apply(edits, {
      quotableLabel: localizeWithPath(_moduleId, 1, "Join Notebook Cells")
    });
  }
}
