...
const {
  FoldingRegions
} = require('../../../../../editor/contrib/folding/browser/foldingRanges.js');
const {
  sanitizeRanges
} = require('../../../../../editor/contrib/folding/browser/syntaxRangeProvider.js');
const {
  CellKind
...
function* getMarkdownHeadersInCell(cellContent) {
  for (const token of marked_1.lexer(cellContent, {
    gfm: true
  })) {
    if (token.type === 'heading') {
      yield {
        depth: token.depth,
        text: renderMarkdownAsPlaintext({
          value: token.raw
        }).trim()
      };
    }
