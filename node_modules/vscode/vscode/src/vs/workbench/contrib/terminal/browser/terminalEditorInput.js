...
} = require('./terminal.service.js');
const {
  getColorClass,
  getUriClasses
} = require('./terminalIcon.js');
const {
...
const {
  TerminalLocation,
  TerminalExitReason
} = require('../../../../platform/terminal/common/terminal.js');
const {
  ILifecycleService
...
const {
  TerminalContextKeys
} = require('../common/terminalContextKey.js');
const {
  IDialogService
...
let TerminalEditorInput = class TerminalEditorInput extends EditorInput {
  ...
  async confirm(terminals) {
    const {
      ...
      detail: terminals.length > 1 ? terminals.map(terminal => terminal.editor.getName()).join('\n') + '\n\n' + localizeWithPath(_moduleId, 2, "Closing will terminate the running processes in the terminals.") : localizeWithPath(_moduleId, 3, "Closing will terminate the running processes in this terminal.")
    });
