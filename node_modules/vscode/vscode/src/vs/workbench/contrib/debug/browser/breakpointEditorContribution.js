...
const {
  BrowserFeatures
} = require('../../../../base/browser/canIUse.js');
const {
...
} = require('../../codeEditor/browser/editorLineNumberMenu.js');
const {
  getBreakpointMessageAndIcon
} = require('./breakpointsView.js');
const {
  BreakpointWidget
} = require('./breakpointWidget.js');
const {
  debugBreakpointHint,
  allBreakpoints,
...
function createBreakpointDecorations(accessor, model, breakpoints, state, breakpointsActivated, showBreakpointsInOverviewRuler) {
  ...
  breakpoints.forEach(breakpoint => {
    ...
    const range = model.validateRange(breakpoint.column ? new Range(breakpoint.lineNumber, breakpoint.column, breakpoint.lineNumber, breakpoint.column + 1) : new Range(breakpoint.lineNumber, column, breakpoint.lineNumber, column + 1));
    result.push({
...
function getBreakpointDecorationOptions(accessor, model, breakpoint, state, breakpointsActivated, showBreakpointsInOverviewRuler, hasOtherBreakpointsOnLine) {
  ...
  } = getBreakpointMessageAndIcon(state, breakpointsActivated, breakpoint, labelService, debugService.getModel());
...
let BreakpointEditorContribution = class BreakpointEditorContribution {
    ...
    this.toDispose.push(this.editor.onMouseDown(async e => {
      ...
      if (e.event.rightButton || isMacintosh && e.event.leftButton && e.event.ctrlKey) {
        ...
        if (breakpoints.length) {
          ...
          } else if (!isLinux && breakpoints.some(bp => !!bp.condition || !!bp.logMessage || !!bp.hitCondition || !!bp.triggeredBy)) {
            const logPoint = breakpoints.every(bp => !!bp.logMessage);
            const breakpointType = logPoint ? localizeWithPath(_moduleId, 1, "Logpoint") : localizeWithPath(_moduleId, 2, "Breakpoint");
            const disabledBreakpointDialogMessage = localizeWithPath(_moduleId, 3, "This {0} has a {1} that will get lost on remove. Consider enabling the {0} instead.", breakpointType.toLowerCase(), logPoint ? localizeWithPath(_moduleId, 4, "message") : localizeWithPath(_moduleId, 5, "condition"));
            const enabledBreakpointDialogMessage = localizeWithPath(_moduleId, 6, "This {0} has a {1} that will get lost on remove. Consider disabling the {0} instead.", breakpointType.toLowerCase(), logPoint ? localizeWithPath(_moduleId, 4, "message") : localizeWithPath(_moduleId, 5, "condition"));
  ...
  getContextMenuActions(breakpoints, uri, lineNumber, column) {
    ...
    if (breakpoints.length === 1) {
      ...
      actions.push(new Action(`workbench.debug.viewlet.action.toggleBreakpoint`, breakpoints[0].enabled ? localizeWithPath(_moduleId, 13, "Disable {0}", breakpointType) : localizeWithPath(_moduleId, 14, "Enable {0}", breakpointType), undefined, true, () => this.debugService.enableOrDisableBreakpoints(!breakpoints[0].enabled, breakpoints[0])));
    } else if (breakpoints.length > 1) {
      const sorted = breakpoints.slice().sort((first, second) => first.column && second.column ? first.column - second.column : 1);
      actions.push(new SubmenuAction('debug.removeBreakpoints', localizeWithPath(_moduleId, 15, "Remove Breakpoints"), sorted.map(bp => new Action('removeInlineBreakpoint', bp.column ? localizeWithPath(_moduleId, 16, "Remove Inline Breakpoint on Column {0}", bp.column) : localizeWithPath(_moduleId, 17, "Remove Line Breakpoint"), undefined, true, () => this.debugService.removeBreakpoints(bp.getId())))));
      actions.push(new SubmenuAction('debug.editBreakpoints', localizeWithPath(_moduleId, 18, "Edit Breakpoints"), sorted.map(bp => new Action('editBreakpoint', bp.column ? localizeWithPath(_moduleId, 19, "Edit Inline Breakpoint on Column {0}", bp.column) : localizeWithPath(_moduleId, 20, "Edit Line Breakpoint"), undefined, true, () => Promise.resolve(this.showBreakpointWidget(bp.lineNumber, bp.column))))));
      actions.push(new SubmenuAction('debug.enableDisableBreakpoints', localizeWithPath(_moduleId, 21, "Enable/Disable Breakpoints"), sorted.map(bp => new Action(bp.enabled ? 'disableColumnBreakpoint' : 'enableColumnBreakpoint', bp.enabled ? bp.column ? localizeWithPath(_moduleId, 22, "Disable Inline Breakpoint on Column {0}", bp.column) : localizeWithPath(_moduleId, 23, "Disable Line Breakpoint") : bp.column ? localizeWithPath(_moduleId, 24, "Enable Inline Breakpoint on Column {0}", bp.column) : localizeWithPath(_moduleId, 25, "Enable Line Breakpoint"), undefined, true, () => this.debugService.enableOrDisableBreakpoints(!bp.enabled, bp)))));
    } else {
      actions.push(new Action('addBreakpoint', localizeWithPath(_moduleId, 26, "Add Breakpoint"), undefined, true, () => this.debugService.addBreakpoints(uri, [{
        lineNumber,
        column
      }])));
...
const debugIconBreakpointForeground = registerColor('debugIcon.breakpointForeground', {
  dark: '#E51400',
  light: '#E51400',
  hcDark: '#E51400',
  hcLight: '#E51400'
}, localizeWithPath(_moduleId, 31, 'Icon color for breakpoints.'));
const debugIconBreakpointDisabledForeground = registerColor('debugIcon.breakpointDisabledForeground', {
  dark: '#848484',
  light: '#848484',
  hcDark: '#848484',
  hcLight: '#848484'
}, localizeWithPath(_moduleId, 32, 'Icon color for disabled breakpoints.'));
const debugIconBreakpointUnverifiedForeground = registerColor('debugIcon.breakpointUnverifiedForeground', {
  dark: '#848484',
  light: '#848484',
  hcDark: '#848484',
  hcLight: '#848484'
}, localizeWithPath(_moduleId, 33, 'Icon color for unverified breakpoints.'));
const debugIconBreakpointCurrentStackframeForeground = registerColor('debugIcon.breakpointCurrentStackframeForeground', {
  dark: '#FFCC00',
  light: '#BE8700',
  hcDark: '#FFCC00',
  hcLight: '#BE8700'
}, localizeWithPath(_moduleId, 34, 'Icon color for the current breakpoint stack frame.'));
const debugIconBreakpointStackframeForeground = registerColor('debugIcon.breakpointStackframeForeground', {
  dark: '#89D185',
  light: '#89D185',
  hcDark: '#89D185',
  hcLight: '#89D185'
}, localizeWithPath(_moduleId, 35, 'Icon color for all breakpoint stack frames.'));
...
exports.debugIconBreakpointForeground = debugIconBreakpointForeground;