...
} = require('./debugSource.js');
const {
  DisassemblyViewInput
} = require('./disassemblyViewInput.js');
const {
  ITextFileService
} = require('../../../services/textfile/common/textfiles.service.js');
const _moduleId = "vs/workbench/contrib/debug/common/debugModel";
class ExpressionContainer {
  ...
  async doGetChildren() {
    ...
    if (!!this.indexedVariables && this.indexedVariables > chunkSize) {
      ...
      for (let i = 0; i < numberOfChunks; i++) {
        ...
        children.push(new Variable(this.session, this.threadId, this, this.reference, `[${start}..${start + count - 1}]`, '', '', undefined, count, undefined, {
          kind: 'virtual'
        }, undefined, undefined, true, start));
      }
      return children;
    }
  ...
  async fetchVariables(start, count, filter) {
    try {
      ...
      const vars = response.body.variables.filter(v => !!v).map(v => {
        ...
        return new Variable(this.session, this.threadId, this, 0, '', undefined, localizeWithPath(_moduleId, 0, "Invalid variable attributes"), 0, 0, undefined, {
          kind: 'virtual'
      ...
      return [new Variable(this.session, this.threadId, this, 0, '', undefined, e.message, 0, 0, undefined, {
        kind: 'virtual'
      }, undefined, undefined, false)];
    }
  }
  ...
  async evaluateExpression(expression, session, stackFrame, context, keepLazyVars = false) {
    if (!session || !stackFrame && context !== 'repl') {
      this.value = context === 'repl' ? localizeWithPath(_moduleId, 1, "Please start a debug session to evaluate expressions") : Expression.DEFAULT_VALUE;
      this.reference = 0;
      return false;
    }
    this.session = session;
...
class Expression extends ExpressionContainer {
  static {
    this.DEFAULT_VALUE = localizeWithPath(_moduleId, 2, "not available");
  }
...
class Thread {
  ...
  getTopStackFrame() {
    ...
    const firstAvailableStackFrame = callStack.find(sf => !!(sf && (this.stoppedDetails?.reason === 'instruction breakpoint' || this.stoppedDetails?.reason === 'step' && this.lastSteppingGranularity === 'instruction') && sf.instructionPointerReference || sf.source && sf.source.available && sf.source.presentationHint !== 'deemphasize'));
    return firstAvailableStackFrame;
  }
  get stateLabel() {
    if (this.stoppedDetails) {
      return this.stoppedDetails.description || (this.stoppedDetails.reason ? localizeWithPath(_moduleId, 3, "Paused on {0}", this.stoppedDetails.reason) : localizeWithPath(_moduleId, 4, "Paused"));
    }
    return localizeWithPath(_moduleId, 5, "Running");
  }
...
class Breakpoint extends BaseBreakpoint {
  ...
  get message() {
    if (this.textFileService.isDirty(this.uri)) {
      return localizeWithPath(_moduleId, 6, "Unverified breakpoint. File is modified, please restart debug session.");
    }
