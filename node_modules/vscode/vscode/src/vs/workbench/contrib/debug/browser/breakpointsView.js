...
} = require('../common/disassemblyViewInput.js');
const {
  SIDE_GROUP,
  ACTIVE_GROUP
...
let BreakpointsView = class BreakpointsView extends ViewPane {
  ...
  renderBody(container) {
    ...
    this.list.onMouseMiddleClick(async ({
      element
    }) => {
  ...
  onListContextMenu(e) {
    ...
    const secondary = [];
    createAndFillInContextMenuActions(this.menu, {
      arg: e.element,
...
let BreakpointsRenderer = class BreakpointsRenderer {
  ...
  renderElement(breakpoint, index, data) {
    ...
    const {
      message,
      icon
    } = getBreakpointMessageAndIcon(this.debugService.state, this.debugService.getModel().areBreakpointsActivated(), breakpoint, this.labelService, this.debugService.getModel());
...
BreakpointsRenderer = BreakpointsRenderer_1 = __decorate([__param(4, IDebugService), __param(5, IHoverService), __param(6, ILabelService)], BreakpointsRenderer);
...
let FunctionBreakpointsRenderer = class FunctionBreakpointsRenderer {
  ...
  renderElement(functionBreakpoint, _index, data) {
    ...
    const {
      icon,
      message
    } = getBreakpointMessageAndIcon(this.debugService.state, this.debugService.getModel().areBreakpointsActivated(), functionBreakpoint, this.labelService, this.debugService.getModel());
    data.icon.className = ThemeIcon.asClassName(icon);
    ...
    if (functionBreakpoint.condition && functionBreakpoint.hitCondition) {
      data.condition.textContent = localizeWithPath(_moduleId, 2, "Condition: {0} | Hit Count: {1}", functionBreakpoint.condition, functionBreakpoint.hitCondition);
    } else {
...
let DataBreakpointsRenderer = class DataBreakpointsRenderer {
  ...
  renderElement(dataBreakpoint, _index, data) {
    ...
    const {
      icon,
      message
    } = getBreakpointMessageAndIcon(this.debugService.state, this.debugService.getModel().areBreakpointsActivated(), dataBreakpoint, this.labelService, this.debugService.getModel());
    data.icon.className = ThemeIcon.asClassName(icon);
    data.toDispose.push(this.hoverService.setupUpdatableHover(getDefaultHoverDelegate('mouse'), data.icon, message ? message : ''));
    ...
    if (dataBreakpoint.condition && dataBreakpoint.hitCondition) {
      data.condition.textContent = localizeWithPath(_moduleId, 2, "Condition: {0} | Hit Count: {1}", dataBreakpoint.condition, dataBreakpoint.hitCondition);
    } else {
...
class FunctionBreakpointInputRenderer {
  ...
  renderElement(functionBreakpoint, _index, data) {
    ...
    if (data.type === 'condition') {
      ...
      ariaLabel = localizeWithPath(_moduleId, 11, "Type expression. Function breakpoint will break when expression evaluates to true");
      ...
      ariaLabel = localizeWithPath(_moduleId, 13, "Type hit count. Function breakpoint will break when hit count is met.");
    }
...
class DataBreakpointInputRenderer {
  ...
  renderElement(dataBreakpoint, _index, data) {
    ...
    if (data.type === 'condition') {
      ...
      ariaLabel = localizeWithPath(_moduleId, 15, "Type expression. Data breakpoint will break when expression evaluates to true");
    } else if (data.type === 'hitCount') {
      data.inputBox.value = dataBreakpoint.hitCondition || '';
      placeholder = localizeWithPath(_moduleId, 16, "Break when hit count is met");
      ariaLabel = localizeWithPath(_moduleId, 17, "Type hit count. Data breakpoint will break when hit count is met.");
    }
    data.inputBox.setAriaLabel(ariaLabel);
    data.inputBox.setPlaceHolder(placeholder);
    setTimeout(() => {
...
class BreakpointsAccessibilityProvider {
  ...
  getAriaLabel(element) {
    ...
    const {
      message
    } = getBreakpointMessageAndIcon(this.debugService.state, this.debugService.getModel().areBreakpointsActivated(), element, this.labelService, this.debugService.getModel());
...
function getBreakpointMessageAndIcon(state, breakpointsActivated, breakpoint$1, labelService, debugModel) {
  ...
  if (breakpoint$1 instanceof InstructionBreakpoint) {
    ...
    if (breakpoint$1.message) {
      ...
      messages.push(localizeWithPath(_moduleId, 32, "Instruction breakpoint at address {0}", breakpoint$1.instructionReference));
    } else {
  ...
  if (breakpoint$1.logMessage || breakpoint$1.condition || breakpoint$1.hitCondition || triggeringBreakpoint) {
    ...
    if (!breakpoint$1.supported) {
      ...
      messages.push(localizeWithPath(_moduleId, 34, "Breakpoints of this type are not supported by the debugger"));
    }
    ...
    if (triggeringBreakpoint) {
      messages.push(localizeWithPath(_moduleId, 36, "Hit after breakpoint: {0}", `${labelService.getUriLabel(triggeringBreakpoint.uri, {
        relative: true
...
class MemoryBreakpointAction extends Action2 {
  async run(accessor, existingBreakpoint) {
    ...
    try {
      ...
      notifications.error(localizeWithPath(_moduleId, 40, "Failed to set data breakpoint at {0}: {1}", range.address, e.message));
    }
    if (!info?.dataId) {
      return;
    }
    ...
    if (info.accessTypes && info.accessTypes?.length > 1) {
      ...
      const selectedAccessType = await quickInput.pick(accessTypes, {
        placeHolder: localizeWithPath(_moduleId, 41, "Select the access type to monitor")
      });
  ...
  getRange(quickInput, defaultValue) {
    return new Promise(resolve => {
      ...
      input.placeholder = localizeWithPath(_moduleId, 43, 'Absolute range (0x1234 - 0x1300) or range of bytes after an address (0x1234 + 0xff)');
      if (defaultValue) {
  ...
  parseAddress(range, isFinal) {
    ...
    if (!parts) {
      return {
        error: localizeWithPath(_moduleId, 44, 'Address should be a range of numbers the form "[Start] - [End]" or "[Start] + [Bytes]"')
      };
    }
    ...
    for (const n of [startStr, endStr]) {
      if (!isNum(n)) {
        return {
          error: localizeWithPath(_moduleId, 45, 'Number must be a decimal integer or hex value starting with \"0x\", got {0}', n)
        };
      }
    }
    if (!isFinal) {
...
registerAction2(class extends ViewAction {
  ...
  async runInView(accessor, view, breakpoint) {
    ...
    if (breakpoint instanceof Breakpoint) {
      ...
      const actions = [new Action('breakpoint.editCondition', localizeWithPath(_moduleId, 58, "Edit Condition..."), undefined, true, async () => view.renderInputBox({
        breakpoint,
        type: 'condition'
      })), new Action('breakpoint.editCondition', localizeWithPath(_moduleId, 59, "Edit Hit Count..."), undefined, true, async () => view.renderInputBox({
        breakpoint,
        type: 'hitCount'
      }))];
