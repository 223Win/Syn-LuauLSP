...
const {
  SelectBox
} = require('../../../../base/browser/ui/selectBox/selectBox.js');
const {
  onUnexpectedError
...
const {} = require('./media/breakpointWidget.css.js');
const {
  EditorCommand,
...
const {
  ZoneWidget
} = require('../../../../editor/contrib/zoneWidget/browser/zoneWidget.js');
const {
  localizeWithPath
} = require('../../../../nls.js');
const {
...
} = require('./breakpointWidget.service.js');
const _moduleId = "vs/workbench/contrib/debug/browser/breakpointWidget";
const $ = $$1;
...
let BreakpointWidget = class BreakpointWidget extends ZoneWidget {
  ...
  get placeholder() {
    ...
    switch (this.context) {
      case 2:
        return localizeWithPath(_moduleId, 0, "Message to log when breakpoint is hit. Expressions within {} are interpolated. '{0}' to accept, '{1}' to cancel.", acceptString, closeString);
      case 1:
        return localizeWithPath(_moduleId, 1, "Break when hit count condition is met. '{0}' to accept, '{1}' to cancel.", acceptString, closeString);
      default:
        return localizeWithPath(_moduleId, 2, "Break when expression evaluates to true. '{0}' to accept, '{1}' to cancel.", acceptString, closeString);
    }
  }
  ...
  _fillContainer(container) {
    ...
    const selectBox = new SelectBox([{
      text: localizeWithPath(_moduleId, 3, "Expression")
    }, {
      text: localizeWithPath(_moduleId, 4, "Hit Count")
    }, {
      text: localizeWithPath(_moduleId, 5, "Log Message")
    }, {
      text: localizeWithPath(_moduleId, 6, "Wait for Breakpoint")
    }], this.context, this.contextViewService, defaultSelectBoxStyles, {
      ariaLabel: localizeWithPath(_moduleId, 7, 'Breakpoint Type')
    });
  ...
  createModesInput(container) {
    ...
    const sb = this.selectModeBox = new SelectBox([{
      text: localizeWithPath(_moduleId, 8, 'Mode'),
      isDisabled: true
    }, ...modes.map(mode => ({
      text: mode.label,
  ...
  createTriggerBreakpointInput(container) {
    ...
    const breakpointOptions = [{
      text: localizeWithPath(_moduleId, 9, 'None'),
      isDisabled: true
    }, ...breakpoints.map(bp => ({
      text: `${this.labelService.getUriLabel(bp.uri, {
        relative: true
      })}: ${bp.lineNumber}`,
      description: localizeWithPath(_moduleId, 10, 'Loading...')
    }))];
    ...
    for (const [i, bp] of breakpoints.entries()) {
      this.textModelService.createModelReference(bp.uri).then(ref => {
        ...
        breakpointOptions[i + 1].description = localizeWithPath(_moduleId, 11, 'Could not load source.');
      });
    }
    const selectBreakpointBox = this.selectBreakpointBox = new SelectBox(breakpointOptions, index + 1, this.contextViewService, defaultSelectBoxStyles, {
      ariaLabel: localizeWithPath(_moduleId, 12, 'Select breakpoint')
    });
    ...
    closeButton.label = localizeWithPath(_moduleId, 13, "Ok");
  ...
  createBreakpointInput(container) {
    ...
    this.toDispose.push(this.languageFeaturesService.completionProvider.register({
      scheme: DEBUG_SCHEME,
...
BreakpointWidget = __decorate([__param(4, IContextViewService), __param(5, IDebugService), __param(6, IThemeService), __param(7, IContextKeyService), __param(8, IInstantiationService), __param(9, IModelService), __param(10, ICodeEditorService), __param(11, IConfigurationService), __param(12, ILanguageFeaturesService), __param(13, IKeybindingService), __param(14, ILabelService), __param(15, ITextModelService)], BreakpointWidget);
