...
const {
  UndoRedoGroup
} = require('../../../../../platform/undoRedo/common/undoRedo.js');
const {
  IUndoRedoService
...
} = require('./modifiedBaseRange.js');
const _moduleId = "vs/workbench/contrib/mergeEditor/browser/model/mergeEditorModel";
let MergeEditorModel = class MergeEditorModel extends EditorModel {
  constructor(base, input1, input2, resultTextModel, diffComputer, options, telemetry, languageService, undoRedoService) {
    ...
    this.baseResultMapping = derived(this, reader => {
      ...
      return new DocumentLineRangeMap(map.lineRangeMappings.map(m => m.inputRange.isEmpty || m.outputRange.isEmpty ? new LineRangeMapping(m.inputRange.deltaStart(-1), m.outputRange.deltaStart(-1)) : m), map.inputLineCount);
    });
    ...
    this.diffComputingState = derived(this, reader => {
      const states = [this.input1TextModelDiffs, this.input2TextModelDiffs, this.resultTextModelDiffs].map(s => s.state.read(reader));
  ...
  computeState(baseRange, conflictingDiffs) {
    ...
    const states = [ModifiedBaseRangeState.base.withInputValue(1, true).withInputValue(2, true, true), ModifiedBaseRangeState.base.withInputValue(2, true).withInputValue(1, true, true), ModifiedBaseRangeState.base.withInputValue(1, true).withInputValue(2, true, false), ModifiedBaseRangeState.base.withInputValue(2, true).withInputValue(1, true, false)];
  ...
  setInputHandled(baseRange, inputNumber, handled, tx) {
    ...
    this.undoRedoService.pushElement({
      ...
      label: localizeWithPath(_moduleId, 0, "Set Input Handled"),
      redo() {
...
class MarkAsHandledUndoRedoElement {
  constructor(resource, mergeEditorModelRef, stateRef, input1Handled, input2Handled) {
    ...
    this.label = localizeWithPath(_moduleId, 1, 'Undo Mark As Handled');
    this.type = 0;
  }
  redo() {
