...
class MappingAlignment {
  static compute(fromInputToOutput1, fromInputToOutput2) {
    ...
    const combinedDiffs = concatArrays(fromInputToOutput1.map(diff => ({
      source: 0,
      diff
    })), fromInputToOutput2.map(diff => ({
      source: 1,
      diff
...
class DocumentRangeMap {
  constructor(rangeMappings, inputLineCount) {
    ...
    this.inputLineCount = inputLineCount;
    assertFn(() => checkAdjacentItems(rangeMappings, (m1, m2) => rangeIsBeforeOrTouching(m1.inputRange, m2.inputRange) && rangeIsBeforeOrTouching(m1.outputRange, m2.outputRange)));
  }
  project(position) {
