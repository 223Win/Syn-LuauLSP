...
class SnippetCompletion {
  constructor(snippet, range) {
    ...
    this.detail = localizeWithPath(_moduleId, 0, "{0} ({1})", snippet.description || snippet.name, snippet.source);
  ...
  async provideCompletionItems(model, position, context) {
    ...
    for (const snippet of snippets) {
      ...
      if (columnOffset < lineContentLow.length) {
        ...
        if (standardAutoClosingPairConditionals?.some(p => p.open === lineContentLow[startPosition.column - 1] && snippet.prefix.startsWith(p.open) && snippet.prefix[snippet.prefix.length - 1] === p.close)) {
          endColumn++;
        }
      }
  ...
  _disambiguateSnippets(suggestions) {
    ...
    for (let i = 0; i < suggestions.length; i++) {
      ...
      for (; to < suggestions.length && item.label === suggestions[to].label; to++) {
        suggestions[to].label.label = localizeWithPath(_moduleId, 1, "{0}, {1}", suggestions[to].label.label, suggestions[to].snippet.name);
      }
      if (to > i + 1) {
        suggestions[i].label.label = localizeWithPath(_moduleId, 1, "{0}, {1}", suggestions[i].label.label, suggestions[i].snippet.name);
        i = to;
      }
    }
  }
