...
const {
  SnippetFile
} = require('./snippetsFile.js');
const {
  ExtensionsRegistry
} = require('../../../services/extensions/common/extensionsRegistry.js');
const {
  languagesExtPoint
} = require('../../../services/language/common/languageService.js');
const {
  SnippetCompletionProvider
} = require('./snippetCompletionProvider.js');
const {
  IExtensionResourceLoaderService
} = require('../../../../platform/extensionResourceLoader/common/extensionResourceLoader.service.js');
const {
  ResourceMap
...
} = require('../../../services/userDataProfile/common/userDataProfile.service.js');
const {
  insertInto
} = require('../../../../base/common/arrays.js');
var SnippetEnablement_1, SnippetUsageTimestamps_1;
const _moduleId = "vs/workbench/contrib/snippets/browser/snippetsService";
var snippetExt;
(function (snippetExt) {
  function toValidSnippet(extension, snippet, languageService) {
    if (isFalsyOrWhitespace(snippet.path)) {
      extension.collector.error(localizeWithPath(_moduleId, 0, "Expected string in `contributes.{0}.path`. Provided value: {1}", extension.description.name, String(snippet.path)));
      return null;
    }
    if (isFalsyOrWhitespace(snippet.language) && !snippet.path.endsWith('.code-snippets')) {
      extension.collector.error(localizeWithPath(_moduleId, 1, "When omitting the language, the value of `contributes.{0}.path` must be a `.code-snippets`-file. Provided value: {1}", extension.description.name, String(snippet.path)));
      return null;
    }
    if (!isFalsyOrWhitespace(snippet.language) && !languageService.isRegisteredLanguageId(snippet.language)) {
      extension.collector.error(localizeWithPath(_moduleId, 2, "Unknown language in `contributes.{0}.language`. Provided value: {1}", extension.description.name, String(snippet.language)));
      return null;
    }
    ...
    if (!isEqualOrParent(snippetLocation, extensionLocation)) {
      extension.collector.error(localizeWithPath(_moduleId, 3, "Expected `contributes.{0}.path` ({1}) to be included inside extension's folder ({2}). This might make the extension non-portable.", extension.description.name, snippetLocation.path, extensionLocation.path));
      return null;
    }
    return {
  ...
  snippetExt.snippetsContribution = {
    description: localizeWithPath(_moduleId, 4, 'Contributes snippets.'),
    type: 'array',
    defaultSnippets: [{
      body: [{
        language: '',
    ...
    items: {
      ...
      properties: {
        language: {
          description: localizeWithPath(_moduleId, 5, 'Language identifier for which this snippet is contributed to.'),
          type: 'string'
        },
        path: {
          description: localizeWithPath(_moduleId, 6, 'Path of the snippets file. The path is relative to the extension folder and typically starts with \'./snippets/\'.'),
          type: 'string'
        }
      }
    }
    ...
    if (languageId) {
      if (this._languageService.isRegisteredLanguageId(languageId)) {
        for (const file of this._files.values()) {
          promises.push(file.load().then(file => file.select(languageId, result)).catch(err => this._logService.error(err, file.location.toString())));
        }
      }
    } else {
      for (const file of this._files.values()) {
        promises.push(file.load().then(file => insertInto(result, result.length, file.data)).catch(err => this._logService.error(err, file.location.toString())));
      }
    }
      ...
      for (const extension of extensions) {
        for (const contribution of extension.value) {
          ...
          if (file) {
            ...
            const file = new SnippetFile(3, validContribution.location, validContribution.language ? [validContribution.language] : undefined, extension.description, this._fileService, this._extensionResourceLoaderService);
            this._files.set(file.location, file);
            if (this._environmentService.isExtensionDevelopment) {
              file.load().then(file => {
                if (file.data.some(snippet => snippet.isBogous)) {
                  extension.collector.warn(localizeWithPath(_moduleId, 7, "One or more snippets from the extension '{0}' very likely confuse snippet-variables and snippet-placeholders (see https://code.visualstudio.com/docs/editor/userdefinedsnippets#_snippet-syntax for more details)", extension.description.name));
                }
              }, err => {
                extension.collector.warn(localizeWithPath(_moduleId, 8, "The snippet file \"{0}\" could not be read.", file.location.toString()));
              });
            }
          }
        }
