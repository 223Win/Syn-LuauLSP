...
const {} = require('./media/tunnelView.css.js');
const {
  localize2WithPath,
  localizeWithPath
} = require('../../../../nls.js');
...
const {
  ...
  TunnelType,
  TunnelEditId
} = require('../../../services/remote/common/remoteExplorerService.js');
const {
  IRemoteExplorerService
} = require('../../../services/remote/common/remoteExplorerService.service.js');
const {
  IClipboardService
...
const {
  ...
  isLocalhost,
  isAllInterfaces
} = require('../../../../platform/tunnel/common/tunnel.js');
const {
  ITunnelService
} = require('../../../../platform/tunnel/common/tunnel.service.js');
const {
  SyncDescriptor
...
const {
  ...
  portsViewIcon
} = require('./remoteIcons.js');
const {
  IExternalUriOpenerService
} = require('../../externalUriOpener/common/externalUriOpenerService.service.js');
const {
  CancellationToken
...
const {
  ...
  TunnelSource
} = require('../../../services/remote/common/tunnelModel.js');
const {
...
var TunnelPanel_1;
const _moduleId = "vs/workbench/contrib/remote/browser/tunnelView";
...
let TunnelViewModel = class TunnelViewModel {
  constructor(remoteExplorerService, tunnelService) {
    ...
    this.input = {
      label: localizeWithPath(_moduleId, 0, "Add Port"),
      icon: undefined,
      ...
      privacy: {
        ...
        label: localizeWithPath(_moduleId, 1, "Private")
      },
      strip: () => undefined
    };
...
class PortColumn {
  constructor() {
    this.label = localizeWithPath(_moduleId, 2, "Port");
    this.tooltip = localizeWithPath(_moduleId, 3, "The label and remote port number of the forwarded port.");
    this.weight = 1;
    this.templateId = 'actionbar';
  }
  project(row) {
...
class LocalAddressColumn {
  constructor() {
    this.label = localizeWithPath(_moduleId, 4, "Forwarded Address");
    this.tooltip = localizeWithPath(_moduleId, 5, "The address that the forwarded port is available at.");
    this.weight = 1;
    this.templateId = 'actionbar';
  }
  project(row) {
  ...
  static getHoverText(localAddress) {
    return function (configurationService) {
      ...
      if (editorConf.multiCursorModifier === 'ctrlCmd') {
        if (isMacintosh) {
          clickLabel = localizeWithPath(_moduleId, 6, "option + click");
        } else {
          clickLabel = localizeWithPath(_moduleId, 7, "alt + click");
        }
      } else {
        if (isMacintosh) {
          clickLabel = localizeWithPath(_moduleId, 8, "cmd + click");
        } else {
          clickLabel = localizeWithPath(_moduleId, 9, "ctrl + click");
        }
      }
...
class RunningProcessColumn {
  constructor() {
    this.label = localizeWithPath(_moduleId, 10, "Running Process");
    this.tooltip = localizeWithPath(_moduleId, 11, "The command line of the process that is using the port.");
    this.weight = 2;
    this.templateId = 'actionbar';
  }
  project(row) {
...
class OriginColumn {
  constructor() {
    this.label = localizeWithPath(_moduleId, 12, "Origin");
    this.tooltip = localizeWithPath(_moduleId, 13, "The source that a forwarded port originates from. Can be an extension, user forwarded, statically forwarded, or automatically forwarded.");
    this.weight = 1;
    this.templateId = 'actionbar';
  }
  project(row) {
...
class PrivacyColumn {
  constructor() {
    this.label = localizeWithPath(_moduleId, 14, "Visibility");
    this.tooltip = localizeWithPath(_moduleId, 15, "The availability of the forwarded port.");
    this.weight = 1;
    this.templateId = 'actionbar';
  }
  project(row) {
...
let ActionBarRenderer = class ActionBarRenderer extends Disposable {
  ...
  renderActionBarItem(element, templateData) {
    ...
    const context = [['view', TUNNEL_VIEW_ID], [TunnelTypeContextKey.key, element.tunnel.tunnelType], [TunnelCloseableContextKey.key, element.tunnel.closeable], [TunnelPrivacyContextKey.key, element.tunnel.privacy.id], [TunnelProtocolContextKey.key, element.tunnel.protocol]];
  ...
  renderInputBox(container, editableData) {
    ...
    const inputBox = new InputBox(container, this.contextViewService, {
      ariaLabel: localizeWithPath(_moduleId, 16, "Press Enter to confirm or Escape to cancel."),
      validationOptions: {
        validation: value => {
          ...
          return {
            ...
            formatContent: true,
            type: message.severity === Severity$1.Error ? 3 : 1
          };
        }
      },
    ...
    this.inputDone = done;
    const toDispose = [inputBox, addStandardDisposableListener(inputBox.inputElement, EventType.KEY_DOWN, async e => {
      if (e.equals(3)) {
        e.stopPropagation();
...
class TunnelItem {
  ...
  get processDescription() {
    ...
    if (this.runningProcess) {
      ...
      description = localizeWithPath(_moduleId, 17, "Process information unavailable");
    }
    return description;
  }
  get tooltipPostfix() {
    ...
    if (this.localAddress) {
      information = localizeWithPath(_moduleId, 18, "Remote port {0}:{1} forwarded to local address {2}. ", this.remoteHost, this.remotePort, this.localAddress);
    } else {
      information = localizeWithPath(_moduleId, 19, "Remote port {0}:{1} not forwarded. ", this.remoteHost, this.remotePort);
    }
    return information;
  }
  get iconTooltip() {
    ...
    if (!isAdd) {
      return `${this.processDescription ? localizeWithPath(_moduleId, 20, "Port has running process.") : localizeWithPath(_moduleId, 21, "No running process.")}`;
    } else {
      return this.label;
    }
  }
  get portTooltip() {
    const isAdd = this.tunnelType === TunnelType.Add;
    if (!isAdd) {
      return `${this.name ? localizeWithPath(_moduleId, 22, "Port labeled {0}. ", this.name) : ''}`;
    } else {
      return '';
    }
  }
  ...
  get privacy() {
    if (this.tunnelService?.privacyOptions) {
      return this.tunnelService?.privacyOptions.find(element => element.id === this._privacy) ?? {
        ...
        label: localizeWithPath(_moduleId, 23, "Unknown")
      };
    } else {
      return {
        id: TunnelPrivacyId.Private,
...
const TunnelViewFocusContextKey = new RawContextKey('tunnelViewFocus', false, localizeWithPath(_moduleId, 24, "Whether the Ports view has focus."));
...
let TunnelPanel = class TunnelPanel extends ViewPane {
  ...
  static {
    this.TITLE = localize2WithPath(_moduleId, 25, "Ports");
  }
  ...
  createTable() {
    ...
    this.table = this.instantiationService.createInstance(WorkbenchTable, 'RemoteTunnels', widgetContainer, new TunnelTreeVirtualDelegate(this.remoteExplorerService), columns, [actionBarRenderer], {
      ...
      accessibilityProvider: {
        ...
        getWidgetAriaLabel: () => localizeWithPath(_moduleId, 26, "Tunnel View")
      },
...
TunnelPanel = TunnelPanel_1 = __decorate([__param(2, IKeybindingService), __param(3, IContextMenuService), __param(4, IContextKeyService), __param(5, IConfigurationService), __param(6, IInstantiationService), __param(7, IViewDescriptorService), __param(8, IOpenerService), __param(9, IQuickInputService), __param(10, ICommandService), __param(11, IMenuService), __param(12, IThemeService), __param(13, IRemoteExplorerService), __param(14, ITelemetryService), __param(15, IHoverService), __param(16, ITunnelService), __param(17, IContextViewService)], TunnelPanel);
...
(function (LabelTunnelAction) {
  ...
  LabelTunnelAction.LABEL = localizeWithPath(_moduleId, 27, "Set Port Label");
      ...
      if (tunnelContext) {
        ...
        return new Promise(resolve => {
          ...
          remoteExplorerService.setEditable(tunnelItem, TunnelEditId.Label, {
            ...
            placeholder: localizeWithPath(_moduleId, 28, "Port label"),
            startingValue
          });
        });
      }
...
const invalidPortString = localizeWithPath(_moduleId, 29, "Forwarded port should be a number or a host:port.");
const maxPortNumber = 65536;
const invalidPortNumberString = localizeWithPath(_moduleId, 30, "Port number must be \u2265 0 and < {0}.", maxPortNumber);
const requiresSudoString = localizeWithPath(_moduleId, 31, "May Require Sudo");
const alreadyForwarded = localizeWithPath(_moduleId, 32, "Port is already forwarded");
...
(function (ForwardPortAction) {
  ...
  ForwardPortAction.LABEL = localize2WithPath(_moduleId, 33, "Forward a Port");
  ForwardPortAction.TREEITEM_LABEL = localizeWithPath(_moduleId, 34, "Forward Port");
  const forwardPrompt = localizeWithPath(_moduleId, 35, "Port number or address (eg. 3000 or 10.10.10.10:2000).");
  ...
  function error(notificationService, tunnelOrError, host, port) {
    if (!tunnelOrError) {
      notificationService.warn(localizeWithPath(_moduleId, 36, "Unable to forward {0}:{1}. The host may not be available or that remote port may already be forwarded", host, port));
    } else if (typeof tunnelOrError === 'string') {
      notificationService.warn(localizeWithPath(_moduleId, 37, "Unable to forward {0}:{1}. {2}", host, port, tunnelOrError));
    }
  }
...
function makeTunnelPicks(tunnels, remoteExplorerService, tunnelService) {
  ...
  if (picks.length === 0) {
    picks.push({
      label: localizeWithPath(_moduleId, 38, "No ports currently forwarded. Try running the {0} command", ForwardPortAction.LABEL.value)
    });
  }
  return picks;
}
...
(function (ClosePortAction) {
  ...
  ClosePortAction.LABEL = localize2WithPath(_moduleId, 39, "Stop Forwarding Port");
  function inlineHandler() {
    return async (accessor, arg) => {
  ...
  function commandPaletteHandler() {
    return async accessor => {
      ...
      const result = await quickInputService.pick(picks, {
        placeHolder: localizeWithPath(_moduleId, 40, "Choose a port to stop forwarding")
      });
...
(function (OpenPortInBrowserAction) {
  ...
  OpenPortInBrowserAction.LABEL = localizeWithPath(_moduleId, 41, "Open in Browser");
  function handler() {
    return async (accessor, arg) => {
      let key;
      if (isITunnelItem(arg)) {
...
(function (OpenPortInPreviewAction) {
  ...
  OpenPortInPreviewAction.LABEL = localizeWithPath(_moduleId, 42, "Preview in Editor");
  function handler() {
    return async (accessor, arg) => {
      let key;
      if (isITunnelItem(arg)) {
...
(function (OpenPortInBrowserCommandPaletteAction) {
  ...
  OpenPortInBrowserCommandPaletteAction.LABEL = localizeWithPath(_moduleId, 43, "Open Port in Browser");
  function handler() {
    return async (accessor, arg) => {
      const remoteExplorerService = accessor.get(IRemoteExplorerService);
      const tunnelService = accessor.get(ITunnelService);
      ...
      if (options.length === 0) {
        options.push({
          label: localizeWithPath(_moduleId, 44, "No ports currently forwarded. Open the Ports view to get started.")
        });
      } else {
        options.push({
          label: localizeWithPath(_moduleId, 45, "Open the Ports view...")
        });
      }
      const picked = await quickPickService.pick(options, {
        placeHolder: localizeWithPath(_moduleId, 46, "Choose the port to open")
      });
...
(function (CopyAddressAction) {
  ...
  CopyAddressAction.INLINE_LABEL = localizeWithPath(_moduleId, 47, "Copy Local Address");
  CopyAddressAction.COMMANDPALETTE_LABEL = localizeWithPath(_moduleId, 48, "Copy Forwarded Port Address");
  ...
  function commandPaletteHandler() {
    return async (accessor, arg) => {
      ...
      const result = await quickInputService.pick(makeTunnelPicks(tunnels, remoteExplorerService, tunnelService), {
        placeHolder: localizeWithPath(_moduleId, 49, "Choose a forwarded port")
      });
      if (result && result.tunnel) {
...
(function (ChangeLocalPortAction) {
  ...
  ChangeLocalPortAction.LABEL = localizeWithPath(_moduleId, 50, "Change Local Address Port");
  function validateInput(tunnelService, value, canElevate) {
    if (!value.match(/^[0-9]+$/)) {
      return {
        content: localizeWithPath(_moduleId, 51, "Local port should be a number."),
        severity: Severity$1.Error
      };
          ...
          onFinish: async (value, success) => {
            ...
            if (success) {
              ...
              if (newForward && typeof newForward !== 'string' && newForward.tunnelLocalPort !== numberValue) {
                notificationService.warn(localizeWithPath(_moduleId, 52, "The local port {0} is not available. Port number {1} has been used instead", value, newForward.tunnelLocalPort ?? newForward.localAddress));
              }
            }
          },
          validationMessage: value => validateInput(tunnelService, value, tunnelService.canElevate),
          placeholder: localizeWithPath(_moduleId, 53, "New local port")
        });
      }
    };
  }
...
(function (SetTunnelProtocolAction) {
  ...
  SetTunnelProtocolAction.LABEL_HTTP = localizeWithPath(_moduleId, 54, "HTTP");
  SetTunnelProtocolAction.LABEL_HTTPS = localizeWithPath(_moduleId, 55, "HTTPS");
...
MenuRegistry.appendMenuItem(MenuId.TunnelContext, {
  ...
  title: localizeWithPath(_moduleId, 56, "Port Visibility"),
...
MenuRegistry.appendMenuItem(MenuId.TunnelContext, {
  ...
  title: localizeWithPath(_moduleId, 57, "Change Port Protocol"),
...
}, localizeWithPath(_moduleId, 58, "The color of the icon for a port that has an associated running process."));
exports.ForwardPortAction = ForwardPortAction;
exports.OpenPortInBrowserAction = OpenPortInBrowserAction;
exports.OpenPortInPreviewAction = OpenPortInPreviewAction;
...
exports.openPreviewEnabledContext = openPreviewEnabledContext;