...
const {
  ...
  TunnelViewModel
} = require('./tunnelView.js');
const {
  IContextKeyService
} = require('../../../../platform/contextkey/common/contextkey.service.js');
const {
  IWorkbenchEnvironmentService
} = require('../../../services/environment/common/environmentService.service.js');
const {
  Registry
...
const {
  ITerminalService
} = require('../../terminal/browser/terminal.service.js');
const {
  IDebugService
} = require('../../debug/common/debug.service.js');
const {
  IRemoteAgentService
...
} = require('../../../services/configuration/common/configuration.service.js');
const {
  Action
...
let ForwardedPortsView = class ForwardedPortsView extends Disposable {
  constructor(contextKeyService, environmentService, remoteExplorerService, tunnelService, activityService, statusbarService) {
    ...
    this._register(Registry.as(Extensions.ViewsRegistry).registerViewWelcomeContent(TUNNEL_VIEW_ID, {
      content: this.environmentService.remoteAuthority ? localizeWithPath(_moduleId, 0, "No forwarded ports. Forward a port to access your running services locally.\n[Forward a Port]({0})", `command:${ForwardPortAction.INLINE_ID}`) : localizeWithPath(_moduleId, 1, "No forwarded ports. Forward a port to access your locally running services over the internet.\n[Forward a Port]({0})", `command:${ForwardPortAction.INLINE_ID}`)
    }));
  ...
  async updateActivityBadge() {
    if (this.remoteExplorerService.tunnelModel.forwarded.size > 0) {
      this.activityBadge.value = this.activityService.showViewActivity(TUNNEL_VIEW_ID, {
        badge: new NumberBadge(this.remoteExplorerService.tunnelModel.forwarded.size, n => n === 1 ? localizeWithPath(_moduleId, 3, "1 forwarded port") : localizeWithPath(_moduleId, 4, "{0} forwarded ports", n))
      });
    }
  }
  updateStatusBar() {
  ...
  get entry() {
    ...
    if (count === 0) {
      ...
      tooltip = localizeWithPath(_moduleId, 6, "Forwarded Ports: {0}", allTunnels.map(forwarded => forwarded.remotePort).join(', '));
    }
    return {
      ...
      this.portListener = this._register(this.remoteExplorerService.tunnelModel.onForwardPort(async () => {
        ...
        if (Array.from(this.remoteExplorerService.tunnelModel.forwarded.values()).filter(tunnel => tunnel.source.source === TunnelSource.Auto).length > fallbackAt) {
          ...
          this.notificationService.notify({
            message: localizeWithPath(_moduleId, 8, "Over 20 ports have been automatically forwarded. The `process` based automatic port forwarding has been switched to `hybrid` in settings. Some ports may no longer be detected."),
            severity: Severity$1.Warning,
            actions: {
              primary: [new Action('switchBack', localizeWithPath(_moduleId, 9, "Undo"), undefined, true, async () => {
...
class OnAutoForwardedAction extends Disposable {
  ...
  async basicMessage(tunnel) {
    ...
    return localizeWithPath(_moduleId, 11, "Your application{0} running on port {1} is available.  ", label ? ` (${label})` : '', tunnel.tunnelRemotePort);
  }
  linkMessage() {
    return localizeWithPath(_moduleId, 12, "[See all forwarded ports]({0})", `command:${TunnelPanel.ID}.focus`);
  }
  async showNotification(tunnel) {
    ...
    if (tunnel.tunnelLocalPort !== tunnel.tunnelRemotePort && this.tunnelService.canElevate && this.tunnelService.isPortPrivileged(tunnel.tunnelRemotePort)) {
      message += localizeWithPath(_moduleId, 13, "You'll need to run as superuser to use port {0} locally.  ", tunnel.tunnelRemotePort);
  ...
  elevateChoice(tunnel) {
    return {
      ...
      run: async () => {
        ...
        this.lastNotification = this.notificationService.prompt(Severity$1.Info, (await this.basicMessage(newTunnel)) + this.linkMessage(), [this.openBrowserChoice(newTunnel), this.openPreviewChoice(tunnel)], {
          neverShowAgain: {
            id: 'remote.tunnelsView.autoForwardNeverShow',
            isSecondary: true
          }
