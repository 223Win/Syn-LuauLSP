...
} = require('../../../../editor/contrib/codeAction/browser/codeAction.js');
const {
  CodeActionKind
...
const _moduleId = "vs/workbench/contrib/codeActions/browser/codeActionsContribution";
const createCodeActionsAutoSave = description => {
  return {
    type: 'string',
    enum: ['always', 'explicit', 'never', true, false],
    enumDescriptions: [localizeWithPath(_moduleId, 0, 'Triggers Code Actions on explicit saves and auto saves triggered by window or focus changes.'), localizeWithPath(_moduleId, 1, 'Triggers Code Actions only when explicitly saved'), localizeWithPath(_moduleId, 2, 'Never triggers Code Actions on save'), localizeWithPath(_moduleId, 3, 'Triggers Code Actions only when explicitly saved. This value will be deprecated in favor of "explicit".'), localizeWithPath(_moduleId, 4, 'Never triggers Code Actions on save. This value will be deprecated in favor of "never".')],
    default: 'explicit',
    description: description
  };
};
const codeActionsOnSaveDefaultProperties = Object.freeze({
  'source.fixAll': createCodeActionsAutoSave(localizeWithPath(_moduleId, 5, "Controls whether auto fix action should be run on file save."))
});
const codeActionsOnSaveSchema = {
  ...
  markdownDescription: localizeWithPath(_moduleId, 6, 'Run Code Actions for the editor on save. Code Actions must be specified and the editor must not be shutting down. Example: `"source.organizeImports": "explicit" `'),
  type: ['object', 'array'],
  additionalProperties: {
    type: 'string',
    enum: ['always', 'explicit', 'never', true, false]
...
let CodeActionsContribution = class CodeActionsContribution extends Disposable {
  ...
  updateConfigurationSchema(codeActionContributions) {
    ...
    for (const [sourceAction, props] of this.getSourceActions(codeActionContributions)) {
      newProperties[sourceAction] = createCodeActionsAutoSave(localizeWithPath(_moduleId, 7, "Controls whether '{0}' actions should be run on file save.", props.title));
    }
  ...
  getSchemaAdditions() {
    ...
    return [conditionalSchema(codeActionCommandId, getActions(HierarchicalKind.Empty)), conditionalSchema(refactorCommandId, getActions(CodeActionKind.Refactor)), conditionalSchema(sourceActionCommandId, getActions(CodeActionKind.Source))];
  }
};
CodeActionsContribution = __decorate([__param(1, IKeybindingService)], CodeActionsContribution);
