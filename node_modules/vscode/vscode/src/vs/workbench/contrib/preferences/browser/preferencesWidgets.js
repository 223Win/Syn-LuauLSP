...
const _moduleId = "vs/workbench/contrib/preferences/browser/preferencesWidgets";
...
let SettingsTargetsWidget = class SettingsTargetsWidget extends Widget {
  ...
  resetLabels() {
    ...
    this.userLocalSettings.label = localizeWithPath(_moduleId, 0, "User");
    this.userRemoteSettings.label = localizeWithPath(_moduleId, 1, "Remote") + (hostLabel ? ` [${hostLabel}]` : '');
    this.workspaceSettings.label = localizeWithPath(_moduleId, 2, "Workspace");
    this.folderSettingsAction.label = localizeWithPath(_moduleId, 3, "Folder");
  }
  create(parent) {
    ...
    this.settingsSwitcherBar = this._register(new ActionBar(settingsTabsWidget, {
      ...
      ariaLabel: localizeWithPath(_moduleId, 4, "Settings Switcher"),
    ...
    this.userLocalSettings.tooltip = localizeWithPath(_moduleId, 0, "User");
    this.userRemoteSettings = new Action('userSettingsRemote', '', '.settings-tab', true, () => this.updateTarget(4));
    const remoteAuthority = this.environmentService.remoteAuthority;
    const hostLabel = remoteAuthority && this.labelService.getHostLabel(Schemas.vscodeRemote, remoteAuthority);
    this.userRemoteSettings.tooltip = localizeWithPath(_moduleId, 1, "Remote") + (hostLabel ? ` [${hostLabel}]` : '');
  ...
  setResultCount(settingsTarget, count) {
    if (settingsTarget === 5) {
      let label = localizeWithPath(_moduleId, 2, "Workspace");
      if (count) {
        label += ` (${count})`;
      }
      this.workspaceSettings.label = label;
    } else if (settingsTarget === 3) {
      let label = localizeWithPath(_moduleId, 0, "User");
      if (count) {
        label += ` (${count})`;
      }
  ...
  async update() {
    ...
    this.workspaceSettings.tooltip = localizeWithPath(_moduleId, 2, "Workspace");
  }
};
...
let SearchWidget = class SearchWidget extends Widget {
  ...
  createInputBox(parent) {
    ...
    const box = this._register(new ContextScopedHistoryInputBox(parent, this.contextViewService, {
      ...this.options,
      showHistoryHint
