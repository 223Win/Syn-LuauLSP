...
const {} = require('./media/keybindings.css.js');
const {
  localizeWithPath
} = require('../../../../nls.js');
const {
...
const _moduleId = "vs/workbench/contrib/preferences/browser/keybindingWidgets";
...
let DefineKeybindingWidget = class DefineKeybindingWidget extends Widget {
  ...
  constructor(parent, instantiationService) {
    ...
    const message = localizeWithPath(_moduleId, 0, "Press desired key combination and then press ENTER.");
  ...
  printExisting(numberOfExisting) {
    if (numberOfExisting > 0) {
      ...
      const text = numberOfExisting === 1 ? localizeWithPath(_moduleId, 1, "1 existing command has this keybinding", numberOfExisting) : localizeWithPath(_moduleId, 2, "{0} existing commands have this keybinding", numberOfExisting);
  ...
  onKeybinding(keybinding) {
    ...
    if (this._chords) {
      for (let i = 1; i < this._chords.length; i++) {
        this._outputNode.appendChild(document.createTextNode(localizeWithPath(_moduleId, 3, "chord to")));
