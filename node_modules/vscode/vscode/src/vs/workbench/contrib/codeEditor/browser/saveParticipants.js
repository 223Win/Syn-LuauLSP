...
} = require('../../../../editor/common/commands/trimTrailingWhitespaceCommand.js');
const {
  EditOperation
...
const {
  getModifiedRanges
} = require('../../format/browser/formatModified.js');
const {
  ITextFileService
} = require('../../../services/textfile/common/textfiles.service.js');
const _moduleId = "vs/workbench/contrib/codeEditor/browser/saveParticipants";
...
let FormatOnSaveParticipant = class FormatOnSaveParticipant {
  ...
  async participate(model, context, progress, token) {
    ...
    const nestedProgress = new Progress(provider => {
      progress.report({
        message: localizeWithPath(_moduleId, 0, "Running '{0}' Formatter ([configure]({1})).", provider.displayName || provider.extensionId && provider.extensionId.value || '???', 'command:workbench.action.openSettings?%5B%22editor.formatOnSave%22%5D')
      });
    });
...
let CodeActionOnSaveParticipant = class CodeActionOnSaveParticipant {
  ...
  async participate(model, context, progress, token) {
    ...
    progress.report({
      message: localizeWithPath(_moduleId, 1, "Quick Fixes")
    });
  ...
  async applyOnSaveActions(model, codeActionsOnSave, excludes, progress, token) {
    const getActionProgress = new class {
      ...
      _report() {
        progress.report({
          message: localizeWithPath(_moduleId, 2, "Getting code actions from '{0}' ([configure]({1})).", [...this._names].map(name => `'${name}'`).join(', '), 'command:workbench.action.openSettings?%5B%22editor.codeActionsOnSave%22%5D')
        });
      }
      report(provider) {
    ...
    for (const codeActionKind of codeActionsOnSave) {
      ...
      try {
        for (const action of actionsToRun.validActions) {
          progress.report({
            message: localizeWithPath(_moduleId, 3, "Applying code action '{0}'.", action.action.title)
          });
...
CodeActionOnSaveParticipant = __decorate([__param(0, IConfigurationService), __param(1, IInstantiationService), __param(2, ILanguageFeaturesService)], CodeActionOnSaveParticipant);
