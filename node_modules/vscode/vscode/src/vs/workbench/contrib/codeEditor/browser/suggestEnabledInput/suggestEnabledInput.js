...
const {} = require('./suggestEnabledInput.css.js');
const {
  append,
  $
...
} = require('../simpleEditorOptions.js');
const {
...
let SuggestEnabledInput = class SuggestEnabledInput extends Widget {
  constructor(id, parent, suggestionProvider, ariaLabel, resourceHandle, options, defaultInstantiationService, modelService, contextKeyService, languageFeaturesService, configurationService) {
    ...
    this.inputWidget = this._register(instantiationService.createInstance(CodeEditorWidget, this.stylingContainer, editorOptions, {
      contributions: EditorExtensionsRegistry.getSomeEditorContributions([SuggestController.ID, SnippetController2.ID, ContextMenuController.ID, MenuPreventer.ID, SelectionClipboardContributionID]),
      isSimpleWidget: true
    }));
    ...
    this._register(Event.chain(this.inputWidget.onKeyDown, $ => $.filter(e => e.keyCode === 3))(e => {
      e.preventDefault();
    }, this));
