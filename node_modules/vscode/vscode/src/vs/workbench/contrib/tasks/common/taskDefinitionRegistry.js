...
const taskDefinitionSchema = {
  ...
  properties: {
    type: {
      type: 'string',
      description: localizeWithPath(_moduleId, 0, 'The actual task type. Please note that types starting with a \'$\' are reserved for internal usage.')
    },
    required: {
      type: 'array',
      items: {
    ...
    when: {
      type: 'string',
      markdownDescription: localizeWithPath(_moduleId, 2, 'Condition which must be true to enable this type of task. Consider using `shellExecutionSupported`, `processExecutionSupported`, and `customExecutionSupported` as appropriate for this task definition. See the [API documentation](https://code.visualstudio.com/api/extension-guides/task-provider#when-clause) for more information.'),
      default: ''
    }
  }
};
...
(function (Configuration) {
  function from(value, extensionId, messageCollector) {
    ...
    if (!taskType || taskType.length === 0) {
      messageCollector.error(localizeWithPath(_moduleId, 3, 'The task type configuration is missing the required \'taskType\' property'));
      return undefined;
    }
    const required = [];
