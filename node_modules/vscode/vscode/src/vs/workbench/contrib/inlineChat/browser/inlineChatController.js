...
const {
  ...
  ErrorResponse,
  Session
} = require('./inlineChatSession.js');
const {
  IInlineChatSessionService
} = require('./inlineChatSessionService.service.js');
const {
  LiveStrategy,
  PreviewStrategy
} = require('./inlineChatStrategies.js');
const {
  InlineChatZoneWidget
} = require('./inlineChatZoneWidget.js');
const {
  INLINE_CHAT_ID,
...
} = require('./inlineChatContentWidget.js');
const {
  MessageController
...
const {
  InlineChatError
} = require('./inlineChatSessionServiceImpl.js');
const {
  ILanguageFeaturesService
} = require('../../../../editor/common/services/languageFeatures.js');
const {
  ChatInputPart
} = require('../../chat/browser/chatInputPart.js');
const {
  OffsetRange
} = require('../../../../editor/common/core/offsetRange.js');
const {
  isEqual
...
let InlineChatController = class InlineChatController {
  ...
  dispose() {
    if (this._currentRun) {
      this._messages.fire(this._session?.chatModel.hasRequests ? 4 : 2);
    }
    this._store.dispose();
  ...
  async ["INIT_UI"](options) {
    ...
    this._sessionStore.add(this._editor.onDidChangeModel(e => {
      const msg = this._session?.chatModel.hasRequests ? 4 : 2;
    ...
    this._sessionStore.add(this._languageFeatureService.completionProvider.register({
        ...
        for (const command of this._session.session.slashCommands) {
          ...
          result.suggestions.push({
            ...
            command: command.executeImmediately ? {
              id: 'workbench.action.chat.acceptInput',
              title: withSlash
            } : undefined
          });
        }
    ...
    const refer = this._session.session.slashCommands?.some(value => {
      if (value.refer) {
        ...
        if (request?.message.text.startsWith(`/${value.command}`)) {
          slashCommandLike = new ChatRequestSlashCommandPart(new OffsetRange(0, 1), new Range(1, 1, 1, 1), {
  ...
  async ["SHOW_RESPONSE"]() {
    ...
    if (response instanceof EmptyResponse) {
      ...
      this._zone.value.widget.updateStatus(status, {
        classes: ['warn']
      });
      return "WAIT_FOR_INPUT";
    } else if (response instanceof ErrorResponse) {
      if (!response.isCancellation) {
        this._zone.value.widget.updateStatus(response.message, {
          classes: ['error']
        });
        this._strategy?.cancel();
      }
  ...
  getInput() {
    return this._input.value.isVisible ? this._input.value.value : this._zone.value.widget.value;
  }
...
InlineChatController = __decorate([__param(1, IInstantiationService), __param(2, IInlineChatSessionService), __param(3, IInlineChatSavingService), __param(4, IEditorWorkerService), __param(5, ILogService), __param(6, IConfigurationService), __param(7, IDialogService), __param(8, IContextKeyService), __param(9, IChatAgentService), __param(10, IChatService), __param(11, ICommandService), __param(12, ILanguageFeaturesService), __param(13, IChatWidgetService)], InlineChatController);
async function showMessageResponse(accessor, query, response) {
  ...
  if (widget && widget.viewModel) {
    chatService.addCompleteRequest(widget.viewModel.sessionId, query, undefined, 0, {
      message: response
    });
