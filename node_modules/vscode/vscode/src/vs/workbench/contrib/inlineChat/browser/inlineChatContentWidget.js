...
const {} = require('./media/inlineChatContentWidget.css.js');
const {
  reset,
  trackFocus
...
let InlineChatContentWidget = class InlineChatContentWidget {
  constructor(_editor, instaService, contextKeyService) {
    ...
    const scopedInstaService = instaService.createChild(new ServiceCollection([IContextKeyService, this._store.add(contextKeyService.createScoped(this._domNode))]));
    this._widget = scopedInstaService.createInstance(ChatWidget, ChatAgentLocation.Editor, {
      resource: true
    }, {
...
InlineChatContentWidget = __decorate([__param(1, IInstantiationService), __param(2, IContextKeyService)], InlineChatContentWidget);
