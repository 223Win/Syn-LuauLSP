...
} = require('../../chat/common/chatService.js');
const {
  IChatService
} = require('../../chat/common/chatService.service.js');
const {
  IInlineChatService
} = require('../common/inlineChat.service.js');
const {
  IEditorService
...
} = require('../../chat/common/chatVariables.service.js');
const {
...
let BridgeAgent = class BridgeAgent {
  ...
  async invoke(request, progress, _history, token) {
    ...
    const inlineProgress = new Progress(data => {
      ...
      if (data.markdownFragment) {
        progress({
          ...
          content: new MarkdownString(data.markdownFragment)
        });
      }
    ...
    try {
      ...
      if (result) {
        if (result.message) {
          inlineProgress.report({
            markdownFragment: result.message.value
          });
        }
        ...
        const markdownContents = result.message ?? new MarkdownString('', {
...
BridgeAgent = __decorate([__param(3, IInstantiationService)], BridgeAgent);
...
class InlineChatContext {
  static {
    this.variableName = '_inlineChatContext';
  }
...
let InlineChatSessionServiceImpl = class InlineChatSessionServiceImpl {
    ...
    this._store.add(chatVariableService.registerVariable({
      ...
      for (const [, data] of this._sessions) {
        if (data.session.chatModel === model) {
          return [{
            level: 'full',
            value: data.session.textModelN.uri
          }];
        }
      }
      return undefined;
  ...
  async createSession(editor, options, token) {
    ...
    const session = new Session(options.editMode, targetUri, textModel0, textModelN, provider, rawSession, store.add(new SessionWholeRange(textModelN, wholeRange)), store.add(new HunkData(this._editorWorkerService, textModel0, textModelN)), chatModel);
...
InlineChatSessionServiceImpl = __decorate([__param(0, IInlineChatService), __param(1, ITelemetryService), __param(2, IModelService), __param(3, ITextModelService), __param(4, IEditorWorkerService), __param(5, ILogService), __param(6, IInstantiationService), __param(7, IEditorService), __param(8, IChatService), __param(9, IChatAgentService), __param(10, IChatVariablesService)], InlineChatSessionServiceImpl);
  ...
  async provideResponse(item, request, progress, token) {
    ...
    await this._chatAgentService.invokeAgent(this.agent.id, {
      ...
      if (part.kind === 'markdownContent') {
        progress.report({
          markdownFragment: part.content.value
        });
      } else if (part.kind === 'agentDetection') {
        progress.report({
          slashCommand: part.command?.name
        });
...
AgentInlineChatProvider = __decorate([__param(1, IChatAgentService)], AgentInlineChatProvider);
