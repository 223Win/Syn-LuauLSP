...
class SessionWholeRange {
  static {
    this._options = ModelDecorationOptions.register({
...
class Session {
  constructor(editMode, targetUri, textModel0, textModelN, provider, session, wholeRange, hunkData, chatModel) {
...
ReplyResponse = __decorate([__param(7, ITextFileService), __param(8, ILanguageService)], ReplyResponse);
...
StashedSession = __decorate([__param(3, IContextKeyService), __param(4, IInlineChatSessionService), __param(5, ILogService)], StashedSession);
let HunkData = class HunkData {
  ...
  async recompute() {
    const diff = await this._editorWorkerService.computeDiff(this._textModel0.uri, this._textModelN.uri, {
    ...
    this._textModelN.changeDecorations(accessorN => {
      this._textModel0.changeDecorations(accessor0 => {
        ...
        for (const hunk of hunks) {
          ...
          textModelNDecorations.push(accessorN.addDecoration(LineRange.asRange(hunk.modified, this._textModelN), HunkData_1._HUNK_TRACKED_RANGE));
          textModel0Decorations.push(accessor0.addDecoration(LineRange.asRange(hunk.original, this._textModel0), HunkData_1._HUNK_TRACKED_RANGE));
  ...
  get pending() {
    return Iterable.reduce(this._data.values(), (r, {
      state
    }) => r + (state === 0 ? 1 : 0), 0);
  }
  _discardEdits(item) {
...
HunkData = HunkData_1 = __decorate([__param(0, IEditorWorkerService)], HunkData);
class RawHunk {
