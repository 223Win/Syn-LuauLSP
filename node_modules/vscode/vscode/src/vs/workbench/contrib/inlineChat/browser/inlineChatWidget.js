...
const {
  ProgressBar
} = require('../../../../base/browser/ui/progressbar/progressbar.js');
const {
  Emitter,
  Event
...
const {} = require('./media/inlineChat.css.js');
const {
  AccessibleDiffViewer
} = require('../../../../editor/browser/widget/diffEditor/components/accessibleDiffViewer.js');
const {
...
const {
  ...
  RangeMapping
} = require('../../../../editor/common/diff/rangeMapping.js');
const {
  ITextModelService
...
} = require('../../../../platform/actions/browser/buttonbar.js');
const {
...
const {
  ChatFollowups
} = require('../../chat/browser/chatFollowups.js');
const {
  ChatModel
} = require('../../chat/common/chatModel.js');
const {
  isWelcomeVM,
  isRequestVM,
  isResponseVM
} = require('../../chat/common/chatViewModel.js');
const {
...
const {
  ChatWidget
} = require('../../chat/browser/chatWidget.js');
const {
  chatRequestBackground
} = require('../../chat/common/chatColors.js');
const {
  Selection
} = require('../../../../editor/common/core/selection.js');
const {
  ChatAgentLocation
} = require('../../chat/common/chatAgents.js');
const {
...
const {
  IChatService
} = require('../../chat/common/chatService.service.js');
const {
...
let InlineChatWidget = class InlineChatWidget {
  constructor(location, options, _instantiationService, _contextKeyService, _keybindingService, _accessibilityService, _configurationService, _accessibleViewService, _textModelResolverService, _chatService, _hoverService) {
    ...
    this._elements = h('div.inline-chat@root', [h('div.chat-widget@chatWidget'), h('div.progress@progress'), h('div.followUps.hidden@followUps'), h('div.previewDiff.hidden@previewDiff'), h('div.accessibleViewer@accessibleViewer'), h('div.status@status', [h('div.label.info.hidden@infoLabel'), h('div.actions.hidden@statusToolbar'), h('div.label.status.hidden@statusLabel'), h('div.actions.hidden@feedbackToolbar')])]);
    ...
    const scopedInstaService = _instantiationService.createChild(new ServiceCollection([IContextKeyService, this._store.add(_contextKeyService.createScoped(this._elements.chatWidget))]));
    ...
    if (options.feedbackMenuId) {
      const feedbackToolbar = this._instantiationService.createInstance(MenuWorkbenchToolBar, this._elements.feedbackToolbar, options.feedbackMenuId, {
        ...workbenchToolbarOptions,
  ...
  _updateAriaLabel() {
    ...
    if (this._accessibilityService.isScreenReaderOptimized()) {
      ...
      if (this._configurationService.getValue("accessibility.verbosity.inlineChat")) {
        ...
        label = kbLabel ? localizeWithPath(_moduleId, 0, "Inline Chat Input, Use {0} for Inline Chat Accessibility Help.", kbLabel) : localizeWithPath(_moduleId, 1, "Inline Chat Input, Run the Inline Chat Accessibility Help command for more information.");
      }
...
const codeEditorWidgetOptions = {
  isSimpleWidget: true,
  contributions: EditorExtensionsRegistry.getSomeEditorContributions([SnippetController2.ID, SuggestController.ID])
};
...
let EditorBasedInlineChatWidget = class EditorBasedInlineChatWidget extends InlineChatWidget {
  constructor(_parentEditor, options, contextKeyService, keybindingService, instantiationService, accessibilityService, configurationService, accessibleViewService, textModelResolverService, chatService, hoverService) {
    super(ChatAgentLocation.Editor, {
      ...options,
  ...
  showEditsPreview(hunks, textModel0, textModelN) {
    ...
    this._previewDiffEditor.value.setModel({
      original: textModel0,
      modified: textModelN
    });
...
EditorBasedInlineChatWidget = __decorate([__param(2, IContextKeyService), __param(3, IKeybindingService), __param(4, IInstantiationService), __param(5, IAccessibilityService), __param(6, IConfigurationService), __param(7, IAccessibleViewService), __param(8, ITextModelService), __param(9, IChatService), __param(10, IHoverService)], EditorBasedInlineChatWidget);
...
HunkAccessibleDiffViewer = HunkAccessibleDiffViewer_1 = __decorate([__param(4, IInstantiationService)], HunkAccessibleDiffViewer);
