...
} = require('./inlineChatController.js');
const {
...
} = require('../../chat/browser/actions/chatAccessibilityHelp.js');
const {
...
class AbstractInlineChatAction extends EditorAction2 {
  static {
    this.category = localize2WithPath(_moduleId, 3, "Inline Chat");
  }
  constructor(desc) {
    super({
      ...desc,
  ...
  constructor() {
    super({
      ...
      menu: {
        ...
        when: ContextKeyExpr.and(CTX_INLINE_CHAT_RESPONSE_TYPES.notEqualsTo("onlyMessages"), CTX_INLINE_CHAT_RESPONSE_TYPES.notEqualsTo("empty"), CTX_INLINE_CHAT_EDIT_MODE.isEqualTo("live")),
        group: '0_main',
        order: 3
      }
    });
...
class AcceptChanges extends AbstractInlineChatAction {
  constructor() {
    super({
      ...
      menu: {
        when: ContextKeyExpr.and(CTX_INLINE_CHAT_RESPONSE_TYPES.notEqualsTo("onlyMessages"), CTX_INLINE_CHAT_RESPONSE_TYPES.notEqualsTo("empty")),
        id: MENU_INLINE_CHAT_WIDGET_STATUS,
        group: '0_main',
        order: 0
      }
...
class CancelSessionAction extends AbstractInlineChatAction {
  constructor() {
    super({
      ...
      icon: Codicon.clearAll,
      precondition: ContextKeyExpr.and(CTX_INLINE_CHAT_VISIBLE, CTX_INLINE_CHAT_EDIT_MODE.isEqualTo("preview")),
      keybinding: {
        weight: 100 - 1,
        primary: 9
      },
      menu: {
        id: MENU_INLINE_CHAT_WIDGET_STATUS,
        when: ContextKeyExpr.and(CTX_INLINE_CHAT_EDIT_MODE.isEqualTo("preview"), CTX_INLINE_CHAT_RESPONSE_TYPES.notEqualsTo("empty")),
        group: '0_main',
        order: 3
      }
    });
...
class CopyRecordings extends AbstractInlineChatAction {
  ...
  async runInlineChatCommand(accessor) {
    ...
    const picks = recordings.map(rec => {
      return {
        rec,
        label: localizeWithPath(_moduleId, 20, "'{0}' and {1} follow ups ({2})", rec.exchanges[0].prompt, rec.exchanges.length - 1, fromNow(rec.when, true)),
...
exports.LOCALIZED_START_INLINE_CHAT_STRING = LOCALIZED_START_INLINE_CHAT_STRING;
exports.MoveToNextHunk = MoveToNextHunk;
exports.MoveToPreviousHunk = MoveToPreviousHunk;
exports.START_INLINE_CHAT = START_INLINE_CHAT;
