...
let ExternalUriOpenerService = class ExternalUriOpenerService extends Disposable {
  ...
  async showOpenerPrompt(openers, targetUri, ctx, token) {
    ...
    items.push({
      label: isWeb ? localizeWithPath(_moduleId, 0, 'Open in new browser window') : localizeWithPath(_moduleId, 1, 'Open in default browser'),
      opener: undefined
    }, {
      type: 'separator'
    }, {
    ...
    const picked = await this.quickInputService.pick(items, {
      placeHolder: localizeWithPath(_moduleId, 3, "How would you like to open: {0}", targetUri.toString())
    });
    if (!picked) {
      return true;
    }
