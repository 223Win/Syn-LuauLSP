...
const {} = require('./media/extensionActions.css.js');
const {
  localizeWithPath,
  localize2WithPath
} = require('../../../../nls.js');
...
} = require('../common/extensions.js');
const {
  IExtensionsWorkbenchService
} = require('../common/extensions.service.js');
const {
  ExtensionsConfigurationInitialContent
} = require('../common/extensionsFileTemplate.js');
const {
  ExtensionManagementErrorCode,
  TargetPlatformToString
} = require('../../../../platform/extensionManagement/common/extensionManagement.js');
const {
...
const {
  IJSONEditingService
} = require('../../../services/configuration/common/jsonEditing.service.js');
const {
  ITextModelService
...
} = require('../../../browser/actions/workspaceCommands.js');
...
} = require('../../../services/themes/common/workbenchThemeService.service.js');
const {
  ILabelService
...
const {
  IProductService
} = require('../../../../platform/product/common/productService.service.js');
const {
  IDialogService
...
const {
  EXTENSIONS_CONFIG
} = require('../../../services/extensionRecommendations/common/workspaceExtensionsConfig.js');
const {
  getErrorMessage,
  isCancellationError
...
} = require('../../../../platform/userDataSync/common/userDataSync.service.js');
const {
  ActionWithDropdownActionViewItem
} = require('../../../../base/browser/ui/dropdown/dropdownActionViewItem.js');
const {
  ILogService
...
} = require('../../../services/extensions/common/extensionManifestPropertiesService.service.js');
const {
  IWorkspaceTrustEnablementService,
  IWorkspaceTrustManagementService
} = require('../../../../platform/workspace/common/workspaceTrust.service.js');
const {
  isVirtualWorkspace
} = require('../../../../platform/workspace/common/virtualWorkspace.js');
const {
  MarkdownString,
...
const {
  getLocale
} = require('../../../../platform/languagePacks/common/languagePacks.js');
const {
  ILocaleService
} = require('../../../services/localization/common/locale.service.js');
const {
  isString
} = require('../../../../base/common/types.js');
const {
  showWindowLogActionId
} = require('../../../services/log/common/logConstants.js');
const {
  ITelemetryService
...
} = require('../../../services/extensionManagement/common/extensionFeatures.service.js');
const {
  Registry
} = require('../../../../platform/registry/common/platform.js');
const {
  IUpdateService
} = require('../../../../platform/update/common/update.service.js');
...
const _moduleId = "vs/workbench/contrib/extensions/browser/extensionsActions";
let PromptExtensionInstallFailureAction = class PromptExtensionInstallFailureAction extends Action {
  ...
  async run() {
    ...
    if (this.error.name === ExtensionManagementErrorCode.Unsupported) {
      const productName = isWeb ? localizeWithPath(_moduleId, 0, "{0} for the Web", this.productService.nameLong) : this.productService.nameLong;
      const message = localizeWithPath(_moduleId, 1, "The '{0}' extension is not available in {1}. Click 'More Information' to learn more.", this.extension.displayName || this.extension.identifier.id, productName);
      const {
        confirmed
      } = await this.dialogService.confirm({
        type: Severity$1.Info,
        message,
        primaryButton: localizeWithPath(_moduleId, 2, "&&More Information"),
        cancelButton: localizeWithPath(_moduleId, 3, "Close")
      });
      if (confirmed) {
    ...
    if (ExtensionManagementErrorCode.ReleaseVersionNotFound === this.error.name) {
      await this.dialogService.prompt({
        ...
        buttons: [{
          label: localizeWithPath(_moduleId, 4, "Install Pre-Release"),
          run: () => {
        ...
        cancelButton: localizeWithPath(_moduleId, 5, "Cancel")
      });
      return;
    }
    ...
    if (ExtensionManagementErrorCode.Signature === this.error.name) {
      await this.dialogService.prompt({
        type: 'error',
        message: localizeWithPath(_moduleId, 6, "{0} cannot verify the '{1}' extension. Are you sure you want to install it?", this.productService.nameLong, this.extension.displayName || this.extension.identifier.id),
        buttons: [{
          label: localizeWithPath(_moduleId, 7, "Install Anyway"),
          run: () => {
            const installAction = this.instantiationService.createInstance(InstallAction, {
    ...
    const operationMessage = this.installOperation === 3 ? localizeWithPath(_moduleId, 8, "Error while updating '{0}' extension.", this.extension.displayName || this.extension.identifier.id) : localizeWithPath(_moduleId, 9, "Error while installing '{0}' extension.", this.extension.displayName || this.extension.identifier.id);
    let additionalMessage;
    const promptChoices = [];
    const downloadUrl = await this.getDownloadUrl();
    if (downloadUrl) {
      additionalMessage = localizeWithPath(_moduleId, 10, "Please check the [log]({0}) for more details.", `command:${showWindowLogActionId}`);
      promptChoices.push({
        label: localizeWithPath(_moduleId, 11, "Try Downloading Manually..."),
        run: () => this.openerService.open(downloadUrl).then(() => {
          this.notificationService.prompt(Severity$1.Info, localizeWithPath(_moduleId, 12, 'Once downloaded, please manually install the downloaded VSIX of \'{0}\'.', this.extension.identifier.id), [{
            label: localizeWithPath(_moduleId, 13, "Install from VSIX..."),
...
PromptExtensionInstallFailureAction = __decorate([__param(4, IProductService), __param(5, IOpenerService), __param(6, INotificationService), __param(7, IDialogService), __param(8, ICommandService), __param(9, ILogService), __param(10, IExtensionManagementServerService), __param(11, IInstantiationService), __param(12, IExtensionGalleryService), __param(13, IExtensionManifestPropertiesService)], PromptExtensionInstallFailureAction);
...
let InstallAction = class InstallAction extends ExtensionAction {
  ...
  static {
    this.Class = `${ExtensionAction.LABEL_ACTION_CLASS} prominent install`;
  }
  ...
  constructor(options, extensionsWorkbenchService, instantiationService, runtimeExtensionService, workbenchThemeService, labelService, dialogService, preferencesService, telemetryService, contextService) {
    super('extensions.install', localizeWithPath(_moduleId, 14, "Install"), InstallAction_1.Class, false);
  ...
  async run() {
    ...
    if (this.extension.deprecationInfo) {
      let detail = localizeWithPath(_moduleId, 15, "This extension is deprecated as it is no longer being maintained.");
      ...
      if (this.extension.deprecationInfo.extension) {
        detail = localizeWithPath(_moduleId, 16, "This extension is deprecated. Use the {0} extension instead.", this.extension.deprecationInfo.extension.displayName);
        const alternateExtension = this.extension.deprecationInfo.extension;
        buttons.push({
          label: localizeWithPath(_moduleId, 17, "&&Open {0}", this.extension.deprecationInfo.extension.displayName),
          run: async () => {
        ...
        detail = localizeWithPath(_moduleId, 18, "This extension is deprecated as this functionality is now built-in to VS Code.");
        const settings = this.extension.deprecationInfo.settings;
        buttons.push({
          label: localizeWithPath(_moduleId, 19, "&&Configure Settings"),
          run: async () => {
      ...
      const {
        ...
        message: localizeWithPath(_moduleId, 20, "Are you sure you want to install '{0}'?", this.extension.displayName),
    ...
    alert(localizeWithPath(_moduleId, 21, "Installing extension {0} started. An editor is now open with more details on this extension", this.extension.displayName));
    ...
    if (extension?.local) {
      alert(localizeWithPath(_moduleId, 22, "Installing extension {0} is completed.", this.extension.displayName));
  ...
  getLabel(primary) {
    if (this.extension?.isWorkspaceScoped && this.extension.resourceExtension && this.contextService.isInsideWorkspace(this.extension.resourceExtension.location)) {
      return localizeWithPath(_moduleId, 23, "Install Workspace Extension");
    }
    if (this.options.installPreReleaseVersion && this.extension?.hasPreReleaseVersion) {
      return primary ? localizeWithPath(_moduleId, 24, "Install Pre-Release") : localizeWithPath(_moduleId, 25, "Install Pre-Release Version");
    }
    if (this.extension?.hasPreReleaseVersion) {
      return primary ? localizeWithPath(_moduleId, 14, "Install") : localizeWithPath(_moduleId, 26, "Install Release Version");
    }
    return localizeWithPath(_moduleId, 14, "Install");
  }
};
...
let InstallDropdownAction = class InstallDropdownAction extends ActionWithDropDownAction {
  ...
  constructor(instantiationService, extensionsWorkbenchService) {
    super(`extensions.installActions`, '', [[instantiationService.createInstance(InstallAction, {
...
InstallDropdownAction = __decorate([__param(0, IInstantiationService), __param(1, IExtensionsWorkbenchService)], InstallDropdownAction);
class InstallingLabelAction extends ExtensionAction {
  static {
    this.LABEL = localizeWithPath(_moduleId, 27, "Installing");
  }
  static {
...
let InstallInOtherServerAction = class InstallInOtherServerAction extends ExtensionAction {
  ...
  static {
    this.INSTALL_LABEL = localizeWithPath(_moduleId, 14, "Install");
  }
  static {
    this.INSTALLING_LABEL = localizeWithPath(_moduleId, 27, "Installing");
  }
  static {
    this.Class = `${ExtensionAction.LABEL_ACTION_CLASS} prominent install`;
  }
  static {
    this.InstallingClass = `${ExtensionAction.LABEL_ACTION_CLASS} install installing`;
  }
...
let RemoteInstallAction = class RemoteInstallAction extends InstallInOtherServerAction {
  ...
  getInstallLabel() {
    return this.extensionManagementServerService.remoteExtensionManagementServer ? localizeWithPath(_moduleId, 28, "Install in {0}", this.extensionManagementServerService.remoteExtensionManagementServer.label) : InstallInOtherServerAction.INSTALL_LABEL;
  }
};
...
let LocalInstallAction = class LocalInstallAction extends InstallInOtherServerAction {
  ...
  getInstallLabel() {
    return localizeWithPath(_moduleId, 29, "Install Locally");
  }
};
...
let WebInstallAction = class WebInstallAction extends InstallInOtherServerAction {
  ...
  getInstallLabel() {
    return localizeWithPath(_moduleId, 30, "Install in Browser");
  }
};
...
let UninstallAction = class UninstallAction extends ExtensionAction {
  ...
  static {
    this.UninstallLabel = localizeWithPath(_moduleId, 31, "Uninstall");
  }
  static {
    this.UninstallingLabel = localizeWithPath(_moduleId, 32, "Uninstalling");
  }
  static {
  ...
  async run() {
    ...
    alert(localizeWithPath(_moduleId, 33, "Uninstalling extension {0} started.", this.extension.displayName));
    try {
      await this.extensionsWorkbenchService.uninstall(this.extension);
      alert(localizeWithPath(_moduleId, 34, "Please reload Visual Studio Code to complete the uninstallation of the extension {0}.", this.extension.displayName));
    } catch (error) {
      this.dialogService.error(getErrorMessage(error));
    }
  }
};
UninstallAction = UninstallAction_1 = __decorate([__param(0, IExtensionsWorkbenchService), __param(1, IDialogService)], UninstallAction);
class AbstractUpdateAction extends ExtensionAction {
  static {
    this.EnabledClass = `${ExtensionAction.LABEL_ACTION_CLASS} prominent update`;
  }
  static {
    this.DisabledClass = `${AbstractUpdateAction.EnabledClass} disabled`;
  }
...
let UpdateAction = class UpdateAction extends AbstractUpdateAction {
  constructor(verbose, extensionsWorkbenchService, instantiationService) {
    super(`extensions.update`, localizeWithPath(_moduleId, 35, "Update"), extensionsWorkbenchService);
  ...
  update() {
    super.update();
    if (this.extension) {
      this.label = this.verbose ? localizeWithPath(_moduleId, 36, "Update to v{0}", this.extension.latestVersion) : localizeWithPath(_moduleId, 35, "Update");
    }
  }
  async run() {
    if (!this.extension) {
    ...
    alert(localizeWithPath(_moduleId, 37, "Updating extension {0} to version {1} started.", this.extension.displayName, this.extension.latestVersion));
    return this.install(this.extension);
  }
  async install(extension) {
    try {
      await this.extensionsWorkbenchService.install(extension, extension.local?.preRelease ? {
        installPreReleaseVersion: true
      } : undefined);
      alert(localizeWithPath(_moduleId, 38, "Updating extension {0} to version {1} completed.", extension.displayName, extension.latestVersion));
    } catch (err) {
      this.instantiationService.createInstance(PromptExtensionInstallFailureAction, extension, extension.latestVersion, 3, err).run();
    }
  }
};
UpdateAction = __decorate([__param(1, IExtensionsWorkbenchService), __param(2, IInstantiationService)], UpdateAction);
let ToggleAutoUpdateForExtensionAction = class ToggleAutoUpdateForExtensionAction extends ExtensionAction {
  ...
  static {
    this.LABEL = localize2WithPath(_moduleId, 39, "Auto Update");
  }
  static {
    this.EnabledClass = `${ExtensionAction.EXTENSION_ACTION_CLASS} auto-update`;
  }
  static {
    this.DisabledClass = `${ToggleAutoUpdateForExtensionAction_1.EnabledClass} hide`;
  }
  ...
  async run() {
    ...
    if (enableAutoUpdate) {
      alert(localizeWithPath(_moduleId, 40, "Enabled auto updates for", this.extension.displayName));
    } else {
      alert(localizeWithPath(_moduleId, 41, "Disabled auto updates for", this.extension.displayName));
    }
  }
};
ToggleAutoUpdateForExtensionAction = ToggleAutoUpdateForExtensionAction_1 = __decorate([__param(2, IExtensionsWorkbenchService), __param(3, IConfigurationService)], ToggleAutoUpdateForExtensionAction);
let ToggleAutoUpdatesForPublisherAction = class ToggleAutoUpdatesForPublisherAction extends ExtensionAction {
  ...
  static {
    this.LABEL = localizeWithPath(_moduleId, 42, "Auto Update All (From Publisher)");
  }
  ...
  async run() {
    ...
    alert(localizeWithPath(_moduleId, 43, "Ignoring updates published by {0}.", this.extension.publisherDisplayName));
...
let MigrateDeprecatedExtensionAction = class MigrateDeprecatedExtensionAction extends ExtensionAction {
  ...
  static {
    this.DisabledClass = `${MigrateDeprecatedExtensionAction_1.EnabledClass} disabled`;
  }
  constructor(small, extensionsWorkbenchService) {
    super('extensionsAction.migrateDeprecatedExtension', localizeWithPath(_moduleId, 44, "Migrate"), MigrateDeprecatedExtensionAction_1.DisabledClass, false);
    this.small = small;
    this.extensionsWorkbenchService = extensionsWorkbenchService;
    this.update();
  }
  update() {
    ...
    this.tooltip = localizeWithPath(_moduleId, 45, "Migrate to {0}", this.extension.deprecationInfo.extension.displayName);
    this.label = this.small ? localizeWithPath(_moduleId, 46, "Migrate") : this.tooltip;
  }
  async run() {
    if (!this.extension?.deprecationInfo?.extension) {
      return;
...
ExtensionDropDownAction = __decorate([__param(4, IInstantiationService)], ExtensionDropDownAction);
...
DropDownMenuActionViewItem = __decorate([__param(2, IContextMenuService)], DropDownMenuActionViewItem);
async function getContextMenuActionsGroups(extension, contextKeyService, instantiationService) {
  return instantiationService.invokeFunction(async accessor => {
    ...
    const actionsGroups = menu.getActions({
...
let ManageExtensionAction = class ManageExtensionAction extends ExtensionDropDownAction {
  ...
  static {
    this.HideManageExtensionClass = `${ManageExtensionAction_1.Class} hide`;
  }
  constructor(instantiationService, extensionService, contextKeyService) {
    ...
    this.tooltip = localizeWithPath(_moduleId, 47, "Manage");
    this.update();
  }
  async getActionGroups() {
    ...
    groups.push([this.instantiationService.createInstance(EnableGloballyAction), this.instantiationService.createInstance(EnableForWorkspaceAction)]);
    groups.push([this.instantiationService.createInstance(DisableGloballyAction), this.instantiationService.createInstance(DisableForWorkspaceAction)]);
    if (updateActions.length) {
      groups.push(updateActions);
    }
    groups.push([...(installActions.length ? installActions : []), this.instantiationService.createInstance(InstallAnotherVersionAction), this.instantiationService.createInstance(UninstallAction)]);
...
ManageExtensionAction = ManageExtensionAction_1 = __decorate([__param(0, IInstantiationService), __param(1, IExtensionService), __param(2, IContextKeyService)], ManageExtensionAction);
...
let TogglePreReleaseExtensionAction = class TogglePreReleaseExtensionAction extends ExtensionAction {
  ...
  static {
    this.LABEL = localizeWithPath(_moduleId, 48, "Pre-Release");
  }
  static {
    this.EnabledClass = `${ExtensionAction.LABEL_ACTION_CLASS} pre-release`;
  }
  static {
    this.DisabledClass = `${TogglePreReleaseExtensionAction_1.EnabledClass} hide`;
  }
  constructor(extensionsWorkbenchService) {
  ...
  update() {
    ...
    if (this.extension.preRelease) {
      this.label = localizeWithPath(_moduleId, 49, "Switch to Release Version");
      this.tooltip = localizeWithPath(_moduleId, 50, "This will switch and enable updates to release versions");
    } else {
      this.label = localizeWithPath(_moduleId, 51, "Switch to Pre-Release Version");
      this.tooltip = localizeWithPath(_moduleId, 52, "This will switch to pre-release version and enable updates to latest version always");
    }
  }
  async run() {
    if (!this.extension) {
...
TogglePreReleaseExtensionAction = TogglePreReleaseExtensionAction_1 = __decorate([__param(0, IExtensionsWorkbenchService)], TogglePreReleaseExtensionAction);
let InstallAnotherVersionAction = class InstallAnotherVersionAction extends ExtensionAction {
  ...
  static {
    this.LABEL = localizeWithPath(_moduleId, 53, "Install Another Version...");
  }
  ...
  async run() {
    ...
    if (!allVersions.length) {
      await this.dialogService.info(localizeWithPath(_moduleId, 54, "This extension has no other versions."));
      return;
    }
    const picks = allVersions.map((v, i) => {
      return {
        id: v.version,
        label: v.version,
        description: `${fromNow(new Date(Date.parse(v.date)), true)}${v.isPreReleaseVersion ? ` (${localizeWithPath(_moduleId, 55, "pre-release")})` : ''}${v.version === this.extension.version ? ` (${localizeWithPath(_moduleId, 56, "current")})` : ''}`,
        latest: i === 0,
    ...
    const pick = await this.quickInputService.pick(picks, {
      placeHolder: localizeWithPath(_moduleId, 57, "Select Version to Install"),
      matchOnDetail: true
    });
    if (pick) {
      ...
      try {
        if (pick.latest) {
          const [extension] = pick.id !== this.extension?.version ? await this.extensionsWorkbenchService.getExtensions([{
            id: this.extension.identifier.id,
            preRelease: pick.isPreReleaseVersion
          }], CancellationToken.None) : [this.extension];
...
InstallAnotherVersionAction = InstallAnotherVersionAction_1 = __decorate([__param(0, IExtensionsWorkbenchService), __param(1, IExtensionGalleryService), __param(2, IQuickInputService), __param(3, IInstantiationService), __param(4, IDialogService)], InstallAnotherVersionAction);
let EnableForWorkspaceAction = class EnableForWorkspaceAction extends ExtensionAction {
  ...
  static {
    this.LABEL = localizeWithPath(_moduleId, 58, "Enable (Workspace)");
  }
  constructor(extensionsWorkbenchService, extensionEnablementService) {
    ...
    this.tooltip = localizeWithPath(_moduleId, 59, "Enable this extension only in this workspace");
    this.update();
  }
  update() {
    this.enabled = false;
...
EnableForWorkspaceAction = EnableForWorkspaceAction_1 = __decorate([__param(0, IExtensionsWorkbenchService), __param(1, IWorkbenchExtensionEnablementService)], EnableForWorkspaceAction);
let EnableGloballyAction = class EnableGloballyAction extends ExtensionAction {
  ...
  static {
    this.LABEL = localizeWithPath(_moduleId, 60, "Enable");
  }
  constructor(extensionsWorkbenchService, extensionEnablementService) {
    super(EnableGloballyAction_1.ID, EnableGloballyAction_1.LABEL, ExtensionAction.LABEL_ACTION_CLASS);
    this.extensionsWorkbenchService = extensionsWorkbenchService;
    this.extensionEnablementService = extensionEnablementService;
    this.tooltip = localizeWithPath(_moduleId, 61, "Enable this extension");
    this.update();
  }
  update() {
    this.enabled = false;
...
EnableGloballyAction = EnableGloballyAction_1 = __decorate([__param(0, IExtensionsWorkbenchService), __param(1, IWorkbenchExtensionEnablementService)], EnableGloballyAction);
let DisableForWorkspaceAction = class DisableForWorkspaceAction extends ExtensionAction {
  ...
  static {
    this.LABEL = localizeWithPath(_moduleId, 62, "Disable (Workspace)");
  }
  constructor(workspaceContextService, extensionsWorkbenchService, extensionEnablementService, extensionService) {
    ...
    this.extensionEnablementService = extensionEnablementService;
    this.extensionService = extensionService;
    this.tooltip = localizeWithPath(_moduleId, 63, "Disable this extension only in this workspace");
    this.update();
  ...
  update() {
    ...
    if (this.extension && this.extension.local && !this.extension.isWorkspaceScoped && this.extensionService.extensions.some(e => areSameExtensions({
      ...
      uuid: e.uuid
    }, this.extension.identifier) && this.workspaceContextService.getWorkbenchState() !== 1)) {
...
DisableForWorkspaceAction = DisableForWorkspaceAction_1 = __decorate([__param(0, IWorkspaceContextService), __param(1, IExtensionsWorkbenchService), __param(2, IWorkbenchExtensionEnablementService), __param(3, IExtensionService)], DisableForWorkspaceAction);
let DisableGloballyAction = class DisableGloballyAction extends ExtensionAction {
  ...
  static {
    this.LABEL = localizeWithPath(_moduleId, 64, "Disable");
  }
  constructor(extensionsWorkbenchService, extensionEnablementService, extensionService) {
    ...
    this.extensionEnablementService = extensionEnablementService;
    this.extensionService = extensionService;
    this.tooltip = localizeWithPath(_moduleId, 65, "Disable this extension");
    this.update();
    this._register(this.extensionService.onDidChangeExtensions(() => this.update()));
  }
  update() {
...
DisableGloballyAction = DisableGloballyAction_1 = __decorate([__param(0, IExtensionsWorkbenchService), __param(1, IWorkbenchExtensionEnablementService), __param(2, IExtensionService)], DisableGloballyAction);
let EnableDropDownAction = class EnableDropDownAction extends ActionWithDropDownAction {
  constructor(instantiationService) {
    super('extensions.enable', localizeWithPath(_moduleId, 66, "Enable"), [[instantiationService.createInstance(EnableGloballyAction), instantiationService.createInstance(EnableForWorkspaceAction)]]);
  }
};
...
let DisableDropDownAction = class DisableDropDownAction extends ActionWithDropDownAction {
  constructor(instantiationService) {
    super('extensions.disable', localizeWithPath(_moduleId, 67, "Disable"), [[instantiationService.createInstance(DisableGloballyAction), instantiationService.createInstance(DisableForWorkspaceAction)]]);
  }
};
...
let ExtensionRuntimeStateAction = class ExtensionRuntimeStateAction extends ExtensionAction {
  ...
  static {
    this.DisabledClass = `${ExtensionRuntimeStateAction_1.EnabledClass} disabled`;
  }
  ...
  update() {
    ...
    this.label = runtimeState.action === "reloadWindow" ? localizeWithPath(_moduleId, 68, 'Reload Window') : runtimeState.action === "restartExtensions" ? localizeWithPath(_moduleId, 69, 'Restart Extensions') : runtimeState.action === "quitAndInstall" ? localizeWithPath(_moduleId, 70, 'Restart to Update') : runtimeState.action === "applyUpdate" || runtimeState.action === "downloadUpdate" ? localizeWithPath(_moduleId, 71, 'Update {0}', this.productService.nameShort) : '';
  }
  async run() {
    const runtimeState = this.extension?.runtimeState;
...
function getQuickPickEntries(themes, currentTheme, extension, showCurrentTheme) {
  ...
  if (showCurrentTheme) {
    picks.push({
      type: 'separator',
      label: localizeWithPath(_moduleId, 56, "current")
    });
    picks.push({
...
let SetColorThemeAction = class SetColorThemeAction extends ExtensionAction {
  ...
  static {
    this.TITLE = localize2WithPath(_moduleId, 72, 'Set Color Theme');
  }
  static {
    this.EnabledClass = `${ExtensionAction.LABEL_ACTION_CLASS} theme`;
  }
  static {
    this.DisabledClass = `${SetColorThemeAction_1.EnabledClass} disabled`;
  }
  ...
  async run({
    ...
    const pickedTheme = await this.quickInputService.pick(picks, {
      placeHolder: localizeWithPath(_moduleId, 73, "Select Color Theme"),
...
let SetFileIconThemeAction = class SetFileIconThemeAction extends ExtensionAction {
  ...
  static {
    this.TITLE = localize2WithPath(_moduleId, 74, 'Set File Icon Theme');
  }
  static {
    this.EnabledClass = `${ExtensionAction.LABEL_ACTION_CLASS} theme`;
  }
  static {
    this.DisabledClass = `${SetFileIconThemeAction_1.EnabledClass} disabled`;
  }
  constructor(extensionService, workbenchThemeService, quickInputService, extensionEnablementService) {
  ...
  async run({
    ...
    const pickedTheme = await this.quickInputService.pick(picks, {
      placeHolder: localizeWithPath(_moduleId, 75, "Select File Icon Theme"),
...
let SetProductIconThemeAction = class SetProductIconThemeAction extends ExtensionAction {
  ...
  static {
    this.TITLE = localize2WithPath(_moduleId, 76, 'Set Product Icon Theme');
  }
  static {
    this.EnabledClass = `${ExtensionAction.LABEL_ACTION_CLASS} theme`;
  }
  static {
    this.DisabledClass = `${SetProductIconThemeAction_1.EnabledClass} disabled`;
  }
  constructor(extensionService, workbenchThemeService, quickInputService, extensionEnablementService) {
  ...
  async run({
    ...
    const pickedTheme = await this.quickInputService.pick(picks, {
      placeHolder: localizeWithPath(_moduleId, 77, "Select Product Icon Theme"),
...
let SetLanguageAction = class SetLanguageAction extends ExtensionAction {
  ...
  static {
    this.TITLE = localize2WithPath(_moduleId, 78, 'Set Display Language');
  }
  static {
    this.EnabledClass = `${ExtensionAction.LABEL_ACTION_CLASS} language`;
  }
  static {
    this.DisabledClass = `${SetLanguageAction_1.EnabledClass} disabled`;
  }
  constructor(extensionsWorkbenchService) {
...
let ClearLanguageAction = class ClearLanguageAction extends ExtensionAction {
  ...
  static {
    this.TITLE = localize2WithPath(_moduleId, 79, 'Clear Display Language');
  }
  static {
    this.EnabledClass = `${ExtensionAction.LABEL_ACTION_CLASS} language`;
  }
  static {
    this.DisabledClass = `${ClearLanguageAction_1.EnabledClass} disabled`;
  }
...
let ShowRecommendedExtensionAction = class ShowRecommendedExtensionAction extends Action {
  ...
  static {
    this.LABEL = localizeWithPath(_moduleId, 80, "Show Recommended Extension");
  }
...
ShowRecommendedExtensionAction = ShowRecommendedExtensionAction_1 = __decorate([__param(1, IPaneCompositePartService), __param(2, IExtensionsWorkbenchService)], ShowRecommendedExtensionAction);
let InstallRecommendedExtensionAction = class InstallRecommendedExtensionAction extends Action {
  ...
  static {
    this.LABEL = localizeWithPath(_moduleId, 81, "Install Recommended Extension");
  }
...
InstallRecommendedExtensionAction = InstallRecommendedExtensionAction_1 = __decorate([__param(1, IPaneCompositePartService), __param(2, IInstantiationService), __param(3, IExtensionsWorkbenchService)], InstallRecommendedExtensionAction);
let IgnoreExtensionRecommendationAction = class IgnoreExtensionRecommendationAction extends Action {
  ...
  constructor(extension, extensionRecommendationsManagementService) {
    ...
    this.tooltip = localizeWithPath(_moduleId, 82, "Do not recommend this extension again");
    this.enabled = true;
  }
  run() {
...
let UndoIgnoreExtensionRecommendationAction = class UndoIgnoreExtensionRecommendationAction extends Action {
  ...
  constructor(extension, extensionRecommendationsManagementService) {
    ...
    this.tooltip = localizeWithPath(_moduleId, 83, "Undo");
    this.enabled = true;
  }
  run() {
...
let SearchExtensionsAction = class SearchExtensionsAction extends Action {
  constructor(searchValue, paneCompositeService) {
    super('extensions.searchExtensions', localizeWithPath(_moduleId, 84, "Search Extensions"), undefined, true);
...
SearchExtensionsAction = __decorate([__param(1, IPaneCompositePartService)], SearchExtensionsAction);
let AbstractConfigureRecommendedExtensionsAction = class AbstractConfigureRecommendedExtensionsAction extends Action {
  ...
  openExtensionsFile(extensionsFileResource) {
    ...
    })), error => Promise.reject(new Error(localizeWithPath(_moduleId, 85, "Unable to create 'extensions.json' file inside the '.vscode' folder ({0}).", error))));
  }
...
let ConfigureWorkspaceRecommendedExtensionsAction = class ConfigureWorkspaceRecommendedExtensionsAction extends AbstractConfigureRecommendedExtensionsAction {
  ...
  static {
    this.LABEL = localizeWithPath(_moduleId, 86, "Configure Recommended Extensions (Workspace)");
  }
...
let ConfigureWorkspaceFolderRecommendedExtensionsAction = class ConfigureWorkspaceFolderRecommendedExtensionsAction extends AbstractConfigureRecommendedExtensionsAction {
  ...
  static {
    this.LABEL = localizeWithPath(_moduleId, 87, "Configure Recommended Extensions (Workspace Folder)");
  }
...
let ExtensionStatusLabelAction = class ExtensionStatusLabelAction extends Action {
  ...
  static {
    this.DISABLED_CLASS = `${ExtensionStatusLabelAction_1.ENABLED_CLASS} hide`;
  }
  get extension() {
    return this._extension;
  }
  ...
  computeLabel() {
    ...
    if (currentStatus !== null) {
      if (currentStatus === 0 && this.status === 1) {
        return canAddExtension() ? this.initialStatus === 1 && this.version !== currentVersion ? localizeWithPath(_moduleId, 88, "Updated") : localizeWithPath(_moduleId, 89, "Installed") : null;
      }
      if (currentStatus === 2 && this.status === 3) {
        this.initialStatus = this.status;
        return canRemoveExtension() ? localizeWithPath(_moduleId, 90, "Uninstalled") : null;
      }
    }
    if (currentEnablementState !== null) {
      ...
      if (!currentlyEnabled && enabled) {
        return canAddExtension() ? localizeWithPath(_moduleId, 91, "Enabled") : null;
      }
      if (currentlyEnabled && !enabled) {
        return canRemoveExtension() ? localizeWithPath(_moduleId, 92, "Disabled") : null;
      }
    }
    return null;
  }
...
let ToggleSyncExtensionAction = class ToggleSyncExtensionAction extends ExtensionDropDownAction {
  ...
  static {
    this.SYNC_CLASS = `${ToggleSyncExtensionAction_1.ICON_ACTION_CLASS} extension-sync ${ThemeIcon.asClassName(syncEnabledIcon)}`;
  }
  ...
  update() {
    ...
    if (this.extension) {
      ...
      this.tooltip = isIgnored ? localizeWithPath(_moduleId, 93, "This extension is ignored during sync") : localizeWithPath(_moduleId, 94, "This extension is synced");
    }
  }
  async run() {
    return super.run({
      actionGroups: [[new Action('extensions.syncignore', this.extensionsWorkbenchService.isExtensionIgnoredToSync(this.extension) ? localizeWithPath(_moduleId, 95, "Sync this extension") : localizeWithPath(_moduleId, 96, "Do not sync this extension"), undefined, true, () => this.extensionsWorkbenchService.toggleExtensionIgnoredToSync(this.extension))]],
      disposeActionsOnHide: true
    });
  }
};
...
let ExtensionStatusAction = class ExtensionStatusAction extends ExtensionAction {
  ...
  async computeAndUpdateStatus() {
    ...
    if (this.extension.isMalicious) {
      this.updateStatus({
        icon: warningIcon,
        message: new MarkdownString(localizeWithPath(_moduleId, 97, "This extension was reported to be problematic."))
      }, true);
      return;
    }
    if (this.extension.deprecationInfo) {
      if (this.extension.deprecationInfo.extension) {
        ...
        this.updateStatus({
          icon: warningIcon,
          message: new MarkdownString(localizeWithPath(_moduleId, 98, "This extension is deprecated. Use the {0} extension instead.", link))
        }, true);
      } else if (this.extension.deprecationInfo.settings) {
        const link = `[${localizeWithPath(_moduleId, 99, "settings")}](${URI.parse(`command:workbench.action.openSettings?${encodeURIComponent(JSON.stringify([this.extension.deprecationInfo.settings.map(setting => `@id:${setting}`).join(' ')]))}`)})`;
        this.updateStatus({
          icon: warningIcon,
          message: new MarkdownString(localizeWithPath(_moduleId, 100, "This extension is deprecated as this functionality is now built-in to VS Code. Configure these {0} to use this functionality.", link))
        }, true);
      } else {
        const message = new MarkdownString(localizeWithPath(_moduleId, 101, "This extension is deprecated as it is no longer being maintained."));
    ...
    if (this.extension.gallery && this.extension.state === 3 && !(await this.extensionsWorkbenchService.canInstall(this.extension))) {
      if (this.extensionManagementServerService.localExtensionManagementServer || this.extensionManagementServerService.remoteExtensionManagementServer) {
        ...
        const message = new MarkdownString(`${localizeWithPath(_moduleId, 102, "The '{0}' extension is not available in {1} for {2}.", this.extension.displayName || this.extension.identifier.id, this.productService.nameLong, TargetPlatformToString(targetPlatform))} [${localizeWithPath(_moduleId, 103, "Learn More")}](https://aka.ms/vscode-platform-specific-extensions)`);
        this.updateStatus({
          icon: warningIcon,
          message
        }, true);
      ...
      if (this.extensionManagementServerService.webExtensionManagementServer) {
        const productName = localizeWithPath(_moduleId, 0, "{0} for the Web", this.productService.nameLong);
        const message = new MarkdownString(`${localizeWithPath(_moduleId, 104, "The '{0}' extension is not available in {1}.", this.extension.displayName || this.extension.identifier.id, productName)} [${localizeWithPath(_moduleId, 105, "Learn Why")}](https://aka.ms/vscode-web-extensions-guide)`);
        this.updateStatus({
          icon: warningIcon,
          message
        }, true);
    ...
    if (this.extension.enablementState === 2) {
      this.updateStatus({
        message: new MarkdownString(localizeWithPath(_moduleId, 106, "This extension is disabled by the environment."))
      }, true);
      return;
    }
    if (this.extension.enablementState === 3) {
      this.updateStatus({
        message: new MarkdownString(localizeWithPath(_moduleId, 107, "This extension is enabled because it is required in the current environment."))
      }, true);
      return;
    }
    if (this.extension.enablementState === 4) {
      ...
      this.updateStatus({
        icon: infoIcon,
        message: new MarkdownString(details ? escapeMarkdownSyntaxTokens(details) : localizeWithPath(_moduleId, 108, "This extension has been disabled because it does not support virtual workspaces."))
      }, true);
      return;
    }
    if (isVirtualWorkspace(this.contextService.getWorkspace())) {
      ...
      if (virtualSupportType === 'limited' || details) {
        this.updateStatus({
          icon: warningIcon,
          message: new MarkdownString(details ? escapeMarkdownSyntaxTokens(details) : localizeWithPath(_moduleId, 109, "This extension has limited features because the current workspace is virtual."))
        }, true);
        return;
      }
    }
    if (this.extension.enablementState === 0 || this.extension.enablementState === 5 && this.workbenchExtensionEnablementService.getDependenciesEnablementStates(this.extension.local).every(([, enablementState]) => this.workbenchExtensionEnablementService.isEnabledEnablementState(enablementState) || enablementState === 0)) {
      ...
      this.updateStatus({
        icon: trustIcon,
        message: new MarkdownString(untrustedDetails ? escapeMarkdownSyntaxTokens(untrustedDetails) : localizeWithPath(_moduleId, 110, "This extension has been disabled because the current workspace is not trusted."))
      }, true);
      return;
    }
    if (this.workspaceTrustEnablementService.isWorkspaceTrustEnabled() && !this.workspaceTrustService.isWorkspaceTrusted()) {
      ...
      if (untrustedSupportType === 'limited' || untrustedDetails) {
        ...
        this.updateStatus({
          icon: trustIcon,
          message: new MarkdownString(untrustedDetails ? escapeMarkdownSyntaxTokens(untrustedDetails) : localizeWithPath(_moduleId, 111, "This extension has limited features because the current workspace is not trusted."))
        }, true);
        return;
      }
    }
        ...
        if (this.extensionManagementServerService.localExtensionManagementServer === this.extension.server) {
          if (this.extensionManifestPropertiesService.prefersExecuteOnWorkspace(this.extension.local.manifest)) {
            if (this.extensionManagementServerService.remoteExtensionManagementServer) {
              message = new MarkdownString(`${localizeWithPath(_moduleId, 112, "This extension is disabled in this workspace because it is defined to run in the Remote Extension Host. Please install the extension in '{0}' to enable.", this.extensionManagementServerService.remoteExtensionManagementServer.label)} [${localizeWithPath(_moduleId, 103, "Learn More")}](https://code.visualstudio.com/api/advanced-topics/remote-extensions#architecture-and-extension-kinds)`);
            }
          }
          ...
          if (this.extensionManifestPropertiesService.prefersExecuteOnUI(this.extension.local.manifest)) {
            if (this.extensionManagementServerService.localExtensionManagementServer) {
              message = new MarkdownString(`${localizeWithPath(_moduleId, 113, "This extension is disabled in this workspace because it is defined to run in the Local Extension Host. Please install the extension locally to enable.", this.extensionManagementServerService.remoteExtensionManagementServer.label)} [${localizeWithPath(_moduleId, 103, "Learn More")}](https://code.visualstudio.com/api/advanced-topics/remote-extensions#architecture-and-extension-kinds)`);
            } else if (isWeb) {
              message = new MarkdownString(`${localizeWithPath(_moduleId, 114, "This extension is disabled because it is defined to run only in {0} for the Desktop.", this.productService.nameLong)} [${localizeWithPath(_moduleId, 103, "Learn More")}](https://code.visualstudio.com/api/advanced-topics/remote-extensions#architecture-and-extension-kinds)`);
            }
          }
        } else if (this.extensionManagementServerService.webExtensionManagementServer === this.extension.server) {
          message = new MarkdownString(`${localizeWithPath(_moduleId, 115, "This extension is disabled because it is not supported in {0} for the Web.", this.productService.nameLong)} [${localizeWithPath(_moduleId, 103, "Learn More")}](https://code.visualstudio.com/api/advanced-topics/remote-extensions#architecture-and-extension-kinds)`);
        }
        if (message) {
          this.updateStatus({
            icon: warningIcon,
    ...
    for (const feature of features) {
      ...
      const manageAccessLink = `[${localizeWithPath(_moduleId, 116, 'Manage Access')}](${URI.parse(`command:extension.open?${encodeURIComponent(JSON.stringify([this.extension.identifier.id, "features", false, feature.id]))}`)})`;
    ...
    if (this.extensionManagementServerService.remoteExtensionManagementServer) {
      if (isLanguagePackExtension(this.extension.local.manifest)) {
        if (!this.extensionsWorkbenchService.installed.some(e => areSameExtensions(e.identifier, this.extension.identifier) && e.server !== this.extension.server)) {
          const message = this.extension.server === this.extensionManagementServerService.localExtensionManagementServer ? new MarkdownString(localizeWithPath(_moduleId, 117, "Install the language pack extension on '{0}' to enable it there also.", this.extensionManagementServerService.remoteExtensionManagementServer.label)) : new MarkdownString(localizeWithPath(_moduleId, 118, "Install the language pack extension locally to enable it there also."));
          this.updateStatus({
            icon: infoIcon,
            message
          }, true);
      ...
      if (this.extension.server === this.extensionManagementServerService.localExtensionManagementServer && runningExtensionServer === this.extensionManagementServerService.remoteExtensionManagementServer) {
        if (this.extensionManifestPropertiesService.prefersExecuteOnWorkspace(this.extension.local.manifest)) {
          this.updateStatus({
            icon: infoIcon,
            message: new MarkdownString(`${localizeWithPath(_moduleId, 119, "This extension is enabled in the Remote Extension Host because it prefers to run there.")} [${localizeWithPath(_moduleId, 103, "Learn More")}](https://code.visualstudio.com/api/advanced-topics/remote-extensions#architecture-and-extension-kinds)`)
          }, true);
        }
        return;
      }
      if (this.extension.server === this.extensionManagementServerService.remoteExtensionManagementServer && runningExtensionServer === this.extensionManagementServerService.localExtensionManagementServer) {
        if (this.extensionManifestPropertiesService.prefersExecuteOnUI(this.extension.local.manifest)) {
          this.updateStatus({
            icon: infoIcon,
            message: new MarkdownString(`${localizeWithPath(_moduleId, 120, "This extension is enabled in the Local Extension Host because it prefers to run there.")} [${localizeWithPath(_moduleId, 103, "Learn More")}](https://code.visualstudio.com/api/advanced-topics/remote-extensions#architecture-and-extension-kinds)`)
          }, true);
        }
        return;
      }
      if (this.extension.server === this.extensionManagementServerService.remoteExtensionManagementServer && runningExtensionServer === this.extensionManagementServerService.webExtensionManagementServer) {
        if (this.extensionManifestPropertiesService.canExecuteOnWeb(this.extension.local.manifest)) {
          this.updateStatus({
            icon: infoIcon,
            message: new MarkdownString(`${localizeWithPath(_moduleId, 121, "This extension is enabled in the Web Worker Extension Host because it prefers to run there.")} [${localizeWithPath(_moduleId, 103, "Learn More")}](https://code.visualstudio.com/api/advanced-topics/remote-extensions#architecture-and-extension-kinds)`)
          }, true);
        }
        return;
      }
    ...
    if (this.extension.enablementState === 5) {
      this.updateStatus({
        icon: warningIcon,
        message: new MarkdownString(localizeWithPath(_moduleId, 122, "This extension has been disabled because it depends on an extension that is disabled."))
      }, true);
      return;
    }
    ...
    if (!this.extension.isWorkspaceScoped && isEnabled && isRunning) {
      if (this.extension.enablementState === 9) {
        this.updateStatus({
          message: new MarkdownString(localizeWithPath(_moduleId, 123, "This extension is enabled for this workspace by the user."))
        }, true);
        return;
      }
      if (this.extensionManagementServerService.localExtensionManagementServer && this.extensionManagementServerService.remoteExtensionManagementServer) {
        if (this.extension.server === this.extensionManagementServerService.remoteExtensionManagementServer) {
          this.updateStatus({
            message: new MarkdownString(localizeWithPath(_moduleId, 124, "Extension is enabled on '{0}'", this.extension.server.label))
          }, true);
          return;
        }
      }
      if (this.extension.enablementState === 8) {
        this.updateStatus({
          message: new MarkdownString(localizeWithPath(_moduleId, 125, "This extension is enabled globally."))
        }, true);
        return;
      }
    }
    if (!isEnabled && !isRunning) {
      if (this.extension.enablementState === 6) {
        this.updateStatus({
          message: new MarkdownString(localizeWithPath(_moduleId, 126, "This extension is disabled globally by the user."))
        }, true);
        return;
      }
      if (this.extension.enablementState === 7) {
        this.updateStatus({
          message: new MarkdownString(localizeWithPath(_moduleId, 127, "This extension is disabled for this workspace by the user."))
        }, true);
        return;
      }
    }
...
let ReinstallAction = class ReinstallAction extends Action {
  ...
  run() {
    return this.quickInputService.pick(this.getEntries(), {
      placeHolder: localizeWithPath(_moduleId, 129, "Select Extension to Reinstall")
  ...
  reinstallExtension(extension) {
    return this.instantiationService.createInstance(SearchExtensionsAction, '@installed ').run().then(() => {
      return this.extensionsWorkbenchService.reinstall(extension).then(extension => {
        const requireReload = !(extension.local && this.extensionService.canAddExtension(toExtensionDescription(extension.local)));
        const message = requireReload ? localizeWithPath(_moduleId, 130, "Please reload Visual Studio Code to complete reinstalling the extension {0}.", extension.identifier.id) : localizeWithPath(_moduleId, 131, "Reinstalling the extension {0} is completed.", extension.identifier.id);
        const actions = requireReload ? [{
          label: localizeWithPath(_moduleId, 132, "Reload Now"),
          run: () => this.hostService.reload()
        }] : [];
        this.notificationService.prompt(Severity$1.Info, message, actions, {
          sticky: true
        });
...
ReinstallAction = ReinstallAction_1 = __decorate([__param(2, IExtensionsWorkbenchService), __param(3, IExtensionManagementServerService), __param(4, IQuickInputService), __param(5, INotificationService), __param(6, IHostService), __param(7, IInstantiationService), __param(8, IExtensionService)], ReinstallAction);
let InstallSpecificVersionOfExtensionAction = class InstallSpecificVersionOfExtensionAction extends Action {
  ...
  static {
    this.LABEL = localizeWithPath(_moduleId, 133, "Install Specific Version of Extension...");
  }
  ...
  async run() {
    const extensionPick = await this.quickInputService.pick(this.getExtensionEntries(), {
      placeHolder: localizeWithPath(_moduleId, 134, "Select Extension"),
      matchOnDetail: true
    });
...
let AbstractInstallExtensionsInServerAction = class AbstractInstallExtensionsInServerAction extends Action {
  ...
  async selectAndInstallExtensions() {
    ...
    if (localExtensionsToInstall.length) {
      ...
      quickPick.placeholder = localizeWithPath(_moduleId, 135, "Select extensions to install");
      ...
      this.notificationService.notify({
        ...
        message: localizeWithPath(_moduleId, 136, "There are no extensions to install.")
      });
    }
  }
    ...
    if (selectedItems.length) {
      ...
      if (localExtensionsToInstall.length) {
        await this.progressService.withProgress({
          location: 15,
          title: localizeWithPath(_moduleId, 137, "Installing Extensions...")
        }, () => this.installExtensions(localExtensionsToInstall));
        this.notificationService.info(localizeWithPath(_moduleId, 138, "Successfully installed extensions."));
      }
    }
  }
};
...
let InstallLocalExtensionsInRemoteAction = class InstallLocalExtensionsInRemoteAction extends AbstractInstallExtensionsInServerAction {
  ...
  get label() {
    if (this.extensionManagementServerService && this.extensionManagementServerService.remoteExtensionManagementServer) {
      return localizeWithPath(_moduleId, 139, "Install Local Extensions in '{0}'...", this.extensionManagementServerService.remoteExtensionManagementServer.label);
    }
    return '';
  }
  getQuickPickTitle() {
    return localizeWithPath(_moduleId, 140, "Install Local Extensions in '{0}'", this.extensionManagementServerService.remoteExtensionManagementServer.label);
  }
...
let InstallRemoteExtensionsInLocalAction = class InstallRemoteExtensionsInLocalAction extends AbstractInstallExtensionsInServerAction {
  ...
  get label() {
    return localizeWithPath(_moduleId, 141, "Install Remote Extensions Locally...");
  }
  getQuickPickTitle() {
    return localizeWithPath(_moduleId, 142, "Install Remote Extensions Locally");
  }
  getExtensionsToInstall(local) {
...
CommandsRegistry.registerCommand('workbench.extensions.action.showExtensionsForLanguage', function (accessor, fileExtension) {
  ...
  return paneCompositeService.openPaneComposite(VIEWLET_ID, 0, true).then(viewlet => viewlet?.getViewPaneContainer()).then(viewlet => {
...
registerColor('extensionButton.background', {
  ...
  hcDark: null,
  hcLight: null
}, localizeWithPath(_moduleId, 143, "Button background color for extension actions."));
registerColor('extensionButton.foreground', {
  dark: buttonForeground,
  light: buttonForeground,
  hcDark: null,
  hcLight: null
}, localizeWithPath(_moduleId, 144, "Button foreground color for extension actions."));
registerColor('extensionButton.hoverBackground', {
  dark: buttonHoverBackground,
  light: buttonHoverBackground,
  hcDark: null,
  hcLight: null
}, localizeWithPath(_moduleId, 145, "Button background hover color for extension actions."));
...
}, localizeWithPath(_moduleId, 146, "Button separator color for extension actions"));
const extensionButtonProminentBackground = registerColor('extensionButton.prominentBackground', {
  dark: buttonBackground,
  light: buttonBackground,
  hcDark: null,
  hcLight: null
}, localizeWithPath(_moduleId, 147, "Button background color for extension actions that stand out (e.g. install button)."));
registerColor('extensionButton.prominentForeground', {
  dark: buttonForeground,
  light: buttonForeground,
  hcDark: null,
  hcLight: null
}, localizeWithPath(_moduleId, 148, "Button foreground color for extension actions that stand out (e.g. install button)."));
registerColor('extensionButton.prominentHoverBackground', {
  dark: buttonHoverBackground,
  light: buttonHoverBackground,
  hcDark: null,
  hcLight: null
}, localizeWithPath(_moduleId, 149, "Button background hover color for extension actions that stand out (e.g. install button)."));
...
exports.extensionButtonProminentBackground = extensionButtonProminentBackground;
exports.getContextMenuActions = getContextMenuActions;
exports.showExtensionsWithIdsCommandId = showExtensionsWithIdsCommandId;