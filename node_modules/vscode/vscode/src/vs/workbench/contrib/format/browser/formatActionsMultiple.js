...
} = require('../../../services/languageStatus/common/languageStatusService.service.js');
const {
  IEditorService
...
const _moduleId = "vs/workbench/contrib/format/browser/formatActionsMultiple";
let DefaultFormatter = class DefaultFormatter extends Disposable {
  ...
  async _updateConfigValues() {
    ...
    DefaultFormatter_1.extensionItemLabels.push(localizeWithPath(_moduleId, 0, 'None'));
    DefaultFormatter_1.extensionDescriptions.push(localizeWithPath(_moduleId, 1, "None"));
  ...
  async _analyzeFormatter(kind, formatter, document) {
    ...
    if (defaultFormatterId) {
      ...
      if (extension && this._extensionEnablementService.isEnabled(toExtension(extension))) {
        ...
        const detail = kind === 1 ? localizeWithPath(_moduleId, 2, "Extension '{0}' is configured as formatter but it cannot format '{1}'-files", extension.displayName || extension.name, langName) : localizeWithPath(_moduleId, 3, "Extension '{0}' is configured as formatter but it can only format '{1}'-files as a whole, not selections or parts of it.", extension.displayName || extension.name, langName);
        return detail;
      }
    ...
    const langName = this._languageService.getLanguageName(document.getLanguageId()) || document.getLanguageId();
    const message = !defaultFormatterId ? localizeWithPath(_moduleId, 4, "There are multiple formatters for '{0}' files. One of them should be configured as default formatter.", DefaultFormatter_1._maybeQuotes(langName)) : localizeWithPath(_moduleId, 5, "Extension '{0}' is configured as formatter but not available. Select a different default formatter to continue.", defaultFormatterId);
    return message;
  }
  async _selectFormatter(formatter, document, mode, kind) {
    ...
    if (mode !== 2) {
      const {
        ...
        message: localizeWithPath(_moduleId, 6, "Configure Default Formatter"),
        detail: formatterOrMessage,
        primaryButton: localizeWithPath(_moduleId, 7, "&&Configure...")
      });
      if (confirmed) {
      ...
      this._notificationService.prompt(Severity$1.Info, formatterOrMessage, [{
        label: localizeWithPath(_moduleId, 8, "Configure..."),
  ...
  async _pickAndPersistDefaultFormatter(formatter, document) {
    ...
    const pick = await this._quickInputService.pick(picks, {
      placeHolder: localizeWithPath(_moduleId, 9, "Select a default formatter for '{0}' files", DefaultFormatter_1._maybeQuotes(langName))
    });
  ...
  _updateStatus() {
    ...
    this._analyzeFormatter(1, formatter, document).then(result => {
      ...
      const command = {
        ...
        title: localizeWithPath(_moduleId, 10, "Configure...")
      };
      ...
      this._languageStatusStore.add(this._languageStatusService.addStatus({
        ...
        name: localizeWithPath(_moduleId, 11, "Formatter Conflicts"),
        selector: {
          language: document.getLanguageId(),
          pattern: document.uri.fsPath
        },
        severity: Severity$1.Error,
        label: localizeWithPath(_moduleId, 12, "Formatting"),
        detail: result,
        busy: false,
        source: '',
        command,
...
Registry.as(Extensions$1.Configuration).registerConfiguration({
  ...
  properties: {
    [DefaultFormatter.configName]: {
      description: localizeWithPath(_moduleId, 13, "Defines a default formatter which takes precedence over all other formatter settings. Must be the identifier of an extension contributing a formatter."),
      type: ['string', 'null'],
      default: null,
...
async function showFormatterPick(accessor, model, formatters) {
  ...
  const picks = formatters.map((provider, index) => {
    ...
    const pick = {
      ...
      description: isDefault ? localizeWithPath(_moduleId, 14, "(default)") : undefined
    };
    if (isDefault) {
  ...
  const configurePick = {
    label: localizeWithPath(_moduleId, 15, "Configure Default Formatter...")
  };
  const pick = await quickPickService.pick([...picks, {
    type: 'separator'
  }, configurePick], {
    placeHolder: localizeWithPath(_moduleId, 16, "Select a formatter"),
  ...
  if (!pick) {
    ...
    const pick = await quickPickService.pick(picks, {
      placeHolder: localizeWithPath(_moduleId, 9, "Select a default formatter for '{0}' files", DefaultFormatter._maybeQuotes(langName))
    });
...
registerEditorAction(class FormatDocumentMultipleAction extends EditorAction {
  constructor() {
    super({
      ...
      label: localizeWithPath(_moduleId, 17, "Format Document With..."),
...
registerEditorAction(class FormatSelectionMultipleAction extends EditorAction {
  constructor() {
    super({
      ...
      label: localizeWithPath(_moduleId, 18, "Format Selection With..."),
      alias: 'Format Code...',
