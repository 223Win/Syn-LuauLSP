...
let CommentsMenus = class CommentsMenus {
  ...
  getActions(menuId, element) {
    ...
    const overlay = [['commentController', element.owner], ['resourceScheme', element.resource.scheme], ['commentThread', element.contextValue], ['canReply', element.thread.canReply]];
...
let CommentNodeRenderer = class CommentNodeRenderer {
  ...
  renderTemplate(container) {
    ...
    const threadMetadata = {
      ...
      timestamp: new TimestampWidget(this.configurationService, this.hoverService, append(metadata, $('.timestamp-container'))),
    ...
    const repliesMetadata = {
      ...
      timestamp: new TimestampWidget(this.configurationService, this.hoverService, append(snippetContainer, $('.timestamp-container')))
    };
  ...
  renderElement(node, index, templateData, height) {
    ...
    if (node.element.range) {
      if (node.element.range.startLineNumber === node.element.range.endLineNumber) {
        ...
        templateData.threadMetadata.range.textContent = localizeWithPath(_moduleId, 8, "[Ln {0}-{1}]", node.element.range.startLineNumber, node.element.range.endLineNumber);
      }
    }
...
CommentNodeRenderer = __decorate([__param(2, IOpenerService), __param(3, IConfigurationService), __param(4, IHoverService), __param(5, IThemeService)], CommentNodeRenderer);
class Filter {
  constructor(options) {
...
let CommentsList = class CommentsList extends WorkbenchObjectTree {
  constructor(labels, container, options, contextKeyService, listService, instantiationService, configurationService, contextMenuService, keybindingService) {
    ...
    const renderers = [instantiationService.createInstance(ResourceWithCommentsRenderer, labels), instantiationService.createInstance(CommentNodeRenderer, actionViewItemProvider, menus)];
...
exports.COMMENTS_VIEW_ID = COMMENTS_VIEW_ID;
exports.COMMENTS_VIEW_STORAGE_ID = COMMENTS_VIEW_STORAGE_ID;
exports.COMMENTS_VIEW_TITLE = COMMENTS_VIEW_TITLE;
exports.CommentNodeRenderer = CommentNodeRenderer;
exports.CommentsList = CommentsList;
exports.CommentsMenus = CommentsMenus;
