...
const {} = require('./media/panel.css.js');
const {
  localizeWithPath
} = require('../../../../nls.js');
const {
...
const {
  ICommentService
} = require('./commentService.service.js');
const {
  IEditorService
...
const {
  CommentsFilters
} = require('./commentsViewActions.js');
const {
  Memento
} = require('../../../common/memento.js');
const {
  IStorageService
} = require('../../../../platform/storage/common/storage.service.js');
const {
  FilterOptions
} = require('./commentsFilterOptions.js');
const {
...
const {
  revealCommentThread
} = require('./commentsController.js');
const {
  registerNavigableContainer
} = require('../../../browser/actions/widgetNavigationCommands.js');
const {
  CommentsModel
} = require('./commentsModel.js');
const {
  IHoverService
...
function createResourceCommentsIterator(model) {
  return Iterable.map(model.resourceCommentThreads, m => {
    ...
    const children = Iterable.map(CommentNodeIt, r => ({
      element: r
    }));
    return {
      element: m,
...
let CommentsPanel = class CommentsPanel extends FilterViewPane {
  ...
  getScreenReaderInfoForNode(element, forAriaLabel) {
    ...
    if (forAriaLabel && this.configurationService.getValue("accessibility.verbosity.comments")) {
      ...
      accessibleViewHint = kbLabel ? localizeWithPath(_moduleId, 3, "Inspect this in the accessible view ({0}).\n", kbLabel) : localizeWithPath(_moduleId, 4, "Inspect this in the accessible view via the command Open Accessible View which is currently not triggerable via keybinding.\n");
    }
    ...
    if (element.range) {
      if (element.threadRelevance === CommentThreadApplicability.Outdated) {
        return accessibleViewHint + localizeWithPath(_moduleId, 5, "Outdated from {0} at line {1} column {2} in {3},{4} comment: {5}", element.comment.userName, element.range.startLineNumber, element.range.startColumn, basename(element.resource), replyCount, typeof element.comment.body === 'string' ? element.comment.body : element.comment.body.value) + replies;
      } else {
        return accessibleViewHint + localizeWithPath(_moduleId, 6, "{0} at line {1} column {2} in {3},{4} comment: {5}", element.comment.userName, element.range.startLineNumber, element.range.startColumn, basename(element.resource), replyCount, typeof element.comment.body === 'string' ? element.comment.body : element.comment.body.value) + replies;
      }
    } else {
      if (element.threadRelevance === CommentThreadApplicability.Outdated) {
        return accessibleViewHint + localizeWithPath(_moduleId, 7, "Outdated from {0} in {1},{2} comment: {3}", element.comment.userName, basename(element.resource), replyCount, typeof element.comment.body === 'string' ? element.comment.body : element.comment.body.value) + replies;
      } else {
        return accessibleViewHint + localizeWithPath(_moduleId, 8, "{0} in {1},{2} comment: {3}", element.comment.userName, basename(element.resource), replyCount, typeof element.comment.body === 'string' ? element.comment.body : element.comment.body.value) + replies;
      }
    }
  }
  getRepliesAsString(node, forAriaLabel) {
    ...
    return '\n' + node.replies.map(reply => localizeWithPath(_moduleId, 9, "{0} {1}", reply.comment.userName, typeof reply.comment.body === 'string' ? reply.comment.body : reply.comment.body.value)).join('\n');
  }
      ...
      accessibilityProvider: {
        getAriaLabel: element => {
          ...
          if (element instanceof ResourceWithCommentThreads) {
            return localizeWithPath(_moduleId, 12, "Comments in {0}, full path {1}", basename(element.resource), element.resource.fsPath);
          }
