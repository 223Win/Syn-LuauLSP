...
const {} = require('./media/review.css.js');
const {
  isDiffEditor,
  isCodeEditor
...
const {
  EditorType
} = require('../../../../editor/common/editorCommon.js');
const {
...
} = require('./commentThreadZoneWidget.js');
const {
  ACTIVE_GROUP,
  SIDE_GROUP
...
} = require('./commentThreadRangeDecorator.js');
const {
  status
...
const _moduleId = "vs/workbench/contrib/comments/browser/commentsController";
...
let CommentController = class CommentController {
  ...
  onEditorChangeCursorPosition(e) {
    const decorations = e ? this.editor?.getDecorationsInRange(Range.fromPositions(e, {
      column: -1,
  ...
  _findNearestCommentingRange(reverse) {
    ...
    if (this.accessibilityService.isScreenReaderOptimized()) {
      ...
      if (commentRangeStart && commentRangeEnd) {
        ...
        oneLine ? status(localizeWithPath(_moduleId, 0, "Line {0}", commentRangeStart)) : status(localizeWithPath(_moduleId, 1, "Lines {0} to {1}", commentRangeStart, commentRangeEnd));
      }
    }
  }
  nextCommentingRange() {
  ...
  onModelChanged() {
    ...
    this.localToDispose.add(this.commentService.onDidUpdateCommentThreads(async e => {
      ...
      for (const thread of added) {
        ...
        const pendingCommentText = (this._pendingNewCommentCache[e.uniqueOwner] && this._pendingNewCommentCache[e.uniqueOwner][thread.threadId]) ?? continueOnCommentText;
      ...
      if (!this._hasRespondedToEditorChange) {
        if (this._commentInfos.some(commentInfo => commentInfo.commentingRanges.ranges.length > 0 || commentInfo.commentingRanges.fileComments)) {
          ...
          if (verbose) {
            ...
            if (keybinding) {
              status(localizeWithPath(_moduleId, 2, "Editor has commenting ranges, run the command Open Accessibility Help ({0}), for more information.", keybinding));
            } else {
              status(localizeWithPath(_moduleId, 3, "Editor has commenting ranges, run the command Open Accessibility Help, which is currently not triggerable via keybinding, for more information."));
            }
          } else {
            status(localizeWithPath(_moduleId, 4, "Editor has commenting ranges."));
          }
        }
      }
    });
    ...
    if (newCommentInfos.length > 1) {
      if (e && range) {
        ...
        return this.quickInputService.pick(picks, {
          placeHolder: localizeWithPath(_moduleId, 5, "Select Comment Provider"),
