...
const {
  isEqual
} = require('../../../../base/common/extpath.js');
const {
  posix
...
} = require('./explorerFileNestingTrie.js');
const {
  assertIsDefined
...
class ExplorerModel {
  constructor(contextService, uriIdentityService, fileService, configService, filesConfigService) {
    ...
    const setRoots = () => this._roots = this.contextService.getWorkspace().folders.map(folder => new ExplorerItem(folder.uri, fileService, configService, filesConfigService, undefined, true, false, false, false, folder.name));
    setRoots();
...
class ExplorerItem {
  ...
  get fileNester() {
    if (!this.root._fileNester) {
      ...
      const patterns = Object.entries(nestingConfig.patterns).filter(entry => typeof entry[0] === 'string' && typeof entry[1] === 'string' && entry[0] && entry[1]).map(([parentPattern, childrenPatterns]) => [this.getPlatformAwareName(parentPattern.trim()), childrenPatterns.split(',').map(p => this.getPlatformAwareName(p.trim().replace(/\u200b/g, '').trim())).filter(p => p !== '')]);
...
ExplorerItem.__decorator = __decorate([memoize], ExplorerItem.prototype, "children", null);
