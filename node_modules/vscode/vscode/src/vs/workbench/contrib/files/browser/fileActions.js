...
} = require('../../../services/workingCopy/common/workingCopyFileService.service.js');
const {
  Codicon
...
const {
  FileDownload,
  BrowserFileUpload
} = require('./fileImportExport.js');
const {
  IPaneCompositePartService
} = require('../../../services/panecomposite/browser/panecomposite.service.js');
const {
  IRemoteAgentService
} = require('../../../services/remote/common/remoteAgentService.service.js');
const {
  IPathService
...
const _moduleId = "vs/workbench/contrib/files/browser/fileActions";
const NEW_FILE_COMMAND_ID = 'explorer.newFile';
const NEW_FILE_LABEL = localize2WithPath(_moduleId, 0, "New File...");
const NEW_FOLDER_COMMAND_ID = 'explorer.newFolder';
const NEW_FOLDER_LABEL = localize2WithPath(_moduleId, 1, "New Folder...");
const TRIGGER_RENAME_LABEL = localizeWithPath(_moduleId, 2, "Rename...");
const MOVE_FILE_TO_TRASH_LABEL = localizeWithPath(_moduleId, 3, "Delete");
const COPY_FILE_LABEL = localizeWithPath(_moduleId, 4, "Copy");
const PASTE_FILE_LABEL = localizeWithPath(_moduleId, 5, "Paste");
const FileCopiedContext = new RawContextKey('fileCopied', false);
const DOWNLOAD_COMMAND_ID = 'explorer.download';
const DOWNLOAD_LABEL = localizeWithPath(_moduleId, 6, "Download...");
const UPLOAD_COMMAND_ID = 'explorer.upload';
const UPLOAD_LABEL = localizeWithPath(_moduleId, 7, "Upload...");
...
async function deleteFiles(explorerService, workingCopyFileService, dialogService, configurationService, elements, useTrash, skipConfirm = false, ignoreIfNotExists = false) {
  let primaryButton;
  if (useTrash) {
    primaryButton = isWindows ? localizeWithPath(_moduleId, 8, "&&Move to Recycle Bin") : localizeWithPath(_moduleId, 9, "&&Move to Trash");
  } else {
    primaryButton = localizeWithPath(_moduleId, 10, "&&Delete");
  }
  ...
  if (dirtyWorkingCopies.size) {
    ...
    if (distinctElements.length > 1) {
      message = localizeWithPath(_moduleId, 11, "You are deleting files with unsaved changes. Do you want to continue?");
    } else if (distinctElements[0].isDirectory) {
      if (dirtyWorkingCopies.size === 1) {
        message = localizeWithPath(_moduleId, 12, "You are deleting a folder {0} with unsaved changes in 1 file. Do you want to continue?", distinctElements[0].name);
      } else {
        message = localizeWithPath(_moduleId, 13, "You are deleting a folder {0} with unsaved changes in {1} files. Do you want to continue?", distinctElements[0].name, dirtyWorkingCopies.size);
      }
    } else {
      message = localizeWithPath(_moduleId, 14, "You are deleting {0} with unsaved changes. Do you want to continue?", distinctElements[0].name);
    }
    const response = await dialogService.confirm({
      type: 'warning',
      message,
      detail: localizeWithPath(_moduleId, 15, "Your changes will be lost if you don't save them."),
      primaryButton
    });
  ...
  let confirmation;
  const deleteDetail = distinctElements.some(e => e.isDirectory) ? localizeWithPath(_moduleId, 16, "This action is irreversible!") : distinctElements.length > 1 ? localizeWithPath(_moduleId, 17, "You can restore these files using the Undo command.") : localizeWithPath(_moduleId, 18, "You can restore this file using the Undo command.");
  if (skipConfirm || useTrash && configurationService.getValue(CONFIRM_DELETE_SETTING_KEY) === false) {
    ...
    detail += detail ? '\n' : '';
    if (isWindows) {
      detail += distinctElements.length > 1 ? localizeWithPath(_moduleId, 19, "You can restore these files from the Recycle Bin.") : localizeWithPath(_moduleId, 20, "You can restore this file from the Recycle Bin.");
    } else {
      detail += distinctElements.length > 1 ? localizeWithPath(_moduleId, 21, "You can restore these files from the Trash.") : localizeWithPath(_moduleId, 22, "You can restore this file from the Trash.");
    }
    confirmation = await dialogService.confirm({
      ...
      primaryButton,
      checkbox: {
        label: localizeWithPath(_moduleId, 23, "Do not ask me again")
      }
    });
  } else {
    let {
  ...
  try {
    ...
    const options = {
      undoLabel: distinctElements.length > 1 ? localizeWithPath(_moduleId, 24, "Delete {0} files", distinctElements.length) : localizeWithPath(_moduleId, 25, "Delete {0}", distinctElements[0].name),
      progressLabel: distinctElements.length > 1 ? localizeWithPath(_moduleId, 26, "Deleting {0} files", distinctElements.length) : localizeWithPath(_moduleId, 27, "Deleting {0}", distinctElements[0].name)
    };
    ...
    let primaryButton;
    if (useTrash) {
      errorMessage = isWindows ? localizeWithPath(_moduleId, 28, "Failed to delete using the Recycle Bin. Do you want to permanently delete instead?") : localizeWithPath(_moduleId, 29, "Failed to delete using the Trash. Do you want to permanently delete instead?");
      detailMessage = deleteDetail;
      primaryButton = localizeWithPath(_moduleId, 30, "&&Delete Permanently");
    } else {
      errorMessage = toErrorMessage(error, false);
      primaryButton = localizeWithPath(_moduleId, 31, "&&Retry");
    }
...
function getMoveToTrashMessage(distinctElements) {
  if (containsBothDirectoryAndFile(distinctElements)) {
    return {
      message: localizeWithPath(_moduleId, 32, "Are you sure you want to delete the following {0} files/directories and their contents?", distinctElements.length),
  ...
  if (distinctElements.length > 1) {
    if (distinctElements[0].isDirectory) {
      return {
        message: localizeWithPath(_moduleId, 33, "Are you sure you want to delete the following {0} directories and their contents?", distinctElements.length),
        detail: getFileNamesMessage(distinctElements.map(e => e.resource))
      };
    }
    return {
      message: localizeWithPath(_moduleId, 34, "Are you sure you want to delete the following {0} files?", distinctElements.length),
      detail: getFileNamesMessage(distinctElements.map(e => e.resource))
    };
  }
  if (distinctElements[0].isDirectory && !distinctElements[0].isSymbolicLink) {
    return {
      message: localizeWithPath(_moduleId, 35, "Are you sure you want to delete '{0}' and its contents?", distinctElements[0].name),
      detail: ''
    };
  }
  return {
    message: localizeWithPath(_moduleId, 36, "Are you sure you want to delete '{0}'?", distinctElements[0].name),
    detail: ''
  };
}
function getDeleteMessage(distinctElements) {
  if (containsBothDirectoryAndFile(distinctElements)) {
    return {
      message: localizeWithPath(_moduleId, 37, "Are you sure you want to permanently delete the following {0} files/directories and their contents?", distinctElements.length),
      detail: getFileNamesMessage(distinctElements.map(e => e.resource))
    };
  }
  if (distinctElements.length > 1) {
    if (distinctElements[0].isDirectory) {
      return {
        message: localizeWithPath(_moduleId, 38, "Are you sure you want to permanently delete the following {0} directories and their contents?", distinctElements.length),
        detail: getFileNamesMessage(distinctElements.map(e => e.resource))
      };
    }
    return {
      message: localizeWithPath(_moduleId, 39, "Are you sure you want to permanently delete the following {0} files?", distinctElements.length),
      detail: getFileNamesMessage(distinctElements.map(e => e.resource))
    };
  }
  if (distinctElements[0].isDirectory) {
    return {
      message: localizeWithPath(_moduleId, 40, "Are you sure you want to permanently delete '{0}' and its contents?", distinctElements[0].name),
      detail: ''
    };
  }
  return {
    message: localizeWithPath(_moduleId, 41, "Are you sure you want to permanently delete '{0}'?", distinctElements[0].name),
    detail: ''
  };
}
...
async function askForOverwrite(fileService, dialogService, targetResource) {
  ...
  const {
    ...
    message: localizeWithPath(_moduleId, 42, "A file or folder with the name '{0}' already exists in the destination folder. Do you want to replace it?", basename$1(targetResource.path)),
    primaryButton: localizeWithPath(_moduleId, 43, "&&Replace")
  });
  return confirmed;
}
class GlobalCompareResourcesAction extends Action2 {
  ...
  static {
    this.LABEL = localize2WithPath(_moduleId, 44, "Compare Active File With...");
  }
  constructor() {
    super({
      ...
      metadata: {
        description: localize2WithPath(_moduleId, 45, "Opens a picker to select a file to diff with the active editor.")
      }
    });
  }
  async run(accessor) {
...
class ToggleAutoSaveAction extends Action2 {
  ...
  constructor() {
    super({
      ...
      title: localize2WithPath(_moduleId, 46, "Toggle Auto Save"),
      f1: true,
      category: Categories.File,
      metadata: {
        description: localize2WithPath(_moduleId, 47, "Toggle the ability to save files automatically after typing")
      }
    });
  }
  run(accessor) {
...
class SaveAllInGroupAction extends BaseSaveAllAction {
  ...
  static {
    this.LABEL = localizeWithPath(_moduleId, 48, "Save All in Group");
  }
  get class() {
...
let CloseGroupAction = class CloseGroupAction extends Action {
  ...
  static {
    this.LABEL = localizeWithPath(_moduleId, 49, "Close Group");
  }
...
class FocusFilesExplorer extends Action2 {
  ...
  static {
    this.LABEL = localize2WithPath(_moduleId, 50, "Focus on Files Explorer");
  }
  constructor() {
    super({
      ...
      category: Categories.File,
      metadata: {
        description: localize2WithPath(_moduleId, 51, "Moves focus to the file explorer view container.")
      }
    });
  }
  async run(accessor) {
...
class ShowActiveFileInExplorer extends Action2 {
  ...
  static {
    this.LABEL = localize2WithPath(_moduleId, 52, "Reveal Active File in Explorer View");
  }
  constructor() {
    super({
      ...
      category: Categories.File,
      metadata: {
        description: localize2WithPath(_moduleId, 53, "Reveals and selects the active file within the explorer view.")
      }
    });
  }
  async run(accessor) {
...
class OpenActiveFileInEmptyWorkspace extends Action2 {
  ...
  static {
    this.LABEL = localize2WithPath(_moduleId, 54, "Open Active File in New Empty Workspace");
  }
  constructor() {
    super({
      ...
      metadata: {
        description: localize2WithPath(_moduleId, 55, "Opens the active file in a new window with no folders open.")
      }
    });
  }
  async run(accessor) {
    ...
    if (fileResource) {
      if (fileService.hasProvider(fileResource)) {
        ...
        dialogService.error(localizeWithPath(_moduleId, 56, "The active editor must contain an openable resource."));
      }
    }
  }
}
function validateFileName(pathService, item, name, os) {
  ...
  if (!name || name.length === 0 || /^\s+$/.test(name)) {
    return {
      content: localizeWithPath(_moduleId, 57, "A file or folder name must be provided."),
      severity: Severity$1.Error
    };
  }
  if (name[0] === '/' || name[0] === '\\') {
    return {
      content: localizeWithPath(_moduleId, 58, "A file or folder name cannot start with a slash."),
      severity: Severity$1.Error
    };
  }
  ...
  if (name !== item.name) {
    ...
    if (child && child !== item) {
      return {
        content: localizeWithPath(_moduleId, 59, "A file or folder **{0}** already exists at this location. Please choose a different name.", name),
        severity: Severity$1.Error
      };
    }
  }
  if (names.some(folderName => !pathService.hasValidBasename(item.resource, os, folderName))) {
    ...
    return {
      content: localizeWithPath(_moduleId, 60, "The name **{0}** is not valid as a file or folder name. Please choose a different name.", trimLongName(escapedName)),
      severity: Severity$1.Error
    };
  }
  if (names.some(name => /^\s|\s$/.test(name))) {
    return {
      content: localizeWithPath(_moduleId, 61, "Leading or trailing whitespace detected in file or folder name."),
...
class CompareNewUntitledTextFilesAction extends Action2 {
  ...
  static {
    this.LABEL = localize2WithPath(_moduleId, 62, "Compare New Untitled Text Files");
  }
  constructor() {
    super({
      ...
      category: Categories.File,
      metadata: {
        description: localize2WithPath(_moduleId, 63, "Opens a new diff editor with two untitled files.")
      }
    });
  }
  async run(accessor) {
...
class CompareWithClipboardAction extends Action2 {
  ...
  static {
    this.LABEL = localize2WithPath(_moduleId, 64, "Compare Active File with Clipboard");
  }
  static {
  ...
  constructor() {
    super({
      ...
      category: Categories.File,
      keybinding: {
        primary: KeyChord(2048 | 41, 33),
        weight: 200
      },
      metadata: {
        description: localize2WithPath(_moduleId, 65, "Opens a new diff editor to compare the active file with the contents of the clipboard.")
      }
    });
  }
  async run(accessor) {
    ...
    if (resource && (fileService.hasProvider(resource) || resource.scheme === Schemas.untitled)) {
      ...
      const editorLabel = localizeWithPath(_moduleId, 66, "Clipboard ↔ {0}", name);
      await editorService.openEditor({
        original: {
...
function onErrorWithRetry(notificationService, error, retry) {
  notificationService.prompt(Severity$1.Error, toErrorMessage(error, false), [{
    label: localizeWithPath(_moduleId, 67, "Retry"),
    run: () => retry()
  }]);
}
  ...
  const onSuccess = async value => {
    try {
      ...
      await explorerService.applyBulkEdit([new ResourceFileEdit(undefined, resourceToCreate, {
        ...
        undoLabel: localizeWithPath(_moduleId, 68, "Create {0}", value),
        progressLabel: localizeWithPath(_moduleId, 69, "Creating {0}", value),
        ...
        if (stat.resource.toString() !== targetResource.toString()) {
          try {
            await explorerService.applyBulkEdit([new ResourceFileEdit(stat.resource, targetResource)], {
              ...
              undoLabel: localizeWithPath(_moduleId, 70, "Rename {0} to {1}", stat.name, value),
              progressLabel: localizeWithPath(_moduleId, 71, "Renaming {0} to {1}", stat.name, value)
            });
            await refreshIfSeparator(value, explorerService);
          } catch (e) {
...
const pasteFileHandler = async (accessor, fileList) => {
  ...
  if (confirmPasteNative && toPaste.files.length >= 1) {
    const message = toPaste.files.length > 1 ? localizeWithPath(_moduleId, 72, "Are you sure you want to paste the following {0} items?", toPaste.files.length) : localizeWithPath(_moduleId, 73, "Are you sure you want to paste '{0}'?", basename$1(toPaste.type === 'paths' ? toPaste.files[0].fsPath : toPaste.files[0].name));
    ...
    const confirmation = await dialogService.confirm({
      ...
      primaryButton: localizeWithPath(_moduleId, 74, "&&Paste")
    });
    if (!confirmation.confirmed) {
      return;
    }
  ...
  try {
    ...
    if (toPaste.type === 'paths') {
      const sourceTargetPairs = coalesce(await Promise.all(toPaste.files.map(async fileToPaste => {
        if (element.resource.toString() !== fileToPaste.toString() && isEqualOrParent(element.resource, fileToPaste)) {
          throw new Error(localizeWithPath(_moduleId, 75, "File to paste is an ancestor of the destination folder"));
        }
      ...
      if (sourceTargetPairs.length >= 1) {
        if (pasteShouldMove) {
          ...
          const options = {
            confirmBeforeUndo: configurationService.getValue().explorer.confirmUndo === "verbose",
            progressLabel: sourceTargetPairs.length > 1 ? localizeWithPath(_moduleId, 76, "Moving {0} files", sourceTargetPairs.length) : localizeWithPath(_moduleId, 77, "Moving {0}", basenameOrAuthority(sourceTargetPairs[0].target)),
            undoLabel: sourceTargetPairs.length > 1 ? localizeWithPath(_moduleId, 78, "Move {0} files", sourceTargetPairs.length) : localizeWithPath(_moduleId, 79, "Move {0}", basenameOrAuthority(sourceTargetPairs[0].target))
          };
          await explorerService.applyBulkEdit(resourceFileEdits, options);
        } else {
          const resourceFileEdits = sourceTargetPairs.map(pair => new ResourceFileEdit(pair.source, pair.target, {
    ...
    onError(notificationService, new Error(localizeWithPath(_moduleId, 80, "The file(s) to paste have been deleted or moved since you copied them. {0}", getErrorMessage(e))));
  } finally {
    if (pasteShouldMove) {
  ...
  async function applyCopyResourceEdit(targets, resourceFileEdits) {
    ...
    const options = {
      ...
      progressLabel: targets.length > 1 ? localizeWithPath(_moduleId, 81, "Copying {0} files", targets.length) : localizeWithPath(_moduleId, 82, "Copying {0}", basenameOrAuthority(targets[0])),
      undoLabel: targets.length > 1 ? localizeWithPath(_moduleId, 83, "Paste {0} files", targets.length) : localizeWithPath(_moduleId, 84, "Paste {0}", basenameOrAuthority(targets[0]))
    };
    await explorerService.applyBulkEdit(resourceFileEdits, options);
  }
};
async function getFilesToPaste(fileList, clipboardService) {
  if (fileList && fileList.length > 0) {
    ...
    return {
      type: 'data',
      files: [...fileList].filter(file => !file.path)
    };
  } else {
    return {
      type: 'paths',
...
class SetActiveEditorReadonlyInSession extends BaseSetActiveEditorReadonlyInSession {
  ...
  static {
    this.LABEL = localize2WithPath(_moduleId, 85, "Set Active Editor Read-only in Session");
  }
  constructor() {
...
class SetActiveEditorWriteableInSession extends BaseSetActiveEditorReadonlyInSession {
  ...
  static {
    this.LABEL = localize2WithPath(_moduleId, 86, "Set Active Editor Writeable in Session");
  }
  constructor() {
...
class ToggleActiveEditorReadonlyInSession extends BaseSetActiveEditorReadonlyInSession {
  ...
  static {
    this.LABEL = localize2WithPath(_moduleId, 87, "Toggle Active Editor Read-only in Session");
  }
  constructor() {
...
class ResetActiveEditorReadonlyInSession extends BaseSetActiveEditorReadonlyInSession {
  ...
  static {
    this.LABEL = localize2WithPath(_moduleId, 88, "Reset Active Editor Read-only in Session");
  }
  constructor() {
    super(ResetActiveEditorReadonlyInSession.ID, ResetActiveEditorReadonlyInSession.LABEL, 'reset');
  }
}
exports.COPY_FILE_LABEL = COPY_FILE_LABEL;
...
exports.DOWNLOAD_COMMAND_ID = DOWNLOAD_COMMAND_ID;
exports.DOWNLOAD_LABEL = DOWNLOAD_LABEL;
exports.FileCopiedContext = FileCopiedContext;
exports.FocusFilesExplorer = FocusFilesExplorer;
exports.GlobalCompareResourcesAction = GlobalCompareResourcesAction;
exports.MOVE_FILE_TO_TRASH_LABEL = MOVE_FILE_TO_TRASH_LABEL;
exports.NEW_FILE_COMMAND_ID = NEW_FILE_COMMAND_ID;
exports.NEW_FILE_LABEL = NEW_FILE_LABEL;
exports.NEW_FOLDER_COMMAND_ID = NEW_FOLDER_COMMAND_ID;
exports.NEW_FOLDER_LABEL = NEW_FOLDER_LABEL;
exports.OpenActiveFileInEmptyWorkspace = OpenActiveFileInEmptyWorkspace;
exports.PASTE_FILE_LABEL = PASTE_FILE_LABEL;
...
exports.TRIGGER_RENAME_LABEL = TRIGGER_RENAME_LABEL;
...
exports.UPLOAD_COMMAND_ID = UPLOAD_COMMAND_ID;
exports.UPLOAD_LABEL = UPLOAD_LABEL;
