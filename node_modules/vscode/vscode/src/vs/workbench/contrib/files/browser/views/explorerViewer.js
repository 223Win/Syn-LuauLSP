...
} = require('../../../../../base/browser/ui/list/listView.js');
const {
  isWeb,
  isMacintosh
...
const {
  IgnoreFile
} = require('../../../../services/search/common/ignoreFile.js');
const {
  ResourceSet
...
} = require('../explorerFileContrib.js');
const Severity$1 = require('../../../../../base/common/severity.js');
var FilesRenderer_1, FileDragAndDrop_1;
const _moduleId = "vs/workbench/contrib/files/browser/views/explorerViewer";
...
ExplorerDataSource = __decorate([__param(1, IProgressService), __param(2, IConfigurationService), __param(3, INotificationService), __param(4, IWorkbenchLayoutService), __param(5, IFileService), __param(6, IExplorerService), __param(7, IWorkspaceContextService), __param(8, IFilesConfigurationService)], ExplorerDataSource);
...
let FilesRenderer = class FilesRenderer {
  ...
  getWidgetAriaLabel() {
    return localizeWithPath(_moduleId, 0, "Files Explorer");
  }
  get templateId() {
  ...
  renderTemplate(container) {
    ...
    const label = templateDisposables.add(this.labels.create(container, {
      ...
      hoverDelegate: experimentalHover ? this.hoverDelegate : undefined
    }));
  ...
  renderInputBox(container, stat, editableData) {
    ...
    const inputBox = new InputBox(label.element, this.contextViewService, {
      ...
      ariaLabel: localizeWithPath(_moduleId, 1, "Type file name. Press Enter to confirm or Escape to cancel."),
...
FilesRenderer = FilesRenderer_1 = __decorate([__param(3, IContextViewService), __param(4, IThemeService), __param(5, IConfigurationService), __param(6, IExplorerService), __param(7, ILabelService), __param(8, IWorkspaceContextService), __param(9, IContextMenuService), __param(10, IHoverService), __param(11, IInstantiationService)], FilesRenderer);
...
let FileDragAndDrop = class FileDragAndDrop {
  ...
  async handleExplorerDrop(data, target, targetIndex, targetSector, originalEvent) {
    ...
    if (confirmDragAndDrop) {
      const message = items.length > 1 && items.every(s => s.isRoot) ? localizeWithPath(_moduleId, 2, "Are you sure you want to change the order of multiple root folders in your workspace?") : items.length > 1 ? localizeWithPath(_moduleId, 3, "Are you sure you want to move the following {0} files into '{1}'?", items.length, target.name) : items[0].isRoot ? localizeWithPath(_moduleId, 4, "Are you sure you want to change the order of root folder '{0}' in your workspace?", items[0].name) : localizeWithPath(_moduleId, 5, "Are you sure you want to move '{0}' into '{1}'?", items[0].name, target.name);
      ...
      const confirmation = await this.dialogService.confirm({
        ...
        primaryButton: localizeWithPath(_moduleId, 7, "&&Move")
      });
  ...
  async doHandleExplorerDropOnCopy(sources, target) {
    ...
    await this.explorerService.applyBulkEdit(resourceFileEdits, {
      ...
      undoLabel: localizeWithPath(_moduleId, 8, "Copy {0}", labelSuffix),
      progressLabel: localizeWithPath(_moduleId, 9, "Copying {0}", labelSuffix)
    });
  ...
  async doHandleExplorerDropOnMove(sources, target) {
    ...
    const options = {
      ...
      undoLabel: localizeWithPath(_moduleId, 10, "Move {0}", labelSuffix),
      progressLabel: localizeWithPath(_moduleId, 11, "Moving {0}", labelSuffix)
    };
    try {
...
function getFileOrFolderLabelSuffix(items) {
  ...
  if (items.every(i => i.isDirectory)) {
    return localizeWithPath(_moduleId, 12, "{0} folders", items.length);
  }
  if (items.every(i => !i.isDirectory)) {
    return localizeWithPath(_moduleId, 13, "{0} files", items.length);
  }
...
exports.explorerRootErrorEmitter = explorerRootErrorEmitter;
