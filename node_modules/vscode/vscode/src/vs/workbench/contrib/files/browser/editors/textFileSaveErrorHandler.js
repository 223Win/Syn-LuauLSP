...
const _moduleId = "vs/workbench/contrib/files/browser/editors/textFileSaveErrorHandler";
...
const conflictEditorHelp = localizeWithPath(_moduleId, 0, "Use the actions in the editor tool bar to either undo your changes or overwrite the content of the file with your changes.");
  ...
  onSaveError(error, model, options) {
    ...
    if (fileOperationError.fileOperationResult === 3) {
      if (this.activeConflictResolutionResource && isEqual(this.activeConflictResolutionResource, model.resource)) {
        ...
        message = localizeWithPath(_moduleId, 1, "Failed to save '{0}': The content of the file is newer. Please compare your version with the file contents or overwrite the content of the file with your changes.", basename(resource));
      ...
      if (isWriteLocked) {
        if (triedToUnlock && canSaveElevated) {
          message = isWindows ? localizeWithPath(_moduleId, 2, "Failed to save '{0}': File is read-only. Select 'Overwrite as Admin' to retry as administrator.", basename(resource)) : localizeWithPath(_moduleId, 3, "Failed to save '{0}': File is read-only. Select 'Overwrite as Sudo' to retry as superuser.", basename(resource));
        } else {
          message = localizeWithPath(_moduleId, 4, "Failed to save '{0}': File is read-only. Select 'Overwrite' to attempt to make it writeable.", basename(resource));
        }
      } else if (canSaveElevated && isPermissionDenied) {
        message = isWindows ? localizeWithPath(_moduleId, 5, "Failed to save '{0}': Insufficient permissions. Select 'Retry as Admin' to retry as administrator.", basename(resource)) : localizeWithPath(_moduleId, 6, "Failed to save '{0}': Insufficient permissions. Select 'Retry as Sudo' to retry as superuser.", basename(resource));
      } else {
        message = localizeWithPath(_moduleId, 7, "Failed to save '{0}': {1}", basename(resource), toErrorMessage(error, false));
      }
    }
    const actions = {
...
let ResolveConflictLearnMoreAction = class ResolveConflictLearnMoreAction extends Action {
  constructor(openerService) {
    super('workbench.files.action.resolveConflictLearnMore', localizeWithPath(_moduleId, 8, "Learn More"));
...
let DoNotShowResolveConflictLearnMoreAction = class DoNotShowResolveConflictLearnMoreAction extends Action {
  constructor(storageService) {
    super('workbench.files.action.resolveConflictLearnMoreDoNotShowAgain', localizeWithPath(_moduleId, 9, "Don't Show Again"));
    this.storageService = storageService;
  }
...
let ResolveSaveConflictAction = class ResolveSaveConflictAction extends Action {
  constructor(model, editorService, notificationService, instantiationService, productService) {
    super('workbench.files.action.resolveConflict', localizeWithPath(_moduleId, 10, "Compare"));
    this.model = model;
    this.editorService = editorService;
    this.notificationService = notificationService;
    this.instantiationService = instantiationService;
  ...
  async run() {
    if (!this.model.isDisposed()) {
      ...
      const editorLabel = localizeWithPath(_moduleId, 11, "{0} (in file) ↔ {1} (in {2}) - Resolve save conflict", name, name, this.productService.nameLong);
...
class SaveModelElevatedAction extends Action {
  constructor(model, options, triedToUnlock) {
    super('workbench.files.action.saveModelElevated', triedToUnlock ? isWindows ? localizeWithPath(_moduleId, 12, "Overwrite as Admin...") : localizeWithPath(_moduleId, 13, "Overwrite as Sudo...") : isWindows ? localizeWithPath(_moduleId, 14, "Retry as Admin...") : localizeWithPath(_moduleId, 15, "Retry as Sudo..."));
    this.model = model;
...
class RetrySaveModelAction extends Action {
  constructor(model, options) {
    super('workbench.files.action.saveModel', localizeWithPath(_moduleId, 16, "Retry"));
    this.model = model;
    this.options = options;
  }
  async run() {
...
class DiscardModelAction extends Action {
  constructor(model) {
    super('workbench.files.action.discardModel', localizeWithPath(_moduleId, 17, "Discard"));
    this.model = model;
  }
  async run() {
    if (!this.model.isDisposed()) {
...
class UnlockModelAction extends Action {
  constructor(model, options) {
    super('workbench.files.action.unlock', localizeWithPath(_moduleId, 18, "Overwrite"));
    this.model = model;
    this.options = options;
  }
  async run() {
...
class SaveModelIgnoreModifiedSinceAction extends Action {
  constructor(model, options) {
    super('workbench.files.action.saveIgnoreModifiedSince', localizeWithPath(_moduleId, 18, "Overwrite"));
    this.model = model;
    this.options = options;
  }
  async run() {
...
let ConfigureSaveConflictAction = class ConfigureSaveConflictAction extends Action {
  constructor(preferencesService) {
    super('workbench.files.action.configureSaveConflict', localizeWithPath(_moduleId, 19, "Configure"));
...
exports.CONFLICT_RESOLUTION_CONTEXT = CONFLICT_RESOLUTION_CONTEXT;
exports.CONFLICT_RESOLUTION_SCHEME = CONFLICT_RESOLUTION_SCHEME;
