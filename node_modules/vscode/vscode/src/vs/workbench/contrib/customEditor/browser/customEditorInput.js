...
const {
  IUndoRedoService
} = require('../../../../platform/undoRedo/common/undoRedo.service.js');
const {
...
} = require('../common/customEditor.service.js');
const {
  IWebviewService
} = require('../../webview/browser/webview.service.js');
const {
  LazilyResolvedWebviewEditorInput
} = require('../../webviewPanel/browser/webviewWorkbenchService.js');
const {
  IWebviewWorkbenchService
} = require('../../webviewPanel/browser/webviewWorkbenchService.service.js');
const {
...
const _moduleId = "vs/workbench/contrib/customEditor/browser/customEditorInput";
let CustomEditorInput = class CustomEditorInput extends LazilyResolvedWebviewEditorInput {
  ...
  static create(instantiationService, resource, viewType, group, options) {
    return instantiationService.invokeFunction(accessor => {
      ...
      const input = instantiationService.createInstance(CustomEditorInput_1, {
        resource,
        viewType
      }, webview, {
  ...
  constructor(init, webview, options, webviewWorkbenchService, instantiationService, labelService, customEditorService, fileDialogService, undoRedoService, fileService, filesConfigurationService, editorGroupsService, layoutService, customEditorLabelService) {
    super({
      ...
      name: ''
    }, webview, webviewWorkbenchService);
  ...
  claim(claimant, targetWindow, scopedContextKeyService) {
    if (this.doCanMove(targetWindow.vscodeWindowId) !== true) {
      throw createEditorOpenError(localizeWithPath(_moduleId, 0, "Unable to open the editor in this window, it contains modifications that can only be saved in the original window."), [toAction({
        id: 'openInOriginalWindow',
        label: localizeWithPath(_moduleId, 1, "Open in Original Window"),
        run: async () => {
  ...
  doCanMove(targetWindowId) {
    if (this.isModified() && this._modelRef?.object.canHotExit === false) {
      ...
      if (sourceWindowId !== targetWindowId) {
        return localizeWithPath(_moduleId, 2, "Unable to move '{0}': The editor contains changes that can only be saved in its current window.", this.getName());
      }
    }
    return true;
  }
};
CustomEditorInput = CustomEditorInput_1 = __decorate([__param(3, IWebviewWorkbenchService), __param(4, IInstantiationService), __param(5, ILabelService), __param(6, ICustomEditorService), __param(7, IFileDialogService), __param(8, IUndoRedoService), __param(9, IFileService), __param(10, IFilesConfigurationService), __param(11, IEditorGroupsService), __param(12, IWorkbenchLayoutService), __param(13, ICustomEditorLabelService)], CustomEditorInput);
