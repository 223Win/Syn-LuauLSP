...
} = require('./chatSlashCommands.service.js');
const {
  IChatVariablesService
} = require('./chatVariables.service.js');
...
let ChatRequestParser = class ChatRequestParser {
  ...
  tryToParseAgent(message, fullMessage, offset, position, parts, location, context) {
    ...
    const agent = agents.length > 1 && context?.selectedAgent ? context.selectedAgent : agents[0];
...
ChatRequestParser = __decorate([__param(0, IChatAgentService), __param(1, IChatVariablesService), __param(2, IChatSlashCommandService)], ChatRequestParser);
