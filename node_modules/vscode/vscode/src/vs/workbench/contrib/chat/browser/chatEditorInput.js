...
const _moduleId = "vs/workbench/contrib/chat/browser/chatEditorInput";
const ChatEditorIcon = registerIcon('chat-editor-label-icon', Codicon.commentDiscussion, localizeWithPath(_moduleId, 0, 'Icon of the chat editor label.'));
let ChatEditorInput = class ChatEditorInput extends EditorInput {
  ...
  constructor(resource, options, chatService) {
    ...
    this.sessionId = options.target && 'sessionId' in options.target ? options.target.sessionId : undefined;
  ...
  getName() {
    return this.model?.title || localizeWithPath(_moduleId, 1, "Chat") + (this.inputCount > 0 ? ` ${this.inputCount + 1}` : '');
  }
  getIcon() {
...
ChatEditorInput = ChatEditorInput_1 = __decorate([__param(2, IChatService)], ChatEditorInput);
