...
let ChatMarkdownDecorationsRenderer = class ChatMarkdownDecorationsRenderer {
  ...
  convertParsedRequestToMarkdown(parsedRequest) {
    ...
    for (const part of parsedRequest.parts) {
      if (part instanceof ChatRequestTextPart) {
        ...
        const uri = part instanceof ChatRequestDynamicVariablePart && part.data.map(d => d.value).find(d => d instanceof URI) || undefined;
        const title = uri ? encodeURIComponent(this.labelService.getUriLabel(uri, {
          relative: true
  ...
  renderFileWidget(href, a) {
    ...
    a.setAttribute('data-href', location.uri.with({
      fragment
    }).toString());
    const label = this.labelService.getUriLabel(location.uri, {
      relative: true
    });
    a.title = location.range ? `${label}#${location.range.startLineNumber}-${location.range.endLineNumber}` : label;
  }
...
ChatMarkdownDecorationsRenderer = __decorate([__param(0, IKeybindingService), __param(1, ILabelService), __param(2, ILogService), __param(3, IChatAgentService), __param(4, IInstantiationService), __param(5, IHoverService)], ChatMarkdownDecorationsRenderer);
