...
const {
  HistoryNavigator
} = require('../../../../base/common/history.js');
const {
  Disposable,
  DisposableStore
...
} = require('./actions/chatExecuteActions.js');
const {
  ChatFollowups
} = require('./chatFollowups.js');
const {
  ChatAgentLocation
} = require('../common/chatAgents.js');
const {
  IChatAgentService
...
} = require('../common/chatWidgetHistoryService.service.js');
const {
...
let ChatInputPart = class ChatInputPart extends Disposable {
  ...
  constructor(location, options, historyService, modelService, instantiationService, contextKeyService, configurationService, keybindingService, accessibilityService) {
    super();
  ...
  _getAriaLabel() {
    ...
    if (verbose) {
      ...
      return kbLabel ? localizeWithPath(_moduleId, 0, "Chat Input,  Type to ask questions or type / for topics, press enter to send out the request. Use {0} for Chat Accessibility Help.", kbLabel) : localizeWithPath(_moduleId, 1, "Chat Input,  Type code here and press Enter to run. Use the Chat Accessibility Help command for more information.");
    }
  ...
  navigateHistory(previous) {
    const historyEntry = (previous ? this.history.previous() ?? this.history.first() : this.history.next()) ?? {
      text: ''
    };
  ...
  render(container, initialValue, widget) {
    ...
    } = this._register(registerAndCreateHistoryNavigationContext(inputScopedContextKeyService, this));
        ...
        if (this.location === ChatAgentLocation.Panel) {
          if ((action.id === SubmitAction.ID || action.id === CancelAction.ID) && action instanceof MenuItemAction) {
            const dropdownAction = this.instantiationService.createInstance(MenuItemAction, {
              ...
              title: localizeWithPath(_moduleId, 3, "More..."),
  ...
  initImplicitContext(container) {
    this.implicitContextCheckbox = new Checkbox('#selection', true, {
      ...defaultCheckboxStyles,
      checkboxBorder: asCssVariableWithDefault(checkboxBorder, inputBackground)
    });
  ...
  async renderFollowups(items, response) {
    ...
    if (items && items.length > 0) {
      this.followupsDisposables.add(this.instantiationService.createInstance(ChatFollowups, this.followupsContainer, items, this.location, undefined, followup => this._onDidAcceptFollowup.fire({
        followup,
        response
      })));
    }
...
ChatInputPart = ChatInputPart_1 = __decorate([__param(2, IChatWidgetHistoryService), __param(3, IModelService), __param(4, IInstantiationService), __param(5, IContextKeyService), __param(6, IConfigurationService), __param(7, IKeybindingService), __param(8, IAccessibilityService)], ChatInputPart);
...
ChatSubmitDropdownActionItem = __decorate([__param(2, IMenuService), __param(3, IContextMenuService), __param(4, IChatAgentService), __param(5, IContextKeyService), __param(6, IKeybindingService), __param(7, INotificationService), __param(8, IThemeService), __param(9, IAccessibilityService)], ChatSubmitDropdownActionItem);
