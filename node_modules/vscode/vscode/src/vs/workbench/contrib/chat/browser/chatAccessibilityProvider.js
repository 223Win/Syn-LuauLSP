...
let ChatAccessibilityProvider = class ChatAccessibilityProvider {
  ...
  _getLabelWithCodeBlockCount(element) {
    ...
    switch (codeBlockCount) {
      case 0:
        label = accessibleViewHint ? localizeWithPath(_moduleId, 3, "{0} {1} {2}", fileTreeCountHint, element.response.asString(), accessibleViewHint) : localizeWithPath(_moduleId, 4, "{0} {1}", fileTreeCountHint, element.response.asString());
        break;
      case 1:
        label = accessibleViewHint ? localizeWithPath(_moduleId, 5, "{0} 1 code block: {1} {2}", fileTreeCountHint, element.response.asString(), accessibleViewHint) : localizeWithPath(_moduleId, 6, "{0} 1 code block: {1}", fileTreeCountHint, element.response.asString());
        break;
      default:
        label = accessibleViewHint ? localizeWithPath(_moduleId, 7, "{0} {1} code blocks: {2}", fileTreeCountHint, codeBlockCount, element.response.asString(), accessibleViewHint) : localizeWithPath(_moduleId, 8, "{0} {1} code blocks", fileTreeCountHint, codeBlockCount, element.response.asString());
        break;
    }
    return label;
  }
};
ChatAccessibilityProvider = __decorate([__param(0, IAccessibleViewService)], ChatAccessibilityProvider);
