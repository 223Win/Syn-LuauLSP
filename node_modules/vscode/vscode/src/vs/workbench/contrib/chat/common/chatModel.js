...
const {
  ChatAgentLocation
} = require('./chatAgents.js');
const {
  IChatAgentService
} = require('./chatAgents.service.js');
const {
  getPromptText,
  reviveParsedChatRequest,
  ChatRequestTextPart
} = require('./chatParserTypes.js');
const {
  isIUsedContext
...
let ChatModel = ChatModel_1 = class ChatModel extends Disposable {
  ...
  get responderUsername() {
    return (this._defaultAgent ? this._defaultAgent.metadata.fullName : this.initialData?.responderUsername) ?? '';
  }
  ...
  _deserialize(obj) {
    ...
    try {
      return requests.map(raw => {
        ...
        const variableData = raw.variableData && Array.isArray(raw.variableData.variables) ? raw.variableData : {
          variables: []
        };
        const request = new ChatRequestModel(this, parsedRequest, variableData);
        if (raw.response || raw.result || raw.responseErrorDetails) {
          const agent = raw.agent && 'metadata' in raw.agent ? this.reviveSerializedAgent(raw.agent) : undefined;
...
ChatModel = ChatModel_1 = __decorate([__param(2, ILogService), __param(3, IChatAgentService), __param(4, IInstantiationService)], ChatModel);
let ChatWelcomeMessageModel = class ChatWelcomeMessageModel {
  static {
    ChatWelcomeMessageModel_1 = this;
  }
  static {
    this.nextId = 0;
  }
...
ChatWelcomeMessageModel = ChatWelcomeMessageModel_1 = __decorate([__param(2, IChatAgentService)], ChatWelcomeMessageModel);
...
exports.ChatModelInitState = ChatModelInitState;
