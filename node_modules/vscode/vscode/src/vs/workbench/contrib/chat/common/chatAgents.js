...
const {
  asJson
} = require('../../../../platform/request/common/request.js');
const {
  IRequestService
} = require('../../../../platform/request/common/request.service.js');
const {
  IStorageService
} = require('../../../../platform/storage/common/storage.service.js');
const {
  CONTEXT_CHAT_ENABLED
} = require('./chatContextKeys.js');
...
let ChatAgentService = class ChatAgentService {
  ...
  getActivatedAgents() {
    return Array.from(this._agents.values()).filter(a => !!a.impl).map(a => new MergedChatAgent(a.data, a.impl));
  }
...
ChatAgentService = __decorate([__param(0, IContextKeyService)], ChatAgentService);
class MergedChatAgent {
  ...
  get extensionPublisherDisplayName() {
    return this.data.extensionPublisherDisplayName;
  }
...
let ChatAgentNameService = class ChatAgentNameService {
  ...
  refresh() {
    ...
    this.update().catch(err => this.logService.warn('Failed to fetch chat participant registry', err)).then(() => timeout(5 * 60 * 1000)).then(() => this.refresh());
  }
  async update() {
...
ChatAgentNameService = ChatAgentNameService_1 = __decorate([__param(0, IProductService), __param(1, IRequestService), __param(2, ILogService), __param(3, IStorageService)], ChatAgentNameService);
exports.ChatAgentLocation = ChatAgentLocation;
