...
const {} = require('./media/chat.css.js');
const {} = require('./media/chatAgentHover.css.js');
const {
  ICodeEditorService
...
} = require('./chatAccessibilityProvider.js');
const {
  ChatInputPart
} = require('./chatInputPart.js');
const {
  ChatListDelegate,
  ChatListItemRenderer
} = require('./chatListRenderer.js');
const {
  ChatEditorOptions
} = require('./chatOptions.js');
const {
  IChatAgentService
...
const {
  ChatRequestParser
} = require('../common/chatRequestParser.js');
const {
  IChatService
} = require('../common/chatService.service.js');
const {
  IChatSlashCommandService
} = require('../common/chatSlashCommands.service.js');
const {
  isResponseVM,
  isRequestVM,
  isWelcomeVM,
...
} = require('../common/codeBlockModelCollection.js');
var ChatWidget_1;
const $ = $$1;
...
let ChatWidget = class ChatWidget extends Disposable {
    ...
    if (this.tree && this._visible) {
      ...
      this.tree.setChildren(null, treeItems, {
        diffIdentityProvider: {
          getId: element => {
            return (isResponseVM(element) || isRequestVM(element) ? element.dataId : element.id) + (isWelcomeVM(element) && this.viewModel ? `_${ChatModelInitState[this.viewModel.initState]}` : '') + `${isRequestVM(element) || isWelcomeVM(element)}` + `${isResponseVM(element) && element.renderData ? `_${this.visibleChangeCount}` : ''}` + (isResponseVM(element) ? `_${element.contentReferences.length}` : '');
          }
        }
      });
  ...
  onContextMenu(e) {
    ...
    const scopedContextKeyService = this.contextKeyService.createOverlay([[CONTEXT_RESPONSE_FILTERED.key, isResponseVM(selected) && !!selected.errorDetails?.responseIsFiltered]]);
  ...
  updateImplicitContextKinds() {
    ...
    const implicitVariables = agentAndSubcommand.commandPart ? agentAndSubcommand.commandPart.command.defaultImplicitVariables : currentAgent?.defaultImplicitVariables;
  ...
  async _acceptInput(opts) {
    if (this.viewModel) {
      ...
      const result = await this.chatService.sendRequest(this.viewModel.sessionId, input, {
        implicitVariablesEnabled: this.inputPart.implicitContextEnabled,
...
ChatWidget = ChatWidget_1 = __decorate([__param(4, ICodeEditorService), __param(5, IContextKeyService), __param(6, IInstantiationService), __param(7, IChatService), __param(8, IChatAgentService), __param(9, IChatWidgetService), __param(10, IContextMenuService), __param(11, IChatAccessibilityService), __param(12, ILogService), __param(13, IThemeService), __param(14, IChatSlashCommandService)], ChatWidget);
