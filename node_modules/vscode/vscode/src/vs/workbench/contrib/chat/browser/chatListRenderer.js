...
const {
  MarkdownRenderer
} = require('../../../../editor/browser/widget/markdownRenderer/browser/markdownRenderer.js');
const {
  Range
...
const {
  ResourceLabels
} = require('../../../browser/labels.js');
const {
  GeneratingPhrase
} = require('./chat.js');
const {
  ChatAgentHover
} = require('./chatAgentHover.js');
const {
  ChatFollowups
} = require('./chatFollowups.js');
const {
  ChatMarkdownDecorationsRenderer
} = require('./chatMarkdownDecorationsRenderer.js');
const {
...
const {
  getNWords
} = require('../common/chatWordCounter.js');
const {
  createFileIconThemableTreeContainerScope
} = require('../../files/browser/views/explorerView.js');
const {
  annotateSpecialMarkdownContent
} = require('../common/annotations.js');
const {
...
let ChatListItemRenderer = class ChatListItemRenderer extends Disposable {
  ...
  renderDetail(element, templateData) {
    ...
    if (element.agent && !element.agent.isDefault) {
      ...
      agentName = this.chatAgentNameService.getAgentNameRestriction(element.agent).map(allowed => allowed ? name : name);
    }
  ...
  getAgentIcon(agent) {
    if (agent.themeIcon) {
      ...
      return this.themeService.getColorTheme().type === ColorScheme.DARK && agent.iconDark ? agent.iconDark : agent.icon;
    }
  }
  basicRenderElement(value, element, index, templateData) {
    ...
    value.forEach((data, index) => {
      const result = data.kind === 'treeData' ? this.renderTreeData(data.treeData, element, templateData, fileTreeIndex++) : data.kind === 'markdownContent' ? this.renderMarkdown(data.content, element, templateData, fillInIncompleteTokens) : data.kind === 'progressMessage' && onlyProgressMessagesAfterI(value, index) ? this.renderProgressMessage(data, false) : data.kind === 'command' ? this.renderCommandButton(element, data) : data.kind === 'textEditGroup' ? this.renderTextEdit(element, data, templateData) : undefined;
      if (result) {
      ...
      renderableResponse.forEach((part, index) => {
        ...
        if (!renderedPart) {
          if (part.kind === 'treeData') {
            ...
            const wordCountResult = this.getDataForProgressiveRender(element, contentToMarkdown(part.content), {
              renderedWordCount: 0,
              lastRenderTime: 0
            });
        ...
        } else if (part.kind === 'progressMessage' && isProgressMessageRenderData(renderedPart) && (renderedPart.isAtEndOfResponse !== onlyProgressMessagesAfterI(renderableResponse, index) || renderedPart.isLast !== (index === renderableResponse.length - 1))) {
          partsToRender[index] = {
            progressMessage: part,
            isAtEndOfResponse: onlyProgressMessagesAfterI(renderableResponse, index),
            isLast: index === renderableResponse.length - 1
  ...
  renderContentReferencesListData(data, element, templateData) {
    ...
    const referencesLabel = data.length > 1 ? localizeWithPath(_moduleId, 2, "Used {0} references", data.length) : localizeWithPath(_moduleId, 3, "Used {0} reference", 1);
  ...
  renderCommandButton(element, commandButton) {
    ...
    const tooltip = enabled ? commandButton.command.tooltip : localizeWithPath(_moduleId, 6, "Button not available in restored chat");
  ...
  renderTextEdit(element, chatTextEdit, templateData) {
    ...
    const data = {
      ...
      diffData: (async () => {
        ...
        const modified = this.modelService.createModel(createTextBufferFactoryFromSnapshot(original.createSnapshot()), {
  ...
  renderMarkdown(markdown, element, templateData, fillInIncompleteTokens = false) {
    ...
    const result = this.renderer.render(markdown, {
      ...
      asyncRenderCallback: () => this._onDidChangeItemHeight.fire({
        element,
        height: templateData.rowContainer.offsetHeight
      })
    });
  ...
  renderCodeBlock(data, text) {
    ...
    if (isResponseVM(data.element)) {
      this.codeBlockModelCollection.update(data.element.sessionId, data.element, data.codeBlockIndex, {
        text,
  ...
  getDataForProgressiveRender(element, data, renderData) {
    ...
    const numWordsToRender = renderData.lastRenderTime === 0 ? 1 : renderData.renderedWordCount + Math.floor((Date.now() - renderData.lastRenderTime) / 1000 * rate);
...
ChatListItemRenderer = ChatListItemRenderer_1 = __decorate([__param(6, IInstantiationService), __param(7, IConfigurationService), __param(8, ILogService), __param(9, IOpenerService), __param(10, IContextKeyService), __param(11, IThemeService), __param(12, ICommandService), __param(13, ITextModelService), __param(14, IModelService), __param(15, IHoverService), __param(16, IChatAgentNameService)], ChatListItemRenderer);
...
ChatListDelegate = __decorate([__param(1, ILogService)], ChatListDelegate);
...
EditorPool = __decorate([__param(3, IInstantiationService)], EditorPool);
...
DiffEditorPool = __decorate([__param(3, IInstantiationService)], DiffEditorPool);
...
ContentReferencesListPool = __decorate([__param(1, IInstantiationService), __param(2, IThemeService)], ContentReferencesListPool);
...
let ContentReferencesListRenderer = class ContentReferencesListRenderer {
  static {
    ContentReferencesListRenderer_1 = this;
  }
  static {
    this.TEMPLATE_ID = 'contentReferencesListRenderer';
  }
...
ContentReferencesListRenderer = ContentReferencesListRenderer_1 = __decorate([__param(1, IChatVariablesService)], ContentReferencesListRenderer);
