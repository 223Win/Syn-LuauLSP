...
} = require('../common/extHostExtensionService.js');
const {
  URI
} = require('../../../base/common/uri.js');
const {
  RequireInterceptor
} = require('../common/extHostRequireInterceptor.js');
const {
  ExtensionRuntime
} = require('../common/extHostTypes.js');
const {
  timeout
} = require('../../../base/common/async.js');
const {
  ExtHostConsoleForwarder
} = require('./extHostConsoleForwarder.js');
const {
  isWebWorker
...
class ExtHostExtensionService extends AbstractExtHostExtensionService {
  ...
  async _loadCommonJSModule(extension, module, activationTimesBuilder) {
    module = module.with({
      path: ensureSuffix(module.path, '.js')
    });
