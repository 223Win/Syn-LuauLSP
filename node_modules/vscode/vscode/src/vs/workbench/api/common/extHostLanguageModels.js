...
} = require('../../services/authentication/common/authentication.js');
const {
  CancellationError
...
let ExtHostLanguageModels = class ExtHostLanguageModels {
  ...
  async $provideLanguageModelResponse(handle, requestId, from, messages, options, token) {
    ...
    const progress = new Progress(async fragment => {
      ...
      this._proxy.$handleProgressChunk(requestId, {
  ...
  async sendChatRequest(extension, languageModelId, messages, options, token) {
    ...
    if (this._isUsingAuth(from, metadata)) {
      ...
      }, options.justification, options.silent);
    ...
    await barrier.wait();
    if (error) {
      throw new LanguageModelError(`Language model '${languageModelId}' errored, check cause for more details`, 'Unknown', error);
    }
  ...
  async _getAuthAccess(from, to, justification, silent) {
    ...
    try {
      const detail = justification ? localizeWithPath(_moduleId, 0, "To allow access to the language models provided by {0}. Justification:\n\n{1}", to.displayName, justification) : localizeWithPath(_moduleId, 1, "To allow access to the language models provided by {0}", to.displayName);
...
exports.IExtHostLanguageModels = IExtHostLanguageModels;