...
class ExtHostTreeViews extends Disposable {
  ...
  registerTreeDataProvider(id, treeDataProvider, extension) {
    const treeView = this.createTreeView(id, {
      treeDataProvider
    }, extension);
    return {
...
class ExtHostTreeView extends Disposable {
  ...
  constructor(viewId, options, proxy, commands, logService, extension) {
    ...
    this._register(onDidChangeData(({
      message,
      elements
    }) => {
      if (elements.length) {
  ...
  reveal(element, options) {
    ...
    if (element) {
      return this.refreshPromise.then(() => this.resolveUnknownParentChain(element)).then(parentChain => this.resolveTreeNode(element, parentChain[parentChain.length - 1]).then(treeNode => this.proxy.$reveal(this.viewId, {
        item: treeNode.item,
  ...
  setExpanded(treeItemHandle, expanded) {
    ...
    if (element) {
      if (expanded) {
        this._onDidExpandElement.fire(Object.freeze({
          element
        }));
      } else {
  ...
  setSelectionAndFocus(selectedHandles, focusedHandle) {
    ...
    if (changedSelection) {
      this._onDidChangeSelection.fire(Object.freeze({
        selection: this.selectedElements
      }));
    }
    if (changedFocus) {
      this._onDidChangeActiveItem.fire(Object.freeze({
  ...
  setVisible(visible) {
    if (visible !== this._visible) {
      ...
      this._onDidChangeVisibility.fire(Object.freeze({
  ...
  async onDrop(treeDataTransfer, targetHandleOrNode, token) {
    ...
    return asPromise(() => this.dndController?.handleDrop ? this.dndController.handleDrop(target, treeDataTransfer, token) : undefined);
  }
  get hasResolve() {
  ...
  resolveTreeNode(element, parent) {
    ...
    return asPromise(() => this.dataProvider.getTreeItem(element)).then(extTreeItem => this.createHandle(element, extTreeItem, parent, true)).then(handle => this.getChildren(parent ? parent.item.handle : undefined).then(() => {
  ...
  createAndRegisterTreeNode(element, extTreeItem, parentNode) {
    ...
    if (extTreeItem.id && this.elements.has(node.item.handle)) {
      throw new Error(localizeWithPath(_moduleId, 0, 'Element with id {0} is already registered', extTreeItem.id));
    }
  ...
  getCheckbox(extensionTreeItem) {
    ...
    return {
      isChecked: checkboxState === TreeItemCheckboxState.Checked,
      tooltip,
