...
let ExtHostTesting = class ExtHostTesting extends Disposable {
  ...
  $publishTestResults(results) {
    this.results = Object.freeze(results.map(TestResults.to).concat(this.results).sort((a, b) => b.completedAt - a.completedAt).slice(0, 32));
  ...
  $acceptDiff(diff) {
    this.observer.applyDiff(diff.map(d => TestsDiffOp.deserialize({
      asCanonicalUri: u => u
    }, d)));
  }
  ...
  async runControllerTestRequest(req, isContinuous, token) {
    ...
    const excludeTests = req.excludeExtIds.map(id => lookup.collection.tree.get(id)).filter(isDefined).filter(exclude => includeTests.some(include => include.fullId.compare(exclude.fullId) === 2));
...
ExtHostTesting = __decorate([__param(0, IExtHostRpcService), __param(1, ILogService)], ExtHostTesting);
