...
} = require('./extHostExtensionActivator.js');
const {
  ExtHostStorage,
  IExtHostStorage
...
} = require('../../services/extensions/common/extensionDescriptionRegistry.js');
const {
...
} = require('../../services/extensions/common/workspaceContains.js');
const {
  ExtHostSecretState,
  IExtHostSecretState
...
let AbstractExtHostExtensionService = AbstractExtHostExtensionService_1 = class AbstractExtHostExtensionService extends Disposable {
  ...
  _doActivateExtension(extensionDescription, reason) {
    ...
    return Promise.all([this._loadCommonJSModule(extensionDescription, joinPath(extensionDescription.extensionLocation, entryPoint), activationTimesBuilder), this._loadExtensionContext(extensionDescription)]).then(values => {
  ...
  _loadExtensionContext(extensionDescription) {
    ...
    return Promise.all([globalState.whenReady, workspaceState.whenReady, this._storagePath.whenReady]).then(() => {
      const that = this;
      let extension;
      let messagePassingProtocol;
      const messagePort = isProposedApiEnabled(extensionDescription, 'ipc') ? this._initData.messagePorts?.get(ExtensionIdentifier.toKey(extensionDescription.identifier)) : undefined;
      return Object.freeze({
        ...
        get languageModelAccessInformation() {
          return lanuageModelAccessInformation;
        },
        get extensionUri() {
  ...
  async _doHandleExtensionTests() {
    ...
    if (!extensionDevelopmentLocationURI || !extensionTestsLocationURI) {
      throw new Error(localizeWithPath(_moduleId, 0, "Cannot load test runner."));
    }
    ...
    if (!testRunner || typeof testRunner.run !== 'function') {
      throw new Error(localizeWithPath(_moduleId, 1, "Path {0} does not point to a valid extension test runner.", extensionTestsLocationURI.toString()));
    }
  ...
  async $startExtensionHost(extensionsDelta) {
    ...
    const {
      globalRegistry,
      myExtensions
    } = applyExtensionsDelta(this._activationEventsReader, this._globalRegistry, this._myRegistry, extensionsDelta);
...
exports.IExtHostExtensionService = IExtHostExtensionService;
exports.IHostUtils = IHostUtils;