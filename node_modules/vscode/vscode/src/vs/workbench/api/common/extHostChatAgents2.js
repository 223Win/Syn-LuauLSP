...
} = require('../../contrib/chat/common/chatService.js');
const {
    ...
    if (!this._apiObject) {
      ...
      this._apiObject = {
          ...
          if ('variableName' in value && !value.value) {
            ...
            if (matchingVarData) {
              ...
              if (matchingVarData.references?.length) {
                ...
                const part = new ChatResponseReferencePart$1(value, iconPath);
        ...
        push(part) {
          ...
          if (part instanceof ChatResponseTextEditPart$1 || part instanceof ChatResponseMarkdownWithVulnerabilitiesPart || part instanceof ChatResponseDetectedParticipantPart$1) {
            checkProposedApiEnabled(that._extension, 'chatParticipantAdditions');
          }
...
class ExtHostChatAgents2 extends Disposable {
  ...
  constructor(mainContext, _logService, commands) {
    ...
    this._completionDisposables = this._register(new DisposableStore());
  ...
  async $invokeAgent(handle, request, context, token) {
    ...
    try {
      ...
      const task = agent.invoke(ChatAgentRequest.to(request), {
  ...
  async prepareHistoryTurns(agentId, context) {
    ...
    for (const h of context.history) {
      ...
      const result = agentId === h.request.agentId ? ehResult : {
        ...ehResult,
        metadata: undefined
      };
      res.push(new ChatRequestTurn(h.request.message, h.request.command, h.request.variables.variables.map(ChatAgentResolvedVariable.to), h.request.agentId));
      const parts = coalesce(h.response.map(r => ChatResponsePart.toContent(r, this.commands.converter)));
  ...
  $acceptFeedback(handle, result, vote, reportIssue) {
    ...
    agent.acceptFeedback(reportIssue ? Object.freeze({
      result: ehResult,
      kind,
      reportIssue
    }) : Object.freeze({
  ...
  async $invokeCompletionProvider(handle, query, token) {
    ...
    return items.map(i => ChatAgentCompletionItem.from(i, this.commands.converter, this._completionDisposables));
  }
  ...
  async $provideSampleQuestions(handle, location, token) {
    ...
    return (await agent.provideSampleQuestions(ChatLocation.to(location), token)).map(f => ChatFollowup.from(f, undefined));
  }
}
class ExtHostChatAgent {
  ...
  get apiAgent() {
    ...
    return {
      ...
      onDidPerformAction: !isProposedApiEnabled(this.extension, 'chatParticipantAdditions') ? undefined : this._onDidPerformAction.event,
      set requester(v) {
        that._requester = v;
        updateMetadataSoon();
      },
