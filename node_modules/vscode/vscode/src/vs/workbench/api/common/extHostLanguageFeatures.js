...
const {
  ...
  NewSymbolNameTriggerKind$1,
  SemanticTokens,
  SemanticTokensEdits,
  SemanticTokensEdit,
  SnippetString,
  InlineEditTriggerKind$1,
  Location,
  CompletionList,
  InlineCompletionTriggerKind$1
...
class DocumentPasteEditProvider {
  ...
  async providePasteEdits(requestId, resource, ranges, dataTransferDto, context, token) {
    ...
    return edits.map((edit, i) => ({
      _cacheId: [cacheId, i],
      title: edit.title ?? localizeWithPath(_moduleId, 0, "Paste using '{0}' extension", this._extension.displayName || this._extension.name),
      kind: edit.kind,
...
class CompletionsAdapter {
  ...
  async resolveCompletionItem(id, token) {
    ...
    if (dto1["n"] !== dto2["n"] || dto1["o"] !== dto2["o"] || !equals(dto1["p"], dto2["p"])) {
  ...
  async provideInlineCompletions(resource, position, context, token) {
    ...
    return {
      ...
      items: normalizedResult.map((item, idx) => {
        ...
        return {
          insertText: typeof insertText === 'string' ? insertText : {
...
class DocumentDropEditAdapter {
  ...
  async provideDocumentOnDropEdits(requestId, uri, position, dataTransferDto, token) {
    ...
    return editsArray.map((edit, i) => ({
      _cacheId: [cacheId, i],
      title: edit.title ?? localizeWithPath(_moduleId, 1, "Drop using '{0}' extension", this._extension.displayName || this._extension.name),
