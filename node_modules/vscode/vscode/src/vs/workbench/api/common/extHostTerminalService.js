...
} = require('../../../platform/terminal/common/environmentVariableShared.js');
const {
...
} = require('../../../platform/terminal/common/terminalDataBuffering.js');
const {
  ThemeColor
...
let BaseExtHostTerminalService = class BaseExtHostTerminalService extends Disposable {
  ...
  async $startExtensionTerminal(id, initialDimensions) {
    ...
    if (!terminal) {
      return {
        message: localizeWithPath(_moduleId, 0, "Could not find the terminal with id {0} on the extension host", id)
      };
    }
    if (!terminal.isOpen) {
      await new Promise(r => {
  ...
  async $provideLinks(terminalId, line) {
    ...
    for (const provider of this._linkProviders) {
      promises.push(Promises.withAsyncBody(async r => {
        cancellationSource.token.onCancellationRequested(() => r({
          provider,
          links: []
        }));
...
WorkerExtHostTerminalService = __decorate([__param(0, IExtHostCommands), __param(1, IExtHostRpcService)], WorkerExtHostTerminalService);
...
exports.IExtHostTerminalService = IExtHostTerminalService;
