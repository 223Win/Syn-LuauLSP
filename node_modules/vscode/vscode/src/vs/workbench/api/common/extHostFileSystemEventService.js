...
class FileSystemWatcher {
  ...
  ensureWatching(mainContext, extension, globPattern, options, correlate) {
    ...
    proxy.$watch(extension.identifier.value, this.session, globPattern.baseUri, {
      recursive,
      excludes: options?.excludes ?? [],
      filter
    }, Boolean(correlate));
    return Disposable.from({
      dispose: () => proxy.$unwatch(this.session)
    });
  }
  dispose() {
...
class ExtHostFileSystemEventService {
  ...
  createFileSystemWatcher(workspace, extension, globPattern, options) {
    return new FileSystemWatcher(this._mainContext, workspace, extension, this._onFileSystemEvent.event, GlobPattern.from(globPattern), options);
  }
  $onFileEvent(events) {
