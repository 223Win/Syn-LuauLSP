...
const {
  asWebviewUri
} = require('../../contrib/webview/common/webview.js');
const {
  CellExecutionUpdateType
} = require('../../contrib/notebook/common/notebookExecutionService.js');
const {
...
const {
  variablePageSize
} = require('../../contrib/notebook/common/notebookKernelService.js');
let ExtHostNotebookKernels = class ExtHostNotebookKernels {
  constructor(mainContext, _initData, _extHostNotebook, _commands, _logService) {
    ...
    const selectKernelApiCommand = new ApiCommand('notebook.selectKernel', '_notebook.selectKernel', 'Trigger kernel picker for specified notebook editor widget', [new ApiCommandArgument('options', 'Select kernel options', v => true, v => {
  ...
  async $provideKernelSourceActions(handle, token) {
    ...
    if (provider) {
      ...
      return (ret ?? []).map(item => NotebookKernelSourceAction.from(item, this._commands.converter, disposables));
    }
    return [];
  }
  ...
  $acceptKernelMessageFromRenderer(handle, editorId, message) {
    ...
    obj.onDidReceiveMessage.fire(Object.freeze({
...
ExtHostNotebookKernels = __decorate([__param(4, ILogService)], ExtHostNotebookKernels);
