...
class ExtHostWorkspaceImpl extends Workspace {
  static toExtHostWorkspace(data, previousConfirmedWorkspace, previousUnconfirmedWorkspace, extHostFileSystemInfo) {
    ...
    const {
      added,
      removed
    } = delta(oldWorkspace ? oldWorkspace.workspaceFolders : [], workspace.workspaceFolders, compareWorkspaceFolderByUri, extHostFileSystemInfo);
    return {
      workspace,
      added,
      removed
...
let ExtHostWorkspace = class ExtHostWorkspace {
  ...
  updateWorkspaceFolders(extension, index, deleteCount, ...workspaceFoldersToAdd) {
    ...
    const {
      added,
      removed
    } = delta(currentWorkspaceFolders, newWorkspaceFolders, compareWorkspaceFolderByUriAndNameAndIndex, this._extHostFileSystemInfo);
    ...
    if (this._proxy) {
      ...
      this._proxy.$updateWorkspaceFolders(extName, index, deleteCount, validatedDistinctWorkspaceFoldersToAdd).then(undefined, error => {
        ...
        this._messageService.$showMessage(Severity$1.Error, localizeWithPath(_moduleId, 0, "Extension '{0}' failed to update workspace folders: {1}", extName, error.toString()), options, []);
      });
    }
  ...
  $acceptWorkspaceData(data) {
    const {
      ...
      added,
      removed
    } = ExtHostWorkspaceImpl.toExtHostWorkspace(data, this._confirmedWorkspace, this._unconfirmedWorkspace, this._extHostFileSystemInfo);
  ...
  async _findFilesImpl(include, filePattern, options, token = CancellationToken.None) {
    ...
    if (include) {
      const {
        includePattern,
        folder
      } = parseSearchInclude(GlobPattern.from(include));
      folderToUse = folder;
      ...
      const {
        includePattern,
        folder
      } = parseSearchInclude(GlobPattern.from(filePattern));
      folderToUse = folder;
      fileQueries.filePattern = includePattern;
    }
    return this._proxy.$startFileSearch(folderToUse ?? null, fileQueries, token).then(data => Array.isArray(data) ? data.map(d => URI.revive(d)) : []);
  }
  async findTextInFiles(query, options, callback, extensionId, token = CancellationToken.None) {
    ...
    const {
      includePattern,
      folder
    } = parseSearchInclude(GlobPattern.from(options.include));
    const excludePattern = typeof options.exclude === 'string' ? options.exclude : options.exclude ? options.exclude.pattern : undefined;
    const queryOptions = {
      ignoreSymlinks: typeof options.followSymlinks === 'boolean' ? !options.followSymlinks : undefined,
      disregardIgnoreFiles: typeof options.useIgnoreFiles === 'boolean' ? !options.useIgnoreFiles : undefined,
...
exports.IExtHostWorkspace = IExtHostWorkspace;