...
class ExtHostInteractiveEditor {
  ...
  constructor(mainContext, extHostCommands, _documents, _logService) {
    ...
    extHostCommands.registerApiCommand(new ApiCommand('vscode.editorChat.start', 'inlineChat.start', 'Invoke a new editor chat session', [new ApiCommandArgument('Run arguments', '', _v => true, v => {
      if (!v) {
        return undefined;
      }
      return {
  ...
  async $prepareSession(handle, uri, range, token) {
    ...
    const session = await entry.provider.prepareInteractiveEditorSession({
      document,
      selection
    }, token);
    if (!session) {
