...
const {
  ...
  ExtHostCell
} = require('./extHostNotebookDocument.js');
const {
  ExtHostNotebookEditor
} = require('./extHostNotebookEditor.js');
const {
  filter
...
const {
  CellSearchModel
} = require('../../contrib/search/common/cellSearchModel.js');
const {
  genericCellMatchesToTextSearchMatches
} = require('../../contrib/search/common/searchNotebookHelpers.js');
const {
  globMatchesResource
...
class ExtHostNotebookController {
  ...
  async $saveNotebook(handle, uriComponents, versionId, options, token) {
    ...
    if (!this._extHostFileSystem.value.isWritableFileSystem(uri.scheme)) {
      throw new FileOperationError(localizeWithPath(_moduleId, 0, "Unable to modify read-only file '{0}'", this._resourceForError(uri)), 6);
    }
    ...
    const promises = Array.from(finalMatchedTargets).map(async uri => {
      ...
      try {
        ...
        simpleCells.forEach((cell, index) => {
          ...
          const webviewResults = outputMatches.flatMap(outputMatch => genericCellMatchesToTextSearchMatches(outputMatch.matches, outputMatch.textBuffer)).map((textMatch, index) => {
  ...
  async _validateWriteFile(uri, options) {
    ...
    if (typeof options?.mtime === 'number' && typeof options.etag === 'string' && options.etag !== ETAG_DISABLED && typeof stat.mtime === 'number' && typeof stat.size === 'number' && options.mtime < stat.mtime && options.etag !== etag({
      ...
      throw new FileOperationError(localizeWithPath(_moduleId, 1, "File Modified Since"), 3, options);
    }
    return;
  }
