...
const {} = require('./textAreaHandler.css.js');
const {
  localizeWithPath
} = require('../../../nls.js');
const {
...
const {
  ...
  TextAreaInput,
  CopyOptions
} = require('./textAreaInput.js');
const {
  TextAreaState,
  PagedScreenReaderStrategy
} = require('./textAreaState.js');
const {
  ViewPart,
  PartFingerprints
...
} = require('../../../base/browser/ui/mouseCursor/mouseCursor.js');
const {
...
const _moduleId = "vs/editor/browser/controller/textAreaHandler";
...
let TextAreaHandler = class TextAreaHandler extends ViewPart {
  constructor(context, viewController, visibleRangeProvider, _keybindingService, _instantiationService) {
    ...
    this.textArea.setAttribute('aria-roledescription', localizeWithPath(_moduleId, 0, "editor"));
  ...
  _getWordBeforePosition(position) {
    const lineContent = this._context.viewModel.getLineContent(position.lineNumber);
    const wordSeparators = getMapForWordSeparators(this._context.configuration.options.get(131), []);
  ...
  _getAriaLabel(options) {
    ...
    if (accessibilitySupport === 1) {
      ...
      const editorNotAccessibleMessage = localizeWithPath(_moduleId, 1, "The editor is not accessible at this time.");
      if (toggleKeybindingLabel) {
        return localizeWithPath(_moduleId, 2, "{0} To enable screen reader optimized mode, use {1}", editorNotAccessibleMessage, toggleKeybindingLabel);
      } else if (runCommandKeybindingLabel) {
        return localizeWithPath(_moduleId, 3, "{0} To enable screen reader optimized mode, open the quick pick with {1} and run the command Toggle Screen Reader Accessibility Mode, which is currently not triggerable via keyboard.", editorNotAccessibleMessage, runCommandKeybindingLabel);
      } else if (keybindingEditorKeybindingLabel) {
        return localizeWithPath(_moduleId, 4, "{0} Please assign a keybinding for the command Toggle Screen Reader Accessibility Mode by accessing the keybindings editor with {1} and run it.", editorNotAccessibleMessage, keybindingEditorKeybindingLabel);
      } else {
