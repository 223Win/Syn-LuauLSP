...
const {
  DiffEditorOptions
} = require('../diffEditor/diffEditorOptions.js');
const {
  DiffEditorViewModel
} = require('../diffEditor/diffEditorViewModel.js');
const {
  IModelService
...
class MultiDiffEditorViewModel extends Disposable {
  ...
  constructor(model, _instantiationService) {
    ...
    this.items = mapObservableArrayCached(this, this._documents, (d, store) => store.add(this._instantiationService.createInstance(DocumentDiffItemViewModel, d, this))).recomputeInitiallyAndOnChange(this._store);
...
let DocumentDiffItemViewModel = class DocumentDiffItemViewModel extends Disposable {
  get originalUri() {
    return this.entry.value.original?.uri;
  }
  get modifiedUri() {
    return this.entry.value.modified?.uri;
  }
