...
} = require('./diffProviderFactoryService.js');
const {
...
} = require('../../../common/diff/defaultLinesDiffComputer/defaultLinesDiffComputer.js');
const {
...
const {
  TextEditInfo
} = require('../../../common/model/bracketPairsTextModelPart/bracketPairsTree/beforeEditPositionMapper.js');
const {
  combineTextEditInfos
} = require('../../../common/model/bracketPairsTextModelPart/bracketPairsTree/combineTextEditInfos.js');
const {
  optimizeSequenceDiffs
} = require('../../../common/diff/defaultLinesDiffComputer/heuristicSequenceOptimizations.js');
const {
  isDefined
    ...
    const updateUnchangedRegions = (result, tx, reader) => {
      ...
      if (lastUnchangedRegions) {
        ...
        const updatedLastUnchangedRegions = filterWithPrevious(lastUnchangedRegions.regions.map((r, idx) => {
          ...
          return new UnchangedRegion(lastUnchangedRegionsOrigRanges[idx].startLineNumber, lastUnchangedRegionsModRanges[idx].startLineNumber, length, Math.min(r.visibleLineCountTop.get(), length), Math.min(r.visibleLineCountBottom.get(), length - r.visibleLineCountTop.get()));
