...
const _moduleId = "vs/editor/browser/widget/diffEditor/features/hideUnchangedRegionsFeature";
let HideUnchangedRegionsFeature = class HideUnchangedRegionsFeature extends Disposable {
  static {
    HideUnchangedRegionsFeature_1 = this;
  }
  static {
    this._breadcrumbsSourceFactory = observableValue('breadcrumbsSourceFactory', undefined);
  }
  ...
  constructor(_editors, _diffModel, _options, _instantiationService) {
    ...
    this.viewZones = derivedWithStore(this, (reader, store) => {
      ...
      for (const r of curUnchangedRegions) {
        ...
        }
        {
          ...
          store.add(new CollapsedCodeOverlayWidget(this._editors.original, origVz, r, r.originalUnchangedRange, !sideBySide, modifiedOutlineSource, l => this._diffModel.get().ensureModifiedLineIsVisible(l, 2, undefined), this._options));
        }
        {
          ...
          store.add(new CollapsedCodeOverlayWidget(this._editors.modified, modViewZone, r, r.modifiedUnchangedRange, false, modifiedOutlineSource, l => this._diffModel.get().ensureModifiedLineIsVisible(l, 2, undefined), this._options));
        }
      }
      return {
        origViewZones,
    ...
    const unchangedLinesDecorationShow = {
      ...
      }).appendMarkdown(localizeWithPath(_moduleId, 0, 'Fold Unchanged Region')),
...
class CollapsedCodeOverlayWidget extends ViewZoneOverlayWidget {
  constructor(_editor, _viewZone, _unchangedRegion, _unchangedRegionRange, _hide, _modifiedOutlineSource, _revealModifiedHiddenLine, _options) {
    ...
    this._nodes = h('div.diff-hidden-lines', [h('div.top@top', {
      title: localizeWithPath(_moduleId, 1, 'Click or drag to show more above')
      ...
      title: localizeWithPath(_moduleId, 2, 'Show Unchanged Region'),
      role: 'button',
      onclick: () => {
      ...
      title: localizeWithPath(_moduleId, 3, 'Click or drag to show more below'),
      role: 'button'
    })]);
    ...
    this._register(addDisposableListener(this._nodes.top, 'mousedown', e => {
      ...
      const mouseMoveListener = addDisposableListener(window, 'mousemove', e => {
        ...
        const lineDelta = Math.round(delta / editor.getOption(67));
    ...
    this._register(autorun(reader => {
      ...
      if (!this._hide) {
        ...
        const linesHiddenText = localizeWithPath(_moduleId, 4, '{0} hidden lines', lineCount);
        const span = $('span', {
          title: localizeWithPath(_moduleId, 5, 'Double click to unfold')
        }, linesHiddenText);
        ...
        if (items.length > 0) {
          ...
          for (let i = 0; i < items.length; i++) {
            ...
            }, [renderIcon(icon), '\u00a0', item.name, ...(i === items.length - 1 ? [] : [renderIcon(Codicon.chevronRight)])]).root;
