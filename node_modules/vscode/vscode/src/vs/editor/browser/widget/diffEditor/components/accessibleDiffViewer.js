...
const {
  OffsetRange
} = require('../../../../common/core/offsetRange.js');
const {
  Position
...
const {
  LineTokens
} = require('../../../../common/tokens/lineTokens.js');
const {
  renderViewLine2,
  RenderLineInput
} = require('../../../../common/viewLayout/viewLineRenderer.js');
const {
  ViewLineRenderingData
} = require('../../../../common/viewModel.js');
const {
  localizeWithPath
} = require('../../../../../nls.js');
const {
...
const {} = require('./accessibleDiffViewer.css.js');
const _moduleId = "vs/editor/browser/widget/diffEditor/components/accessibleDiffViewer";
const accessibleDiffViewerInsertIcon = registerIcon('diff-review-insert', Codicon.add, localizeWithPath(_moduleId, 0, 'Icon for \'Insert\' in accessible diff viewer.'));
const accessibleDiffViewerRemoveIcon = registerIcon('diff-review-remove', Codicon.remove, localizeWithPath(_moduleId, 1, 'Icon for \'Remove\' in accessible diff viewer.'));
const accessibleDiffViewerCloseIcon = registerIcon('diff-review-close', Codicon.close, localizeWithPath(_moduleId, 2, 'Icon for \'Close\' in accessible diff viewer.'));
...
let View = class View extends Disposable {
  constructor(_element, _model, _width, _height, _models, _languageService) {
    ...
    this._register(autorun(reader => {
      ...
      if (this._model.canClose.read(reader)) {
        this._actionBar.push(new Action('diffreview.close', localizeWithPath(_moduleId, 3, "Close"), 'close-diff-review ' + ThemeIcon.asClassName(accessibleDiffViewerCloseIcon), true, async () => _model.close()), {
          label: false,
          icon: true
        });
      }
  ...
  _render(store) {
    ...
    container.setAttribute('aria-label', localizeWithPath(_moduleId, 4, 'Accessible Diff Viewer. Use arrow up and down to navigate.'));
    ...
    for (const viewItem of group?.lines || []) {
      ...
      if (viewItem.type === LineType.Header) {
        ...
        const getAriaLines = lines => lines === 0 ? localizeWithPath(_moduleId, 5, "no lines changed") : lines === 1 ? localizeWithPath(_moduleId, 6, "1 line changed") : localizeWithPath(_moduleId, 7, "{0} lines changed", lines);
        ...
        header.setAttribute('aria-label', localizeWithPath(_moduleId, 8, "Difference {0} of {1}: original line {2}, {3}, modified line {4}, {5}", diffIndex + 1, diffsLength, r.original.startLineNumber, originalChangedLinesCntAria, r.modified.startLineNumber, modifiedChangedLinesCntAria));
  ...
  _createRow(item, lineHeight, width, originalOptions, originalModel, originalModelOpts, modifiedOptions, modifiedModel, modifiedModelOpts) {
    const originalLayoutInfo = originalOptions.get(145);
    const originalLineNumbersWidth = originalLayoutInfo.glyphMarginWidth + originalLayoutInfo.lineNumbersWidth;
    const modifiedLayoutInfo = modifiedOptions.get(145);
    ...
    if (lineContent.length === 0) {
      lineContent = localizeWithPath(_moduleId, 9, "blank");
    }
    let ariaLabel = '';
    switch (item.type) {
      case LineType.Unchanged:
        if (item.originalLineNumber === item.modifiedLineNumber) {
          ariaLabel = localizeWithPath(_moduleId, 10, "{0} unchanged line {1}", lineContent, item.originalLineNumber);
        } else {
          ariaLabel = localizeWithPath(_moduleId, 11, "{0} original line {1} modified line {2}", lineContent, item.originalLineNumber, item.modifiedLineNumber);
        }
        break;
      case LineType.Added:
        ariaLabel = localizeWithPath(_moduleId, 12, "+ {0} modified line {1}", lineContent, item.modifiedLineNumber);
        break;
      case LineType.Deleted:
        ariaLabel = localizeWithPath(_moduleId, 13, "- {0} original line {1}", lineContent, item.originalLineNumber);
        break;
    }
  ...
  _getLineHtml(model, options, tabSize, lineNumber, languageIdCodec) {
    ...
    const r = renderViewLine2(new RenderLineInput(fontInfo.isMonospace && !options.get(33), fontInfo.canUseHalfwidthRightwardsArrow, lineContent, false, isBasicASCII, containsRTL, 0, lineTokens, [], tabSize, 0, fontInfo.spaceWidth, fontInfo.middotWidth, fontInfo.wsmiddotWidth, options.get(117), options.get(99), options.get(94), options.get(51) !== EditorFontLigatures.OFF, null));
    return r.html;
  }
};
