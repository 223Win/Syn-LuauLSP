...
} = require('./stickyScrollWidget.js');
const {
  StickyLineCandidateProvider
} = require('./stickyScrollProvider.js');
const {
...
} = require('../../gotoSymbol/browser/goToSymbol.js');
const {
  goToDefinitionWithLocation
} = require('../../inlayHints/browser/inlayHintsLocations.js');
const {
  Position
...
const {
  StickyRange
} = require('./stickyScrollElement.js');
const {
  StandardMouseEvent
} = require('../../../../base/browser/mouseEvent.js');
const {
  FoldingController
} = require('../../folding/browser/folding.js');
const {
  toggleCollapseState
} = require('../../folding/browser/foldingModel.js');
...
let StickyScrollController = class StickyScrollController extends Disposable {
  ...
  _toggleFoldingRegionForLine(line) {
    ...
    const scrollTop = (foldingIcon.isCollapsed ? this._editor.getTopForLineNumber(foldingIcon.foldingEndLine) : this._editor.getTopForLineNumber(foldingIcon.foldingStartLine)) - this._editor.getOption(67) * stickyLine.index + 1;
  ...
  _readConfigurationChange(event) {
    if (event.hasChanged(115) || event.hasChanged(73) || event.hasChanged(67) || event.hasChanged(110) || event.hasChanged(68)) {
      this._readConfiguration();
    }
    if (event.hasChanged(68)) {
      this._renderStickyScroll(0);
    }
  }
  _needsUpdate(event) {
