const {
  MarkdownString
} = require('../../../../base/common/htmlContent.js');
const {
  Disposable
} = require('../../../../base/common/lifecycle.js');
const {
  registerEditorContribution
} = require('../../../browser/editorExtensions.js');
const {
  MessageController
} = require('../../message/browser/messageController.js');
const {
  localizeWithPath
} = require('../../../../nls.js');
const _moduleId = "vs/editor/contrib/readOnlyMessage/browser/contribution";
class ReadOnlyMessageController extends Disposable {
  static {
    this.ID = 'editor.contrib.readOnlyMessageController';
  }
  constructor(editor) {
    super();
    this.editor = editor;
    this._register(this.editor.onDidAttemptReadOnlyEdit(() => this._onDidAttemptReadOnlyEdit()));
  }
  _onDidAttemptReadOnlyEdit() {
    const messageController = MessageController.get(this.editor);
    if (messageController && this.editor.hasModel()) {
      let message = this.editor.getOptions().get(92);
      if (!message) {
        if (this.editor.isSimpleWidget) {
          message = new MarkdownString(localizeWithPath(_moduleId, 0, "Cannot edit in read-only input"));
        } else {
          message = new MarkdownString(localizeWithPath(_moduleId, 1, "Cannot edit in read-only editor"));
        }
      }
      messageController.showMessage(message, this.editor.getPosition());
    }
  }
}
registerEditorContribution(ReadOnlyMessageController.ID, ReadOnlyMessageController, 2);
exports.ReadOnlyMessageController = ReadOnlyMessageController;