...
const {} = require('./goToDefinitionAtPosition.css.js');
const {
  EditorState
...
const _moduleId = "vs/editor/contrib/gotoSymbol/browser/link/goToDefinitionAtPosition";
let GotoDefinitionAtPositionEditorContribution = class GotoDefinitionAtPositionEditorContribution {
  ...
  async startFindDefinition(position) {
    ...
    if (results.length > 1) {
      ...
      this.addDecoration(combinedRange, new MarkdownString().appendText(localizeWithPath(_moduleId, 0, "Click to show {0} definitions.", results.length)));
    } else {
