...
const {
  GhostTextWidget
} = require('./ghostTextWidget.js');
const {
  InlineCompletionContextKeys
} = require('./inlineCompletionContextKeys.js');
const {
...
} = require('./inlineCompletionsModel.js');
const {
  SuggestWidgetAdaptor
} = require('./suggestWidgetInlineCompletionProvider.js');
const {
  localizeWithPath
} = require('../../../../nls.js');
const {
...
const _moduleId = "vs/editor/contrib/inlineCompletions/browser/inlineCompletionsController";
let InlineCompletionsController = class InlineCompletionsController extends Disposable {
  ...
  constructor(editor, _instantiationService, _contextKeyService, _configurationService, _commandService, _debounceService, _languageFeaturesService, _accessibilitySignalService, _keybindingService, _accessibilityService) {
    ...
    this._enabledInConfig = observableFromEvent(this.editor.onDidChangeConfiguration, () => this.editor.getOption(62).enabled);
...
.monaco-editor .ghost-text {
    ...
    this._register(editor.onDidChangeModelContent(e => transaction(tx => this.updateObservables(tx, getReason(e)))));
    ...
    this._register(this.editor.onDidBlurEditorWidget(() => {
      if (this._contextKeyService.getContextKeyValue('accessibleViewIsShown') || this._configurationService.getValue('editor.inlineSuggest.keepOnBlur') || editor.getOption(62).keepOnBlur) {
        return;
      }
  ...
  provideScreenReaderUpdate(content) {
    ...
    let hint;
    if (!accessibleViewShowing && accessibleViewKeybinding && this.editor.getOption(149)) {
      hint = localizeWithPath(_moduleId, 0, "Inspect this in the accessible view ({0})", accessibleViewKeybinding.getAriaLabel());
    }
