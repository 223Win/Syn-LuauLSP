...
} = require('../../inlineCompletions/browser/inlineCompletionsHintsWidget.js');
const {
  IKeybindingService
...
const {
  ContentHoverWidget
} = require('./contentHoverWidget.js');
const {
  ContentHoverController
} = require('./contentHoverController.js');
const {} = require('./hover.css.js');
const {
  MarginHoverWidget
} = require('./marginHoverWidget.js');
var HoverController_1;
const _sticky = false;
let HoverController = class HoverController extends Disposable {
  static {
    HoverController_1 = this;
  }
  static {
    this.ID = 'editor.contrib.hover';
  }
  ...
  _hookListeners() {
    const hoverOpts = this._editor.getOption(60);
    this._hoverSettings = {
  ...
  _isMouseOnMarginHoverWidget(mouseEvent) {
    ...
    return target.type === 12 && target.detail === MarginHoverWidget.ID;
  }
  _isMouseOnContentHoverWidget(mouseEvent) {
    ...
    return target.type === 9 && target.detail === ContentHoverWidget.ID;
  }
  _onEditorMouseUp() {
  ...
  _shouldNotRecomputeCurrentHoverWidget(mouseEvent) {
    ...
    const isTextSelectedWithinContentHoverWidget = (mouseEvent, sticky) => {
      return sticky && this._contentWidget?.containsNode(mouseEvent.event.browserEvent.view?.document.activeElement) && !mouseEvent.event.browserEvent.view?.getSelection()?.isCollapsed;
    };
  ...
  _reactToEditorMouseMove(mouseEvent) {
    ...
    const decoratorActivatedOn = this._editor.getOption(148);
    ...
    const contentHoverShowsOrWillShow = this._tryShowHoverWidget(mouseEvent, 0);
    if (contentHoverShowsOrWillShow) {
      return;
    }
    const glyphWidgetShowsOrWillShow = this._tryShowHoverWidget(mouseEvent, 1);
  ...
  _onKeyDown(e) {
    ...
    const shouldKeepHoverVisible = resolvedKeyboardEvent.kind === 1 || resolvedKeyboardEvent.kind === 2 && (resolvedKeyboardEvent.commandId === SHOW_OR_FOCUS_HOVER_ACTION_ID || resolvedKeyboardEvent.commandId === INCREASE_HOVER_VERBOSITY_ACTION_ID || resolvedKeyboardEvent.commandId === DECREASE_HOVER_VERBOSITY_ACTION_ID) && this._contentWidget?.isVisible;
    if (e.keyCode === 5 || e.keyCode === 6 || e.keyCode === 57 || e.keyCode === 4 || shouldKeepHoverVisible) {
      return;
    }
    this._hideWidgets();
  }
