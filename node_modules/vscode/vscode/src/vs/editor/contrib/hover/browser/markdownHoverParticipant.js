...
} = require('../../../../platform/hover/browser/hover.js');
const {
  IHoverService
...
let MarkdownHoverParticipant = class MarkdownHoverParticipant {
  ...
  createLoadingMessage(anchor) {
    return new MarkdownHover(this, anchor.range, [new MarkdownString().appendText(localizeWithPath(_moduleId, 2, "Loading..."))], false, 2000);
  }
  computeSync(anchor, lineDecorations) {
    ...
    if (stopRenderingLineAfter >= 0 && lineLength > stopRenderingLineAfter && anchor.range.startColumn >= stopRenderingLineAfter) {
      ...
      result.push(new MarkdownHover(this, anchor.range, [{
        value: localizeWithPath(_moduleId, 3, "Rendering paused for long line for performance reasons. This can be configured via `editor.stopRenderingLineAfter`.")
      }], false, index++));
    }
    if (!stopRenderingMessage && typeof maxTokenizationLineLength === 'number' && lineLength >= maxTokenizationLineLength) {
      result.push(new MarkdownHover(this, anchor.range, [{
        value: localizeWithPath(_moduleId, 4, "Tokenization is skipped for long lines for performance reasons. This can be configured via `editor.maxTokenizationLineLength`.")
      }], false, index++));
    }
...
class MarkdownRenderedHoverParts extends Disposable {
  constructor(hoverParts, hoverPartsContainer, _editor, _languageService, _openerService, _keybindingService, _hoverService, _configurationService, _onFinishedRendering) {
    super();
    this._editor = _editor;
    this._languageService = _languageService;
    this._openerService = _openerService;
  ...
  _renderHoverExpansionAction(container, action, actionEnabled) {
    ...
    if (isActionIncrease) {
      ...
      store.add(this._hoverService.setupUpdatableHover(hoverDelegate, actionElement, kb ? localizeWithPath(_moduleId, 5, "Increase Verbosity ({0})", kb.getLabel()) : localizeWithPath(_moduleId, 6, "Increase Verbosity")));
    } else {
      const kb = this._keybindingService.lookupKeybinding(DECREASE_HOVER_VERBOSITY_ACTION_ID);
      store.add(this._hoverService.setupUpdatableHover(hoverDelegate, actionElement, kb ? localizeWithPath(_moduleId, 7, "Decrease Verbosity ({0})", kb.getLabel()) : localizeWithPath(_moduleId, 8, "Decrease Verbosity")));
    }
    if (!actionEnabled) {
    ...
    store.add(new KeyDownAction(actionElement, actionFunction, [3, 10]));
    return store;
  }
