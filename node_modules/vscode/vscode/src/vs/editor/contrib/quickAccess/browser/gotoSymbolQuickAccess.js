...
} = require('./editorNavigationQuickAccess.js');
const {
  localizeWithPath
} = require('../../../../nls.js');
const {
...
const _moduleId = "vs/editor/contrib/quickAccess/browser/gotoSymbolQuickAccess";
let AbstractGotoSymbolQuickAccessProvider = class AbstractGotoSymbolQuickAccessProvider extends AbstractEditorNavigationQuickAccessProvider {
  ...
  static {
    this.PREFIX_BY_CATEGORY = `${AbstractGotoSymbolQuickAccessProvider_1.PREFIX}${AbstractGotoSymbolQuickAccessProvider_1.SCOPE_PREFIX}`;
  }
  ...
  provideWithoutTextEditor(picker) {
    this.provideLabelPick(picker, localizeWithPath(_moduleId, 0, "To go to a symbol, first open a text editor with symbol information."));
  ...
  doProvideWithoutEditorSymbols(context, model, picker, token) {
    ...
    this.provideLabelPick(picker, localizeWithPath(_moduleId, 1, "The active text editor does not provide symbol information."));
    (async () => {
      ...
      try {
        ...
        if (items.length > 0) {
          ...
          if (query.original.length > 0) {
            this.provideLabelPick(picker, localizeWithPath(_moduleId, 2, "No matching editor symbols"));
          } else {
            this.provideLabelPick(picker, localizeWithPath(_moduleId, 3, "No editor symbols"));
          }
        }
      } finally {
        if (!token.isCancellationRequested) {
  ...
  async doGetSymbolPicks(symbolsPromise, query, options, token) {
    ...
    if (openSideBySideDirection) {
      buttons = [{
        ...
        tooltip: openSideBySideDirection === 'right' ? localizeWithPath(_moduleId, 4, "Open to the Side") : localizeWithPath(_moduleId, 5, "Open to the Bottom")
      }];
    }
    ...
    if (filterBySymbolKind) {
      ...
      symbolPicks = [{
        label: localizeWithPath(_moduleId, 6, "symbols ({0})", filteredSymbolPicks.length),
        type: 'separator'
...
const FALLBACK_NLS_SYMBOL_KIND = localizeWithPath(_moduleId, 7, "properties ({0})");
const NLS_SYMBOL_KIND_CACHE = {
  [5]: localizeWithPath(_moduleId, 8, "methods ({0})"),
  [11]: localizeWithPath(_moduleId, 9, "functions ({0})"),
  [8]: localizeWithPath(_moduleId, 10, "constructors ({0})"),
  [12]: localizeWithPath(_moduleId, 11, "variables ({0})"),
  [4]: localizeWithPath(_moduleId, 12, "classes ({0})"),
  [22]: localizeWithPath(_moduleId, 13, "structs ({0})"),
  [23]: localizeWithPath(_moduleId, 14, "events ({0})"),
  [24]: localizeWithPath(_moduleId, 15, "operators ({0})"),
  [10]: localizeWithPath(_moduleId, 16, "interfaces ({0})"),
  [2]: localizeWithPath(_moduleId, 17, "namespaces ({0})"),
  [3]: localizeWithPath(_moduleId, 18, "packages ({0})"),
  [25]: localizeWithPath(_moduleId, 19, "type parameters ({0})"),
  [1]: localizeWithPath(_moduleId, 20, "modules ({0})"),
  [6]: localizeWithPath(_moduleId, 7, "properties ({0})"),
  [9]: localizeWithPath(_moduleId, 21, "enumerations ({0})"),
  [21]: localizeWithPath(_moduleId, 22, "enumeration members ({0})"),
  [14]: localizeWithPath(_moduleId, 23, "strings ({0})"),
  [0]: localizeWithPath(_moduleId, 24, "files ({0})"),
  [17]: localizeWithPath(_moduleId, 25, "arrays ({0})"),
  [15]: localizeWithPath(_moduleId, 26, "numbers ({0})"),
  [16]: localizeWithPath(_moduleId, 27, "booleans ({0})"),
  [18]: localizeWithPath(_moduleId, 28, "objects ({0})"),
  [19]: localizeWithPath(_moduleId, 29, "keys ({0})"),
  [7]: localizeWithPath(_moduleId, 30, "fields ({0})"),
  [13]: localizeWithPath(_moduleId, 31, "constants ({0})")
};
