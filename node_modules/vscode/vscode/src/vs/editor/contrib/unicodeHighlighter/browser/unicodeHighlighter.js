...
const {} = require('./unicodeHighlighter.css.js');
const {
  EditorAction,
...
} = require('../../../common/services/unicodeTextModelHighlighter.js');
const {
...
} = require('../../../common/viewModel/viewModelDecorations.js');
const {
...
const warningIcon = registerIcon('extensions-warning-message', Codicon.warning, localizeWithPath(_moduleId, 0, 'Icon shown with a warning message in the extensions editor.'));
    ...
    this._updateState = state => {
      if (state && state.hasMore) {
        ...
        if (state.nonBasicAsciiCharacterCount >= max) {
          data = {
            message: localizeWithPath(_moduleId, 1, 'This document contains many non-basic ASCII unicode characters'),
        ...
        case 0:
          {
            if (isBasicASCII(highlightInfo.reason.confusableWith)) {
              reason = localizeWithPath(_moduleId, 5, 'The character {0} could be confused with the ASCII character {1}, which is more common in source code.', codePointStr, formatCodePointMarkdown(highlightInfo.reason.confusableWith.codePointAt(0)));
            } else {
              reason = localizeWithPath(_moduleId, 6, 'The character {0} could be confused with the character {1}, which is more common in source code.', codePointStr, formatCodePointMarkdown(highlightInfo.reason.confusableWith.codePointAt(0)));
            }
            break;
          }
        case 1:
          ...
          break;
        case 2:
          reason = localizeWithPath(_moduleId, 8, 'The character {0} is not a basic ASCII character.', codePointStr);
          break;
      }
...
class ShowExcludeOptions extends EditorAction {
  ...
  async run(accessor, editor, args) {
    ...
    function getExcludeCharFromBeingHighlightedLabel(codePoint) {
      if (InvisibleCharacters.isInvisibleCharacter(codePoint)) {
        return localizeWithPath(_moduleId, 21, 'Exclude {0} (invisible character) from being highlighted', codePointToHex(codePoint));
      }
      return localizeWithPath(_moduleId, 22, 'Exclude {0} from being highlighted', `${codePointToHex(codePoint)} "${char}"`);
    }
    const options = [];
    if (reason.kind === 0) {
      for (const locale of reason.notAmbiguousInLocales) {
        options.push({
          label: localizeWithPath(_moduleId, 23, "Allow unicode characters that are more common in the language \"{0}\".", locale),
          run: async () => {
