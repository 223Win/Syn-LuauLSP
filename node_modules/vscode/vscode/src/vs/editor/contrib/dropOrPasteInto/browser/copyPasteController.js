...
const {
  ClipboardEventUtils
} = require('../../../browser/controller/textAreaInput.js');
const {
  toVSDataTransfer,
...
} = require('../../inlineProgress/browser/inlineProgress.js');
const {
  MessageController
...
} = require('./postEditWidget.js');
var CopyPasteController_1;
const _moduleId = "vs/editor/contrib/dropOrPasteInto/browser/copyPasteController";
const changePasteTypeCommandId = 'editor.changePasteType';
const pasteWidgetVisibleCtx = new RawContextKey('pasteWidgetVisible', false, localizeWithPath(_moduleId, 0, "Whether the paste widget is showing"));
...
let CopyPasteController = class CopyPasteController extends Disposable {
  ...
  constructor(editor, instantiationService, _bulkEditService, _clipboardService, _languageFeaturesService, _quickInputService, _progressService) {
    ...
    this._postPasteWidgetManager = this._register(instantiationService.createInstance(PostEditWidgetManager, 'pasteIntoEditor', editor, pasteWidgetVisibleCtx, {
      ...
      label: localizeWithPath(_moduleId, 1, "Show paste options...")
    }));
  }
  changePasteType() {
  ...
  isPasteAsEnabled() {
    return this._editor.getOption(85).enabled && !this._editor.getOption(91);
  }
  async finishedPaste() {
  ...
  async handlePaste(e) {
    ...
    const allPotentialMimeTypes = [...e.clipboardData.types, ...(metadata?.providerCopyMimeTypes ?? []), Mimes.uriList];
  ...
  showPasteAsNoEditMessage(selections, preference) {
    MessageController.get(this._editor)?.showMessage(localizeWithPath(_moduleId, 2, "No paste edits for '{0}' found", preference instanceof HierarchicalKind ? preference.value : preference.providerId), selections[0].getStartPosition());
  }
  doPasteInline(allProviders, selections, dataTransfer, metadata, clipboardEvent) {
    const p = createCancelablePromise(async token => {
      ...
      const tokenSource = new EditorStateCancellationTokenSource(editor, 1 | 2, undefined, token);
      try {
        ...
        if (providerEdits.length) {
          ...
          }, canShowWidget, async (edit, token) => {
    ...
    this._pasteProgressManager.showWhile(selections[0].getEndPosition(), localizeWithPath(_moduleId, 3, "Running paste handlers. Click to cancel"), p);
  ...
  showPasteAsPick(preference, allProviders, selections, dataTransfer, metadata) {
      ...
      try {
        ...
        if (preference) {
          ...
          const selected = await this._quickInputService.pick(providerEdits.map(edit => ({
            ...
            placeHolder: localizeWithPath(_moduleId, 4, "Select Paste Action")
          });
    ...
    this._progressService.withProgress({
      location: 10,
      title: localizeWithPath(_moduleId, 5, "Running paste handlers")
    }, () => p);
  }
...
exports.changePasteTypeCommandId = changePasteTypeCommandId;
exports.pasteWidgetVisibleCtx = pasteWidgetVisibleCtx;