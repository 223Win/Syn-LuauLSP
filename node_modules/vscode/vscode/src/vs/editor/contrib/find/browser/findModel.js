...
const {
  SearchParams
} = require('../../../common/model/textModelSearch.js');
const {
  FindDecorations
} = require('./findDecorations.js');
const {
  ReplaceAllCommand
} = require('./replaceAllCommand.js');
const {
  parseReplaceString,
  ReplacePattern
} = require('./replacePattern.js');
const {
  RawContextKey
...
class FindModelBoundToEditorModel {
  ...
  _moveToPrevMatch(before, isRecursed = false) {
    ...
    let prevMatch = model.findPreviousMatch(this._state.searchString, position, this._state.isRegex, this._state.matchCase, this._state.wholeWord ? this._editor.getOption(131) : null, false);
    if (prevMatch && prevMatch.range.isEmpty() && prevMatch.range.getStartPosition().equals(position)) {
      position = this._prevSearchPosition(position);
      prevMatch = model.findPreviousMatch(this._state.searchString, position, this._state.isRegex, this._state.matchCase, this._state.wholeWord ? this._editor.getOption(131) : null, false);
    }
    if (!prevMatch) {
      return;
    }
  ...
  _getNextMatch(after, captureMatches, forceMove, isRecursed = false) {
    ...
    const model = this._editor.getModel();
    let position = new Position(lineNumber, column);
    let nextMatch = model.findNextMatch(this._state.searchString, position, this._state.isRegex, this._state.matchCase, this._state.wholeWord ? this._editor.getOption(131) : null, captureMatches);
    if (forceMove && nextMatch && nextMatch.range.isEmpty() && nextMatch.range.getStartPosition().equals(position)) {
      position = this._nextSearchPosition(position);
      nextMatch = model.findNextMatch(this._state.searchString, position, this._state.isRegex, this._state.matchCase, this._state.wholeWord ? this._editor.getOption(131) : null, captureMatches);
    }
    if (!nextMatch) {
      return null;
    }
  ...
  _findMatches(findScopes, captureMatches, limitResultCount) {
    ...
    return this._editor.getModel().findMatches(this._state.searchString, searchRanges, this._state.isRegex, this._state.matchCase, this._state.wholeWord ? this._editor.getOption(131) : null, captureMatches, limitResultCount);
  }
  replaceAll() {
    if (!this._hasMatches()) {
      return;
  ...
  _largeReplaceAll() {
    const searchParams = new SearchParams(this._state.searchString, this._state.isRegex, this._state.matchCase, this._state.wholeWord ? this._editor.getOption(131) : null);
...
exports.CONTEXT_FIND_WIDGET_NOT_VISIBLE = CONTEXT_FIND_WIDGET_NOT_VISIBLE;
...
exports.TogglePreserveCaseKeybinding = TogglePreserveCaseKeybinding;
