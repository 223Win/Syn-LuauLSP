...
const {
  Toggle
} = require('../../../../base/browser/ui/toggle/toggle.js');
const {
  Sash
...
const {} = require('./findWidget.css.js');
const {
  Range
...
} = require('../../../../platform/history/browser/contextScopedHistoryWidget.js');
const {
  showHistoryKeybindingHint
} = require('../../../../platform/history/browser/historyWidgetKeybindingHint.js');
const {
  asCssVariable
...
const _moduleId = "vs/editor/contrib/find/browser/findWidget";
const findSelectionIcon = registerIcon('find-selection', Codicon.selection, localizeWithPath(_moduleId, 0, 'Icon for \'Find in Selection\' in the editor find widget.'));
const findCollapsedIcon = registerIcon('find-collapsed', Codicon.chevronRight, localizeWithPath(_moduleId, 1, 'Icon to indicate that the editor find widget is collapsed.'));
const findExpandedIcon = registerIcon('find-expanded', Codicon.chevronDown, localizeWithPath(_moduleId, 2, 'Icon to indicate that the editor find widget is expanded.'));
const findReplaceIcon = registerIcon('find-replace', Codicon.replace, localizeWithPath(_moduleId, 3, 'Icon for \'Replace\' in the editor find widget.'));
const findReplaceAllIcon = registerIcon('find-replace-all', Codicon.replaceAll, localizeWithPath(_moduleId, 4, 'Icon for \'Replace All\' in the editor find widget.'));
const findPreviousMatchIcon = registerIcon('find-previous-match', Codicon.arrowUp, localizeWithPath(_moduleId, 5, 'Icon for \'Find Previous\' in the editor find widget.'));
const findNextMatchIcon = registerIcon('find-next-match', Codicon.arrowDown, localizeWithPath(_moduleId, 6, 'Icon for \'Find Next\' in the editor find widget.'));
const NLS_FIND_DIALOG_LABEL = localizeWithPath(_moduleId, 7, "Find / Replace");
const NLS_FIND_INPUT_LABEL = localizeWithPath(_moduleId, 8, "Find");
const NLS_FIND_INPUT_PLACEHOLDER = localizeWithPath(_moduleId, 9, "Find");
const NLS_PREVIOUS_MATCH_BTN_LABEL = localizeWithPath(_moduleId, 10, "Previous Match");
const NLS_NEXT_MATCH_BTN_LABEL = localizeWithPath(_moduleId, 11, "Next Match");
const NLS_TOGGLE_SELECTION_FIND_TITLE = localizeWithPath(_moduleId, 12, "Find in Selection");
const NLS_CLOSE_BTN_LABEL = localizeWithPath(_moduleId, 13, "Close");
const NLS_REPLACE_INPUT_LABEL = localizeWithPath(_moduleId, 14, "Replace");
const NLS_REPLACE_INPUT_PLACEHOLDER = localizeWithPath(_moduleId, 15, "Replace");
const NLS_REPLACE_BTN_LABEL = localizeWithPath(_moduleId, 16, "Replace");
const NLS_REPLACE_ALL_BTN_LABEL = localizeWithPath(_moduleId, 17, "Replace All");
const NLS_TOGGLE_REPLACE_MODE_BTN_LABEL = localizeWithPath(_moduleId, 18, "Toggle Replace");
const NLS_MATCHES_COUNT_LIMIT_TITLE = localizeWithPath(_moduleId, 19, "Only the first {0} results are highlighted, but all find operations work on the entire text.", MATCHES_LIMIT);
const NLS_MATCHES_LOCATION = localizeWithPath(_moduleId, 20, "{0} of {1}");
const NLS_NO_RESULTS = localizeWithPath(_moduleId, 21, "No results");
...
class FindWidget extends Widget {
  ...
  _getAriaLabel(label, currentMatch, searchString) {
    if (label === NLS_NO_RESULTS) {
      return searchString === '' ? localizeWithPath(_moduleId, 22, "{0} found", label) : localizeWithPath(_moduleId, 23, "{0} found for '{1}'", label, searchString);
    }
    if (currentMatch) {
      const ariaLabel = localizeWithPath(_moduleId, 24, "{0} found for '{1}', at {2}", label, searchString, currentMatch.startLineNumber + ':' + currentMatch.startColumn);
    ...
    return localizeWithPath(_moduleId, 25, "{0} found for '{1}'", label, searchString);
  }
    ...
    if (e.equals(ctrlKeyMod | 3)) {
      if (this._keybindingService.dispatchEvent(e, e.target)) {
        ...
        if (isWindows && isNative && !this._ctrlEnterReplaceAllWarningPrompted) {
          this._notificationService.info(localizeWithPath(_moduleId, 26, 'Ctrl+Enter now inserts line break instead of replacing all. You can modify the keybinding for editor.action.replaceAll to override this behavior.'));
...
exports.NLS_MATCHES_LOCATION = NLS_MATCHES_LOCATION;
exports.NLS_NO_RESULTS = NLS_NO_RESULTS;
exports.SimpleButton = SimpleButton;
exports.findNextMatchIcon = findNextMatchIcon;
exports.findPreviousMatchIcon = findPreviousMatchIcon;
exports.findReplaceAllIcon = findReplaceAllIcon;
