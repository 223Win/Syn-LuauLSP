...
} = require('../../../../base/browser/ui/hover/hoverDelegate2.js');
const {
...
const {} = require('./renameWidget.css.js');
const {
  applyFontInfo
} = require('../../../browser/config/domFontInfo.js');
const {
  Position
...
const _moduleId = "vs/editor/contrib/rename/browser/renameWidget";
const CONTEXT_RENAME_INPUT_VISIBLE = new RawContextKey('renameInputVisible', false, localizeWithPath(_moduleId, 0, "Whether the rename input widget is visible"));
let RenameWidget = class RenameWidget {
  ...
  getPosition() {
    ...
    const positionPreference = this._nPxAvailableBelow > candidateViewHeight * 6 ? [2, 1] : [1, 2];
    return {
  ...
  beforeRender() {
    ...
    this._label.innerText = localizeWithPath(_moduleId, 2, "{0} to Rename, {1} to Preview", this._keybindingService.lookupKeybinding(accept)?.getLabel(), this._keybindingService.lookupKeybinding(preview)?.getLabel());
  ...
  getInput(where, currentName, supportPreview, requestRenameCandidates, cts) {
    ...
    if (requestRenameCandidates === undefined) {
      ...
      disposeOnDone.add(addDisposableListener(this._inputWithButton.button, EventType.KEY_DOWN, e => {
        ...
        if (keyEvent.equals(3) || keyEvent.equals(10)) {
...
class RenameCandidateListView {
  ...
  setCandidates(candidates) {
    ...
    status(localizeWithPath(_moduleId, 3, "Received {0} rename suggestions", candidates.length));
  }
  clearCandidates() {
...
class InputWithButton {
  ...
  get domNode() {
    if (!this._domNode) {
      ...
      this._inputNode.setAttribute('aria-label', localizeWithPath(_moduleId, 4, "Rename input. Type new name and press Enter to commit."));
      this._domNode.appendChild(this._inputNode);
      this._buttonNode = document.createElement('div');
      this._buttonNode.className = 'rename-suggestions-button';
      this._buttonNode.setAttribute('tabindex', '0');
      this._buttonGenHoverText = localizeWithPath(_moduleId, 5, "Generate new name suggestions");
      this._buttonCancelHoverText = localizeWithPath(_moduleId, 6, "Cancel");
