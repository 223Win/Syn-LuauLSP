...
} = require('./renameWidget.js');
var RenameController_1;
const _moduleId = "vs/editor/contrib/rename/browser/rename";
class RenameSkeleton {
  ...
  async _provideRenameEdits(newName, i, rejects, token) {
    ...
    if (!result) {
      return this._provideRenameEdits(newName, i + 1, rejects.concat(localizeWithPath(_moduleId, 0, "No result.")), token);
  ...
  async run() {
    ...
    try {
      ...
      if (e instanceof CancellationError) {
        ...
        if (typeof e === 'string' || isMarkdownString(e)) {
          MessageController.get(this.editor)?.showMessage(e || localizeWithPath(_moduleId, 1, "An unknown error occurred while resolving rename location"), position);
        }
      }
      return undefined;
    } finally {
    ...
    const renameOperation = raceCancellation(skeleton.provideRenameEdits(inputFieldResult.newName, cts2.token), cts2.token).then(async renameResult => {
      ...
      this._bulkEditService.apply(renameResult, {
        ...
        label: localizeWithPath(_moduleId, 2, "Renaming '{0}' to '{1}'", loc?.text, inputFieldResult.newName),
        code: 'undoredo.rename',
        quotableLabel: localizeWithPath(_moduleId, 3, "Renaming {0} to {1}", loc?.text, inputFieldResult.newName),
        respectAutoSaveConfig: true
      }).then(result => {
        trace('edits applied');
        if (result.ariaSummary) {
          alert(localizeWithPath(_moduleId, 4, "Successfully renamed '{0}' to '{1}'. Summary: {2}", loc.text, inputFieldResult.newName, result.ariaSummary));
        }
      }).catch(err => {
        trace(`error when applying edits ${JSON.stringify(err, null, '\t')}`);
        this._notificationService.error(localizeWithPath(_moduleId, 5, "Rename failed to apply edits"));
        this._logService.error(err);
      });
    }, err => {
      trace('error when providing rename edits', JSON.stringify(err, null, '\t'));
      this._notificationService.error(localizeWithPath(_moduleId, 6, "Rename failed to compute edits"));
      this._logService.error(err);
    }).finally(() => {
      cts2.dispose();
    });
...
class RenameAction extends EditorAction {
  constructor() {
    super({
      ...
      label: localizeWithPath(_moduleId, 7, "Rename Symbol"),
      alias: 'Rename Symbol',
...
registerAction2(class FocusNextRenameSuggestion extends Action2 {
  constructor() {
    super({
      ...
      title: {
        ...localize2WithPath(_moduleId, 8, "Focus Next Rename Suggestion")
      },
      precondition: CONTEXT_RENAME_INPUT_VISIBLE,
      keybinding: [{
        primary: 18,
...
registerAction2(class FocusPreviousRenameSuggestion extends Action2 {
  constructor() {
    super({
      ...
      title: {
        ...localize2WithPath(_moduleId, 9, "Focus Previous Rename Suggestion")
      },
      precondition: CONTEXT_RENAME_INPUT_VISIBLE,
      keybinding: [{
        primary: 16,
...
Registry.as(Extensions.Configuration).registerConfiguration({
  ...
  properties: {
    'editor.rename.enablePreview': {
      scope: 5,
      description: localizeWithPath(_moduleId, 10, "Enable/disable the ability to preview changes before renaming"),
      default: true,
      type: 'boolean'
    }
  }
