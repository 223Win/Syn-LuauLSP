...
} = require('./bracketPairsTree/brackets.js');
const {
  lengthAdd,
  lengthGetColumnCountIfZeroLineCount,
  lengthZero
} = require('./bracketPairsTree/length.js');
const {
  parseDocument
} = require('./bracketPairsTree/parser.js');
const {
  DenseKeyProvider
} = require('./bracketPairsTree/smallImmutableSet.js');
const {
  TextBufferTokenizer
} = require('./bracketPairsTree/tokenizer.js');
function fixBracketsInLine(tokens, languageConfigurationService) {
  ...
  function processNode(node, offset) {
    ...
    } else if (node.kind === 3) ;else if (node.kind === 0 || node.kind === 1) {
