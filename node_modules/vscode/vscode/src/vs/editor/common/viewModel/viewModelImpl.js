...
} = require('../config/editorOptions.js');
const {
  CursorsController
} = require('../cursor/cursor.js');
const {
  CursorConfiguration
} = require('../cursorCommon.js');
const {
  Position
} = require('../core/position.js');
const {
  Range
} = require('../core/range.js');
const {
  InternalModelContentChangeEvent
} = require('../textModelEvents.js');
const {
  TokenizationRegistry
} = require('../languages.js');
const {
  PLAINTEXT_LANGUAGE_ID
} = require('../languages/modesRegistry.js');
const {
  tokenizeLineToHTML
} = require('../languages/textToHtmlTokenizer.js');
const {
...
} = require('../viewEvents.js');
const {
  ViewLayout
} = require('../viewLayout/viewLayout.js');
const {
  MinimapTokensColorTracker
} = require('./minimapTokensColorTracker.js');
const {
...
} = require('./viewModelDecorations.js');
const {
  ...
  ViewZonesChangedEvent$1
} = require('../viewModelEventDispatcher.js');
const {
  ViewModelLinesFromModelAsIs,
  ViewModelLinesFromProjectedModel
} = require('./viewModelLines.js');
const {
  GlyphMarginLanesModel
} = require('./glyphLanesModel.js');
class ViewModel extends Disposable {
  constructor(editorId, configuration, model, domLineBreaksComputerFactory, monospaceLineBreaksComputerFactory, scheduleAtNextAnimationFrame, languageConfigurationService, _themeService, _attachedView) {
    ...
    if (this.model.isTooLargeForTokenization()) {
      ...
      const wrappingStrategy = options.get(139);
      const wrappingInfo = options.get(146);
      const wrappingIndent = options.get(138);
  ...
  getVisibleRangesPlusViewportAboveBelow() {
    const layoutInfo = this._configuration.options.get(145);
  ...
  getRichTextToCopy(modelRanges, emptySelectionClipboard) {
    ...
    return {
      mode: languageId,
      html: `<div style="` + `color: ${colorMap[1]};` + `background-color: ${colorMap[2]};` + `font-family: ${fontFamily};` + `font-weight: ${fontInfo.fontWeight};` + `font-size: ${fontInfo.fontSize}px;` + `line-height: ${fontInfo.lineHeight}px;` + `white-space: pre;` + `">` + this._getHTMLToCopy(range, colorMap) + '</div>'
    };
  }
  ...
  revealAllCursors(source, revealHorizontal, minimalReveal = false) {
    this._withViewEventsCollector(eventsCollector => this._cursor.revealAll(eventsCollector, source, minimalReveal, 0, revealHorizontal, 0));
  }
