...
} = require('./quickInputUtils.js');
const {
  IConfigurationService
} = require('../../configuration/common/configuration.service.js');
const {
  WorkbenchHoverDelegate
} = require('../../hover/browser/hover.js');
const {
  IHoverService
} = require('../../hover/browser/hover.service.js');
const {
  QuickInputListFocus
} = require('./quickInputTree.js');
const _moduleId = "vs/platform/quickinput/browser/quickInput";
const backButton = {
  iconClass: ThemeIcon.asClassName(Codicon.quickInputBack),
  tooltip: localizeWithPath(_moduleId, 0, "Back"),
  handle: -1
};
class QuickInput extends Disposable {
  static {
    this.noPromptMessage = localizeWithPath(_moduleId, 1, "Press 'Enter' to confirm your input or 'Escape' to cancel");
  }
  constructor(ui) {
    super();
    this.ui = ui;
  ...
  getSteps() {
    if (this.step && this.totalSteps) {
      return localizeWithPath(_moduleId, 2, "{0}/{1}", this.step, this.totalSteps);
    }
    if (this.step) {
...
class QuickPick extends QuickInput {
  ...
  static {
    this.DEFAULT_ARIA_LABEL = localizeWithPath(_moduleId, 3, "Type to narrow down results.");
  }
  get quickNavigate() {
...
class InputBox extends QuickInput {
  ...
  set prompt(prompt) {
    this._prompt = prompt;
    this.noValidationMessage = prompt ? localizeWithPath(_moduleId, 4, "{0} (Press 'Enter' to confirm or 'Escape' to cancel)", prompt) : QuickInput.noPromptMessage;
    this.update();
  }
  show() {
    if (!this.visible) {
...
let QuickInputHoverDelegate = class QuickInputHoverDelegate extends WorkbenchHoverDelegate {
  ...
  getOverrideOptions(options) {
    const showHoverHint = (options.content instanceof HTMLElement ? options.content.textContent ?? '' : typeof options.content === 'string' ? options.content : options.content.value).includes('\n');
    return {
      persistence: {
        hideOnKeyDown: false
      },
