...
} = require('../../workspace/common/workspace.js');
const {
  ResourceMap
...
let UserDataProfilesService = class UserDataProfilesService extends Disposable {
  ...
  createDefaultProfile() {
    const defaultProfile = toUserDataProfile('__default__profile__', localizeWithPath(_moduleId, 0, "Default"), this.environmentService.userRoamingDataHome, this.profilesCacheHome);
    return {
      ...defaultProfile,
  ...
  async cleanUp() {
    ...
    if (await this.fileService.exists(this.profilesHome)) {
      ...
      await Promise.all((stat.children || []).filter(child => child.isDirectory && this.profiles.every(p => !this.uriIdentityService.extUri.isEqual(p.location, child.resource))).map(child => this.fileService.del(child.resource, {
        recursive: true
      })));
    }
  }
