...
const {
  PagedList
} = require('../../../base/browser/ui/list/listPaging.js');
const {
  List,
  TypeNavigationMode,
...
const {
  Table
} = require('../../../base/browser/ui/table/tableWidget.js');
const {
  TreeFindMode,
  TreeFindMatchType
} = require('../../../base/browser/ui/tree/abstractTree.js');
const {
  AsyncDataTree,
...
const {
  DataTree
} = require('../../../base/browser/ui/tree/dataTree.js');
const {
  ObjectTree,
  CompressibleObjectTree
} = require('../../../base/browser/ui/tree/objectTree.js');
const {
  Emitter,
  Event
...
} = require('../../contextkey/common/contextkeys.js');
const {
  IContextViewService
...
const {
  IListService
} = require('./listService.service.js');
const _moduleId = "vs/platform/list/browser/listService";
class ListService {
  get lastFocusedList() {
...
configurationRegistry.registerConfiguration({
  id: 'workbench',
  order: 7,
  title: localizeWithPath(_moduleId, 0, "Workbench"),
  type: 'object',
  properties: {
    [multiSelectModifierSettingKey]: {
      type: 'string',
      enum: ['ctrlCmd', 'alt'],
      markdownEnumDescriptions: [localizeWithPath(_moduleId, 1, "Maps to `Control` on Windows and Linux and to `Command` on macOS."), localizeWithPath(_moduleId, 2, "Maps to `Alt` on Windows and Linux and to `Option` on macOS.")],
      default: 'ctrlCmd',
      description: localizeWithPath(_moduleId, 3, "The modifier to be used to add an item in trees and lists to a multi-selection with the mouse (for example in the explorer, open editors and scm view). The 'Open to Side' mouse gestures - if supported - will adapt such that they do not conflict with the multiselect modifier.")
    },
    [openModeSettingKey]: {
      type: 'string',
      enum: ['singleClick', 'doubleClick'],
      default: 'singleClick',
      description: localizeWithPath(_moduleId, 4, "Controls how to open items in trees and lists using the mouse (if supported). Note that some trees and lists might choose to ignore this setting if it is not applicable.")
    },
    [horizontalScrollingKey]: {
      type: 'boolean',
      default: false,
      description: localizeWithPath(_moduleId, 5, "Controls whether lists and trees support horizontal scrolling in the workbench. Warning: turning on this setting has a performance implication.")
    },
    [scrollByPageKey]: {
      type: 'boolean',
      default: false,
      description: localizeWithPath(_moduleId, 6, "Controls whether clicks in the scrollbar scroll page by page.")
    },
    [treeIndentKey]: {
      type: 'number',
      default: 8,
      minimum: 4,
      maximum: 40,
      description: localizeWithPath(_moduleId, 7, "Controls tree indentation in pixels.")
    },
    [treeRenderIndentGuidesKey]: {
      type: 'string',
      enum: ['none', 'onHover', 'always'],
      default: 'onHover',
      description: localizeWithPath(_moduleId, 8, "Controls whether the tree should render indent guides.")
    },
    [listSmoothScrolling]: {
      type: 'boolean',
      default: false,
      description: localizeWithPath(_moduleId, 9, "Controls whether lists and trees have smooth scrolling.")
    },
    [mouseWheelScrollSensitivityKey]: {
      type: 'number',
      default: 1,
      markdownDescription: localizeWithPath(_moduleId, 10, "A multiplier to be used on the `deltaX` and `deltaY` of mouse wheel scroll events.")
    },
    [fastScrollSensitivityKey]: {
      type: 'number',
      default: 5,
      markdownDescription: localizeWithPath(_moduleId, 11, "Scrolling speed multiplier when pressing `Alt`.")
    },
    [defaultFindModeSettingKey]: {
      type: 'string',
      enum: ['highlight', 'filter'],
      enumDescriptions: [localizeWithPath(_moduleId, 12, "Highlight elements when searching. Further up and down navigation will traverse only the highlighted elements."), localizeWithPath(_moduleId, 13, "Filter elements when searching.")],
      default: 'highlight',
      description: localizeWithPath(_moduleId, 14, "Controls the default find mode for lists and trees in the workbench.")
    },
    [keyboardNavigationSettingKey]: {
      type: 'string',
      enum: ['simple', 'highlight', 'filter'],
      enumDescriptions: [localizeWithPath(_moduleId, 15, "Simple keyboard navigation focuses elements which match the keyboard input. Matching is done only on prefixes."), localizeWithPath(_moduleId, 16, "Highlight keyboard navigation highlights elements which match the keyboard input. Further up and down navigation will traverse only the highlighted elements."), localizeWithPath(_moduleId, 17, "Filter keyboard navigation will filter out and hide all the elements which do not match the keyboard input.")],
      default: 'highlight',
      description: localizeWithPath(_moduleId, 18, "Controls the keyboard navigation style for lists and trees in the workbench. Can be simple, highlight and filter."),
      deprecated: true,
      deprecationMessage: localizeWithPath(_moduleId, 19, "Please use 'workbench.list.defaultFindMode' and	'workbench.list.typeNavigationMode' instead.")
    },
    [defaultFindMatchTypeSettingKey]: {
      type: 'string',
      enum: ['fuzzy', 'contiguous'],
      enumDescriptions: [localizeWithPath(_moduleId, 20, "Use fuzzy matching when searching."), localizeWithPath(_moduleId, 21, "Use contiguous matching when searching.")],
      default: 'fuzzy',
      description: localizeWithPath(_moduleId, 22, "Controls the type of matching used when searching lists and trees in the workbench.")
    },
    [treeExpandMode]: {
      type: 'string',
      enum: ['singleClick', 'doubleClick'],
      default: 'singleClick',
      description: localizeWithPath(_moduleId, 23, "Controls how tree folders are expanded when clicking the folder names. Note that some trees and lists might choose to ignore this setting if it is not applicable.")
    },
    [treeStickyScroll]: {
      type: 'boolean',
      default: true,
      description: localizeWithPath(_moduleId, 24, "Controls whether sticky scrolling is enabled in trees.")
    },
    [treeStickyScrollMaxElements]: {
      type: 'number',
      minimum: 1,
      default: 7,
      markdownDescription: localizeWithPath(_moduleId, 25, "Controls the number of sticky elements displayed in the tree when `#workbench.tree.enableStickyScroll#` is enabled.")
    },
    [typeNavigationModeSettingKey]: {
      type: 'string',
      enum: ['automatic', 'trigger'],
      default: 'automatic',
      markdownDescription: localizeWithPath(_moduleId, 26, "Controls how type navigation works in lists and trees in the workbench. When set to `trigger`, type navigation begins once the `list.triggerTypeNavigation` command is run.")
    }
  }
});
exports.ListService = ListService;
exports.RawWorkbenchListFocusContextKey = RawWorkbenchListFocusContextKey;
exports.RawWorkbenchListScrollAtBoundaryContextKey = RawWorkbenchListScrollAtBoundaryContextKey;
...
exports.WorkbenchListDoubleSelection = WorkbenchListDoubleSelection;
exports.WorkbenchListFocusContextKey = WorkbenchListFocusContextKey;
exports.WorkbenchListHasSelectionOrFocus = WorkbenchListHasSelectionOrFocus;
exports.WorkbenchListMultiSelection = WorkbenchListMultiSelection;
exports.WorkbenchListScrollAtBottomContextKey = WorkbenchListScrollAtBottomContextKey;
exports.WorkbenchListScrollAtTopContextKey = WorkbenchListScrollAtTopContextKey;
exports.WorkbenchListSelectionNavigation = WorkbenchListSelectionNavigation;
exports.WorkbenchListSupportsFind = WorkbenchListSupportsFind;
exports.WorkbenchListSupportsMultiSelectContextKey = WorkbenchListSupportsMultiSelectContextKey;
...
exports.WorkbenchTreeElementCanCollapse = WorkbenchTreeElementCanCollapse;
exports.WorkbenchTreeElementCanExpand = WorkbenchTreeElementCanExpand;
exports.WorkbenchTreeElementHasChild = WorkbenchTreeElementHasChild;
exports.WorkbenchTreeElementHasParent = WorkbenchTreeElementHasParent;
exports.WorkbenchTreeFindOpen = WorkbenchTreeFindOpen;
exports.WorkbenchTreeStickyScrollFocused = WorkbenchTreeStickyScrollFocused;
