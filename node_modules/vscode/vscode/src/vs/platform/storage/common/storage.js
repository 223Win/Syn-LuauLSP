...
const {
  ...
  StorageHint
} = require('../../../base/parts/storage/common/storage.js');
const {
  isUserDataProfile
} = require('../../userDataProfile/common/userDataProfile.js');
  ...
  async flush(reason = WillSaveStateReason.NONE) {
    ...
    switch (reason) {
      case WillSaveStateReason.NONE:
        await Promises.settled([applicationStorage?.whenFlushed() ?? Promise.resolve(), profileStorage?.whenFlushed() ?? Promise.resolve(), workspaceStorage?.whenFlushed() ?? Promise.resolve()]);
        break;
      case WillSaveStateReason.SHUTDOWN:
        await Promises.settled([applicationStorage?.flush(0) ?? Promise.resolve(), profileStorage?.flush(0) ?? Promise.resolve(), workspaceStorage?.flush(0) ?? Promise.resolve()]);
        break;
    }
  }
  async log() {
...
exports.WillSaveStateReason = WillSaveStateReason;
