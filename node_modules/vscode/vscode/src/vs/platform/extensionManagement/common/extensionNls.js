...
const _moduleId = "vs/platform/extensionManagement/common/extensionNls";
  ...
  const processEntry = (obj, key, command) => {
    ...
    if (isString(value)) {
      ...
      if (length > 1 && str[0] === '%' && str[length - 1] === '%') {
        ...
        if (!message) {
          if (!originalMessage) {
            logger.warn(`[${extensionManifest.name}]: ${localizeWithPath(_moduleId, 0, "Couldn't find message for key {0}.", messageKey)}`);
          }
          return;
        }
