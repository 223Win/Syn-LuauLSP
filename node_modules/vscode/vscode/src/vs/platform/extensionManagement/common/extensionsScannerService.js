...
} = require('../../../base/common/jsonErrorMessages.js');
const {
  Disposable
...
const {
  semverExports
} = require('../../../../../../external/vscode-semver/semver.js');
...
const {
  IEnvironmentService
} = require('../../environment/common/environment.service.js');
const {
  computeTargetPlatform,
  areSameExtensions,
  ExtensionKey,
  getGalleryExtensionId
} = require('./extensionManagementUtil.js');
const {
  UNDEFINED_PUBLISHER,
...
} = require('../../extensions/common/extensionValidator.js');
const {
  toFileOperationResult
} = require('../../files/common/files.js');
const {
  IFileService
} = require('../../files/common/files.service.js');
const {
...
const {
  IProductService
} = require('../../product/common/productService.service.js');
const {
  Emitter
...
} = require('./extensionsProfileScannerService.js');
const {
  IExtensionsProfileScannerService
} = require('./extensionsProfileScannerService.service.js');
const {
  IUserDataProfilesService
} = require('../../userDataProfile/common/userDataProfile.service.js');
const {
  IUriIdentityService
} = require('../../uriIdentity/common/uriIdentity.service.js');
const {
  localizeManifest
} = require('./extensionNls.js');
...
let AbstractExtensionsScannerService = class AbstractExtensionsScannerService extends Disposable {
  ...
  async scanAllExtensions(systemScanOptions, userScanOptions, includeExtensionsUnderDev) {
    const [system, user] = await Promise.all([this.scanSystemExtensions(systemScanOptions), this.scanUserExtensions(userScanOptions)]);
  ...
  async createExtensionScannerInput(location, profile, type, excludeObsolete, language$1, validate, profileScanOptions, productVersion) {
    ...
    return new ExtensionScannerInput(location, mtime, applicationExtensionsLocation, applicationExtensionsLocationMtime, profile, profileScanOptions, type, excludeObsolete, validate, productVersion.version, productVersion.date, this.productService.commit, !this.environmentService.isBuilt, language$1, translations);
  }
...
AbstractExtensionsScannerService = __decorate([__param(4, IUserDataProfilesService), __param(5, IExtensionsProfileScannerService), __param(6, IFileService), __param(7, ILogService), __param(8, IEnvironmentService), __param(9, IProductService), __param(10, IUriIdentityService), __param(11, IInstantiationService)], AbstractExtensionsScannerService);
  ...
  async getLocalizedMessages(extensionLocation, extensionManifest, nlsConfiguration) {
    ...
    const reportErrors = (localized, errors) => {
      errors.forEach(error => {
        this.logService.error(this.formatMessage(extensionLocation, localizeWithPath(_moduleId, 0, "Failed to parse {0}: {1}.", localized?.path, getParseErrorMessage(error.error))));
      });
    };
  ...
  async translateManifest(extensionLocation, extensionManifest, nlsConfiguration) {
    ...
    if (localizedMessages) {
      try {
        ...
        if (errors.length > 0) {
          errors.forEach(error => {
            this.logService.error(this.formatMessage(extensionLocation, localizeWithPath(_moduleId, 0, "Failed to parse {0}: {1}.", localizedMessages.default?.path, getParseErrorMessage(error.error))));
          });
          return extensionManifest;
        } else if (getNodeType(localizedMessages) !== 'object') {
          this.logService.error(this.formatMessage(extensionLocation, localizeWithPath(_moduleId, 1, "Invalid format {0}: JSON object expected.", localizedMessages.default?.path)));
          return extensionManifest;
        }
...
ExtensionManifestTranslator = __decorate([__param(0, IFileService), __param(1, ILogService)], ExtensionManifestTranslator);
let ExtensionsScanner = class ExtensionsScanner extends ExtensionManifestTranslator {
  ...
  async scanExtensionManifest(extensionLocation) {
    ...
    try {
      ...
      if (toFileOperationResult(error) !== 1) {
        this.logService.error(this.formatMessage(extensionLocation, localizeWithPath(_moduleId, 2, "Cannot read file {0}: {1}.", manifestLocation.path, error.message)));
      }
      return null;
    }
    let manifest;
    try {
      ...
      for (const e of errors) {
        this.logService.error(this.formatMessage(extensionLocation, localizeWithPath(_moduleId, 3, "Failed to parse {0}: [{1}, {2}] {3}.", manifestLocation.path, e.offset, e.length, getParseErrorMessage(e.error))));
      }
      return null;
    }
    if (getNodeType(manifest) !== 'object') {
      this.logService.error(this.formatMessage(extensionLocation, localizeWithPath(_moduleId, 4, "Invalid manifest file {0}: Not an JSON object.", manifestLocation.path)));
      return null;
    }
    return manifest;
  }
};
ExtensionsScanner = __decorate([__param(1, IExtensionsProfileScannerService), __param(2, IUriIdentityService), __param(3, IFileService), __param(4, ILogService)], ExtensionsScanner);
...
CachedExtensionsScanner = __decorate([__param(2, IUserDataProfilesService), __param(3, IExtensionsProfileScannerService), __param(4, IUriIdentityService), __param(5, IFileService), __param(6, ILogService)], CachedExtensionsScanner);
exports.AbstractExtensionsScannerService = AbstractExtensionsScannerService;
exports.ExtensionManifestTranslator = ExtensionManifestTranslator;
