...
const {} = require('../../../base/common/path.js');
const {} = require('../../../base/common/strings.js');
const {
  isNumber
...
function ensureFileSystemProviderError(error) {
  if (!error) {
    return createFileSystemProviderError(localizeWithPath(_moduleId, 0, "Unknown Error"), FileSystemProviderErrorCode.Unknown);
  }
  return error;
}
...
class ByteSize {
  ...
  static formatSize(size) {
    ...
    if (size < ByteSize.KB) {
      return localizeWithPath(_moduleId, 1, "{0}B", size.toFixed(0));
    }
    if (size < ByteSize.MB) {
      return localizeWithPath(_moduleId, 2, "{0}KB", (size / ByteSize.KB).toFixed(2));
    }
    if (size < ByteSize.GB) {
      return localizeWithPath(_moduleId, 3, "{0}MB", (size / ByteSize.MB).toFixed(2));
    }
    if (size < ByteSize.TB) {
      return localizeWithPath(_moduleId, 4, "{0}GB", (size / ByteSize.GB).toFixed(2));
    }
    return localizeWithPath(_moduleId, 5, "{0}TB", (size / ByteSize.TB).toFixed(2));
  }
}
...
exports.AutoSaveConfiguration = AutoSaveConfiguration;
exports.ByteSize = ByteSize;
exports.ETAG_DISABLED = ETAG_DISABLED;
exports.FILES_ASSOCIATIONS_CONFIG = FILES_ASSOCIATIONS_CONFIG;
exports.FILES_EXCLUDE_CONFIG = FILES_EXCLUDE_CONFIG;
exports.FILES_READONLY_EXCLUDE_CONFIG = FILES_READONLY_EXCLUDE_CONFIG;
exports.FILES_READONLY_FROM_PERMISSIONS_CONFIG = FILES_READONLY_FROM_PERMISSIONS_CONFIG;
exports.FILES_READONLY_INCLUDE_CONFIG = FILES_READONLY_INCLUDE_CONFIG;
...
exports.HotExitConfiguration = HotExitConfiguration;
