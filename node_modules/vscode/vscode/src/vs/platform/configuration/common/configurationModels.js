...
const {
  ...
  Extensions
} = require('./configurationRegistry.js');
const {
  Registry
...
class ConfigurationModelParser {
  ...
  parseRaw(raw, options) {
    ...
    this._configurationModel = new ConfigurationModel(contents, keys, overrides, hasExcludedProperties ? [raw] : undefined, this.logService);
  ...
  filter(properties, configurationProperties, filterOverriddenProperties, options) {
    ...
    for (const key in properties) {
      if (OVERRIDE_PROPERTY_REGEX.test(key) && filterOverriddenProperties) {
        ...
        if (!options.exclude?.includes(key) && (options.include?.includes(key) || (scope === undefined || options.scopes === undefined || options.scopes.includes(scope)) && !(options.skipRestricted && propertySchema?.restricted))) {
...
let Configuration$1 = class Configuration {
  ...
  compareAndUpdateDefaultConfiguration(defaults, keys) {
    ...
    for (const key of keys) {
      for (const overrideIdentifier of overrideIdentifiersFromKey(key)) {
        ...
        const keys = [...toKeys.filter(key => fromKeys.indexOf(key) === -1), ...fromKeys.filter(key => toKeys.indexOf(key) === -1), ...fromKeys.filter(key => !equals$1(this._defaultConfiguration.override(overrideIdentifier).getValue(key), defaults.override(overrideIdentifier).getValue(key)))];
  ...
  compareAndUpdateLocalUserConfiguration(user) {
    const {
      ...
      removed,
      overrides
    } = compare(this.localUserConfiguration, user);
    const keys = [...added, ...updated, ...removed];
    if (keys.length) {
  ...
  compareAndUpdateRemoteUserConfiguration(user) {
    const {
      ...
      removed,
      overrides
    } = compare(this.remoteUserConfiguration, user);
    const keys = [...added, ...updated, ...removed];
    if (keys.length) {
  ...
  compareAndUpdateWorkspaceConfiguration(workspaceConfiguration) {
    const {
      ...
      removed,
      overrides
    } = compare(this.workspaceConfiguration, workspaceConfiguration);
    const keys = [...added, ...updated, ...removed];
    if (keys.length) {
  ...
  compareAndUpdateFolderConfiguration(resource, folderConfiguration) {
    ...
    const {
      ...
      removed,
      overrides
    } = compare(currentFolderConfiguration, folderConfiguration);
    const keys = [...added, ...updated, ...removed];
  ...
  compareAndDeleteFolderConfiguration(folder) {
    ...
    const {
      ...
      removed,
      overrides
    } = compare(folderConfig, undefined);
    return {
      keys: [...added, ...updated, ...removed],
      overrides
    };
...
function compare(from, to) {
  const {
    ...
    removed,
    updated
  } = compareConfigurationContents(to?.rawConfiguration, from?.rawConfiguration);
  const overrides = [];
