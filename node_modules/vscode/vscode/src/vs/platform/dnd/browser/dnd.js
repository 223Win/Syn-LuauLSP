...
} = require('../../files/browser/htmlFileSystemProvider.js');
const {
  WebFileSystemAccess
} = require('../../files/browser/webFileSystemAccess.js');
const {
  ByteSize
...
const {
  extractSelection
} = require('../../opener/common/opener.js');
const {
  Registry
} = require('../../registry/common/platform.js');
const _moduleId = "vs/platform/dnd/browser/dnd";
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    if (file) {
      if (file.size > 100 * ByteSize.MB) {
        dialogService.warn(localizeWithPath(_moduleId, 0, "File is too large to open as untitled editor. Please upload it first into the file explorer and then try again."));
        continue;
      }
      const result = new DeferredPromise();
      results.push(result);
...
exports.CodeDataTransfers = CodeDataTransfers;
