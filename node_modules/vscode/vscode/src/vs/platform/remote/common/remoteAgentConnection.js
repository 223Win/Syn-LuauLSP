...
const {
  Client,
  PersistentProtocol
} = require('../../../base/parts/ipc/common/ipc.net.js');
const {
  RemoteAuthorityResolverError
} = require('./remoteAuthorityResolver.js');
...
async function doConnectRemoteAgentManagement(options, timeoutCancellationToken) {
  const {
    protocol
  } = await connectToRemoteExtensionHostAgentAndReadOneMessage(options, 1, undefined, timeoutCancellationToken);
  return {
    protocol
  };
}
async function doConnectRemoteAgentExtensionHost(options, startArguments, timeoutCancellationToken) {
  const {
    protocol,
    firstMessage
  } = await connectToRemoteExtensionHostAgentAndReadOneMessage(options, 2, startArguments, timeoutCancellationToken);
