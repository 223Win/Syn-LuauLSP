...
class Gesture extends Disposable {
  ...
  constructor() {
    ...
    this._register(Event.runAndSubscribe(onDidRegisterWindow, ({
      ...
      disposables.add(addDisposableListener(window.document, 'touchstart', e => this.onTouchStart(e), {
        passive: false
      }));
      disposables.add(addDisposableListener(window.document, 'touchend', e => this.onTouchEnd(window, e)));
      disposables.add(addDisposableListener(window.document, 'touchmove', e => this.onTouchMove(e), {
        passive: false
      }));
    }, {
      window: mainWindow,
  ...
  onTouchEnd(targetWindow, e) {
    ...
    for (let i = 0, len = e.changedTouches.length; i < len; i++) {
      ...
      if (holdTime < Gesture.HOLD_DELAY && Math.abs(data.initialPageX - tail(data.rollingPageX)) < 30 && Math.abs(data.initialPageY - tail(data.rollingPageY)) < 30) {
        ...
        this.inertia(targetWindow, dispatchTo, timestamp, Math.abs(deltaX) / deltaT, deltaX > 0 ? 1 : -1, finalX, Math.abs(deltaY) / deltaT, deltaY > 0 ? 1 : -1, finalY);
      }
