...
const _moduleId = "vs/base/browser/ui/menu/menubar";
const $ = $$1;
var MenubarState;
...
class MenuBar extends Disposable {
  ...
  createOverflowMenu() {
    const label = this.isCompact ? localizeWithPath(_moduleId, 0, 'Application Menu') : localizeWithPath(_moduleId, 1, 'More');
    const buttonElement = $('div.menubar-menu-button', {
      'role': 'menuitem',
    ...
    const titleElement = $('div.menubar-menu-title.toolbar-toggle-more' + ThemeIcon.asCSSSelector(Codicon.menuBarMore), {
      'role': 'none',
      'aria-hidden': true
    });
    buttonElement.appendChild(titleElement);
  ...
  updateLabels(titleElement, buttonElement, label) {
    ...
    if (this.options.enableMnemonics) {
      ...
      if (escMatch) {
        ...
        titleElement.append(ltrim(replaceDoubleEscapes(cleanLabel.substr(0, escMatch.index)), ' '), $('mnemonic', {
          'aria-hidden': 'true'
        }, escMatch[3]), rtrim(replaceDoubleEscapes(cleanLabel.substr(escMatch.index + escMatch[0].length)), ' '));
      } else {
