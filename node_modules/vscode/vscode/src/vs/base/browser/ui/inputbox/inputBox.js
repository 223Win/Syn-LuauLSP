...
const {
  renderFormattedText,
  renderText
} = require('../../formattedTextRenderer.js');
const {
  ActionBar
...
const {
  ScrollableElement
} = require('../scrollbar/scrollableElement.js');
const {
  Widget
} = require('../widget.js');
const {
...
const {
  HistoryNavigator
} = require('../../../common/history.js');
const {
  equals
} = require('../../../common/objects.js');
const {} = require('./inputBox.css.js');
const {
  localizeWithPath
} = require('../../../../nls.js');
const _moduleId = "vs/base/browser/ui/inputbox/inputBox";
const $ = $$1;
...
class InputBox extends Widget {
  ...
  _showMessage() {
    ...
    if (this.message.type === 3) {
      alertText = localizeWithPath(_moduleId, 0, "Error: {0}", this.message.content);
    } else if (this.message.type === 2) {
      alertText = localizeWithPath(_moduleId, 1, "Warning: {0}", this.message.content);
    } else {
      alertText = localizeWithPath(_moduleId, 2, "Info: {0}", this.message.content);
    }
    alert(alertText);
    this.state = 'open';
  }
...
class HistoryInputBox extends InputBox {
  constructor(container, contextViewProvider, options) {
    const NLS_PLACEHOLDER_HISTORY_HINT_SUFFIX_NO_PARENS = localizeWithPath(_moduleId, 3, ' or {0} for history', `\u21C5`);
    const NLS_PLACEHOLDER_HISTORY_HINT_SUFFIX_IN_PARENS = localizeWithPath(_moduleId, 4, ' ({0} for history)', `\u21C5`);
  ...
  showNextValue() {
    ...
    this.value = next ?? '';
    status(this.value ? this.value : localizeWithPath(_moduleId, 5, "Cleared Input"));
  }
  showPreviousValue() {
    if (!this.history.has(this.value)) {
      this.addToHistory();
...
exports.unthemedInboxStyles = unthemedInboxStyles;