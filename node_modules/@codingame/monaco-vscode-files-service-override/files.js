const {
  __decorate,
  __param
} = require('vscode/external/tslib/tslib.es6.js');
const {
  StandaloneServices
} = require('vscode/vscode/vs/editor/standalone/browser/standaloneServices');
const {
  SyncDescriptor
} = require('vscode/vscode/vs/platform/instantiation/common/descriptors');
const {
  mkdirp,
  FileService
} = require('./vscode/src/vs/platform/files/common/fileService.js');
const {
  ILogService
} = require('vscode/vscode/vs/platform/log/common/log.service');
const {
  InMemoryFileSystemProvider
} = require('vscode/vscode/vs/platform/files/common/inMemoryFilesystemProvider');
exports.InMemoryFileSystemProvider = InMemoryFileSystemProvider;
const {
  URI
} = require('vscode/vscode/vs/base/common/uri');
const {
  IFileService
} = require('vscode/vscode/vs/platform/files/common/files.service');
const {
  FileType,
  FilePermission,
...
} = require('vscode/vscode/vs/platform/files/common/files');
exports.FileChangeType = FileChangeType;
exports.FilePermission = FilePermission;
exports.FileSystemProviderCapabilities = FileSystemProviderCapabilities;
...
const {
  ...
  DisposableStore,
  toDisposable
} = require('vscode/vscode/vs/base/common/lifecycle');
const {
  extUri,
  joinPath
} = require('vscode/vscode/vs/base/common/resources');
const {
  Emitter,
  Event
} = require('vscode/vscode/vs/base/common/event');
const {
  HTMLFileSystemProvider
} = require('vscode/vscode/vs/platform/files/browser/htmlFileSystemProvider');
exports.HTMLFileSystemProvider = HTMLFileSystemProvider;
const {
  Schemas
} = require('vscode/vscode/vs/base/common/network');
const {
  IndexedDBFileSystemProvider
} = require('./vscode/src/vs/platform/files/browser/indexedDBFileSystemProvider.js');
const {
  IndexedDB
} = require('vscode/vscode/vs/base/browser/indexedDB');
exports.IndexedDB = IndexedDB;
const {
  ITelemetryService
} = require('vscode/vscode/vs/platform/telemetry/common/telemetry.service');
const {
  BufferLogger
} = require('vscode/vscode/vs/platform/log/common/bufferLog');
const {
  ITextFileService
} = require('vscode/vscode/vs/workbench/services/textfile/common/textfiles.service');
const {
  BrowserTextFileService
} = require('./vscode/src/vs/workbench/services/textfile/browser/browserTextFileService.js');
const {
  FilesConfigurationService
} = require('vscode/vscode/vs/workbench/services/filesConfiguration/common/filesConfigurationService');
const {
  IFilesConfigurationService
} = require('vscode/vscode/vs/workbench/services/filesConfiguration/common/filesConfigurationService.service');
const {
  BrowserElevatedFileService
} = require('./vscode/src/vs/workbench/services/files/browser/elevatedFileService.js');
const {
  IElevatedFileService
} = require('vscode/vscode/vs/workbench/services/files/common/elevatedFileService.service');
const {
  registerServiceInitializePreParticipant,
  checkServicesNotInitialized
} = require('vscode/lifecycle');
const {
  logsPath
} = require('vscode/workbench');
const {} = require('vscode/vscode/vs/workbench/contrib/files/browser/files.contribution._configuration');
...
class RegisteredFileSystemProvider extends Disposable {
  ...
  registerFile(file) {
    ...
    disposableStore.add(file.onDidRename(({
      from,
      to
    }) => {
  ...
  async readdir(resource) {
    const includedPaths = Array.from(this.files.keys()).map(uri => URI.parse(uri)).filter(uri => uri.authority === resource.authority && uri.path.startsWith(addTrailingSlash(resource.path))).map(uri => extUri.relativePath(resource, uri));
    ...
    return [...files.map(path => [path, FileType.File]), ...directories.map(path => [path, FileType.Directory])];
  }
  ...
  async readFromDelegates(caller) {
    ...
    for (const delegate of this.delegates) {
      try {
        ...
        if (err instanceof FileSystemProviderError && [FileSystemProviderErrorCode.NoPermissions, FileSystemProviderErrorCode.FileNotFound, FileSystemProviderErrorCode.Unavailable].includes(err.code)) {
          continue;
        }
        throw err;
      }
...
class DelegateFileSystemProvider {
  constructor(options) {
    ...
    this.readFile = this.options.delegate.readFile != null ? resource => {
      return this.options.delegate.readFile(this.options.toDelegate(resource));
    } : undefined;
    this.writeFile = this.options.delegate.writeFile != null ? (resource, content, opts) => {
...
void userDataFileSystemProvider.mkdir(URI.from({
...
FileServiceOverride = __decorate([__param(1, ITelemetryService)], FileServiceOverride);
...
exports.DelegateFileSystemProvider = DelegateFileSystemProvider;
exports.EmptyFileSystemProvider = EmptyFileSystemProvider;
exports.IndexedDBFileSystemProvider = IndexedDBFileSystemProvider;
exports.OverlayFileSystemProvider = OverlayFileSystemProvider;
exports.RegisteredFile = RegisteredFile;
exports.RegisteredFileSystemProvider = RegisteredFileSystemProvider;
exports.RegisteredMemoryFile = RegisteredMemoryFile;
exports.RegisteredReadOnlyFile = RegisteredReadOnlyFile;
exports.createIndexedDBProviders = createIndexedDBProviders;
exports.default = getServiceOverride;
exports.initFile = initFile;
exports.registerCustomProvider = registerCustomProvider;
exports.registerExtensionFile = registerExtensionFile;
exports.registerFileSystemOverlay = registerFileSystemOverlay;
exports.registerHTMLFileSystemProvider = registerHTMLFileSystemProvider;